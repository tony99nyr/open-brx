...
All control is over the documented Bluetooth serial protocol (plus IR / nRF *observation*). Power-cycling always restores a tagger; Battle Company's USB updater is the factory-restore path. This is also why the project stays compatible with stock and community gear.
...
Protocol discovery and the tagger-rider ESP32 concept originate with **LaserTagMods** (JEDGE / JBOX). The objective-node reference design is **Jay / Extreme Laser Tag's JBOX Mini**. Owner-community knowledge (including David Knox's audio map) informed the sound bank. Open BRX is a fresh, independent, clean-room implementation — facts restated with credit, no code copied, official PDFs linked rather than rehosted.
Source: README.md §Credit, docs/VISION.md §Sourcing/policy, docs/spec/README.md §7

## Sequencing: prove → open-source → let demand pull.
The project builds the four units its owner needs, hardens them in real games, and open-sources everything; assembled Companions / kits / fleet bundles only follow if people ask *and* it's proven reliable.
