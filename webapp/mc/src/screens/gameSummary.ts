 ...
 *  server leaves it exactly as the operator left it. Predicting 2/2 for it made the confirm line
 *  promise a move the confirming tap would not make. Stated on the `$TID`s like the server, not the
 *  team ids: two declared teams sharing one tid are ONE side however they are named. */
export function predictedSplit(
  players: { player_num: number; team_id: string | null }[],
  prevTeams: { team_id: string }[],
  newTeams: { team_id: string; tid?: number }[],
  ...
  // `one_team_fault()`: >= 2 declared teams, >= 2 rostered players, and fewer than 2 POPULATED $TIDs.
 ...
 *  was. */
export function splitLine(players: { player_num: number; team_id: string | null }[], prevTeams: { team_id: string }[], newTeams: { team_id: string; tid?: number }[]): string {
