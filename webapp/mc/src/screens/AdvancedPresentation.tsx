            ...
            <>
              ...
              {view.summary.headset && (() => {
                ...
                const items: [string, string][] = [
                  ...
                  ['WHILE OUT', hs.death === 'native' || hs.death === 'flash' ? 'NATIVE BRIGHT FLASH, 0.75 S' : col(hs.death) + ' BLINK + NATIVE FLASH'], ['ON RESPAWN', hs.respawn_flash ? 'WHITE FLASH' : 'NONE'],
                  ...
                  <tbody>
                    {view.events.map(r => (
                      <tr key={r.event} data-testid={`pres-row-${r.event}`} aria-label={r.enabled ? undefined : `${r.event} sound off`}
                        ...
                        <td style={{ padding: '6px 8px', whiteSpace: 'nowrap' }}><Colour idx={r.headset} />{r.flash ? <span style={{ marginLeft: 8, color: T.ok }} title="the headset's small flash LED, native-bright">⚡ {r.flash.toUpperCase()} FLASH</span> : null}</td>
