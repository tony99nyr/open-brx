// UNROSTERED PHONES (2026-09-13, A38). Field 2026-09-12: "4 guns connected, only 2 in lobby" — a
  ...
  return (
    <button type="button" data-unrostered-phones={n} className="hov-acc-ink hit44" onClick={() => setView('muster')}
      ...
      style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', boxSizing: 'border-box', textAlign: 'left',
               background: 'rgba(255,193,7,.08)', border: `1px solid ${T.warn}`, borderLeft: `3px solid ${T.warn}`,
               padding: '10px 14px', cursor: 'pointer', font: F.chk(700, 12), letterSpacing: '.06em', lineHeight: 1.5, color: T.warn, minHeight: 44, ...style }}>
      ▲ {n} CONNECTED PHONE{n === 1 ? '' : 'S'} NOT IN THE ROSTER — CLAIM {n === 1 ? 'IT' : 'THEM'} ON ARMORY ▸
    </button>
  );
}
