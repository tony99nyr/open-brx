...
export function EvictButton({ nodeId, label = 'EVICT' }: { nodeId: string; label?: string }) {
  ...
  if (!arm) return <GhostButton size={11} pad="2px 8px" title={`Kick node ${nodeId}: closes its link and frees its player (confirm step)`} onClick={() => setArm(true)}>{label}</GhostButton>;
  return (
    <span style={{ display: 'inline-flex', gap: 4 }}>
      <GhostButton size={11} pad="2px 8px" color={T.bad} border={T.bad} hoverClass="hov-badbg" title="Confirm eviction" onClick={async () => { setArm(false); await run(() => api.evictNode(nodeId)); }}>CONFIRM KICK</GhostButton>
      <GhostButton size={11} pad="2px 8px" onClick={() => setArm(false)}>✕</GhostButton>
    </span>
  );
}
