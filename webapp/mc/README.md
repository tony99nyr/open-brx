...
| flag | what it demos |
|---|---|
| `?mock&faults=1` | **A36/A37 — the four config-proof states, one gun each.** GUN-A acks an OLDER `config_id` (a stale ack: `ok: true`, which is exactly why truthiness alone missed it in the field), GUN-B echoes a mismatched `$ALCD` (`GUN ECHO ≠ CONFIG`), GUN-C reports a pool above what its head grants (`GUN POOL ≠ CONFIG`), and GUN-E answers with `$START`'s `$LCD` only — **NOT ECHOED**, the ordinary answer on v4.32 firmware, rendered neutral grey and never counted as a proof. Sticky **until the first RE-PUSH**: the three reds survive the initial push (the point is to look at them) and the `RE-PUSH CONFIG` button on LOBBY clears them, because on a real server that is exactly what a re-push does. GUN-E's NOT ECHOED row stays: it is the firmware declining to echo its weapon, and no number of pushes changes that. Without it the mock always acked with the config it had just pushed and none of the four could be shown. |
