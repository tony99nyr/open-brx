...
item is on air or waiting is voice-silent. Its line is dropped, not held for later. Its banner or card still shows in
its turn. The log says `<kind> silent: kill streak on air`. A card that was muted stays muted if a kill displaces it
...
CONFIRMED shows first, and TAKES THE LEAD follows when the kill card's slot ends, without its line (silent during a
kill streak). `app/tools/screens.mjs` checks that
