...
`python.exe -m brx_mcp soak <address> <pattern> <minutes> […]` is the screamers-investigation
instrument (docs/bench-screamers-2026-09-19.md Phase C); it touches a gun, so it runs on Windows
Python like every other `brx-mcp` command. Every pattern and flag is in `python -m brx_mcp`'s usage
text (`mcp/brx_mcp/__main__.py`'s module docstring) — that is the canonical, current list; do not
duplicate it here.
