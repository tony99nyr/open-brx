...
**🔴 blocking, at a glance (2026-09-17):** **F208** a gun can die with the HUD holding the player alive and no
way back (phone-side detection built 2026-09-16, bench gate remains) · **F209** the respawn delay collapses in
a burst and spawn protection does not hold (both halves built at the desk 2026-09-16, bench gate remains), both
from [`game-test-2026-09-13.md`](game-test-2026-09-13.md), root-caused from the MC store.
...
- **F208 🔴 A GUN CAN DIE WHILE THE HUD HOLDS THE PLAYER ALIVE, WITH NO WAY BACK.** 2026-09-13 18:47:34, the incident that ended the session: a 45-damage hit, then ROCCO's status is **byte-identical for 105 s** (`alive=True hp=45 armor=5 ammo=36 shots=25 src=gun`) while his node log shows no `☠ down` and the frame ring shows `$BUT` press/release pairs still arriving — the trigger being pulled, nothing firing. Two separable faults: (a) **there is no staleness detector on the pool anywhere**, so a gun silent for two minutes looks exactly like a healthy idle one on the HUD and on the MC board; (b) the phone ended with `alive:false, respawnIn:0, respawnGate:"trigger"` under an `auto` config (**F212**), i.e. gated on a pull that the ring proves was arriving. Fix: a pool-staleness watchdog that says so instead of showing a confident stale number, plus an operator mid-match resync (today the only exits are a whole-lobby RE-PUSH or ending the game — Tony ended the game). [`game-test-2026-09-13.md`](game-test-2026-09-13.md) A3. `build`. **→ 2026-09-16:** (a) phone half BUILT (A45). Any-frame silence would not have caught this one: `$BUT` pairs and a `$VOLTS` arrived throughout. The node now calls its pool stale on `no_fire` (3 trigger presses in a row on a live, loaded gun with no `$HP`/`$LCD`/`$ALCD` within 1500 ms of each) or `silent` (no frame for 185 s; the captures put idle `$VOLTS` at ~60 s, not 30 s, with a 120.3 s gap when one goes missing). Engine state `poolStale`, status `pool_stale` + `pool_stale_ms`. **Still open:** `types.py` `pool_stale: NotRequired[Literal["silent","no_fire"]]`, `pool_stale_ms: NotRequired[int]` and the MC board, the HUD rendering, the operator resync (b), and a `$QUERY,*` probe on `no_fire` (its `$LCD` would read the real pool, but a mid-match `$QUERY` is unbenched). **Bench gate:** a charge-rifle hold and a swap to an empty slot must not trip `no_fire`. **→ 2026-09-17:** the operator resync gap in (b) is closed: MC's RESYNC GUN and FORCE RESPAWN (A47) give a per-player recovery without ending the match, and a third `pool_stale` reason, `write_lost`, flags a spawn or revive write that resolved false, with RESYNC GUN as its fix.
...
- **F235 🟠 Needs Tony at the bench** the operator menu on hardware. **FORCE RESPAWN is proven 2026-09-18**: MC sent it, the phone acted, the feed carried both the send and the phone's own `operator_result`, and the player came back at full pools. RESYNC GUN and RELINK GUN are still to run.
- **F236 🟠 Needs Tony at the bench** confirm match resume after an MC restart mid-match, and adopt with no snapshot, against a real running match. `build`.
- **F237 🟠 Needs Tony at the bench** confirm the Pixel 5 BLE flood is gone: no Choreographer skipped frames, taps land instantly, and stations are still detected in a scanner-respawn game with the 25-per-second scan guard in place. `trigger`.
- **F238 ✅ CLOSED 2026-09-18 on the bench.** the energy gauge on the charge rifle reads correctly: the cell as a percentage, the reserve as cell pills, and HOLD TO RECHARGE once the cell cannot pay for a charge. NOT ENOUGH ENERGY correctly stays hidden until the cell is part-full. (The out-of-energy wording it exposed is **F257**.)
- **F239 ✅ CLOSED 2026-09-18 on the bench.** the full-screen OVERHEAT takes over the HUD on the charge rifle, confirmed by Tony.
- **F240 🟠 Needs Tony at the bench** NIGHT OPS dims the LEDs only, and each player's HUD skin is correct. `eyes`.
- **F241 🟠 Needs Tony at the bench** the shot-ready cue. `eyes`.
- **F242 ✅ CLOSED 2026-09-18 on the bench.** the results overlay opens from the player name and from the game time, both tabs, closable, confirmed mid-match. (The name's left margin is fixed in the same session.)
- **F243 ✅ CLOSED 2026-09-18 on the bench.** HARDWARE READY and ENABLE BACKHAUL against a real `cloudflared` tunnel: the button ran STARTING then BACKHAUL ON, the join QR carried both the LAN and the public URL, and both phones joined. (The coverage line it exposed is **F256**.)
- **F244 ✅ CLOSED 2026-09-18 on the bench.** MARK ALL READY marked both players ready from the operator's side.
- **F245 ✅ CLOSED 2026-09-18 on the bench.** Tony on the charge rifle's overheat: "yes it sounded like overheating". `$PLAY,C19` is the right cue after all, and the full-screen OVERHEAT shows with it (F239).
- **F246 🟡 Needs Tony at the bench** an energy-weapon reload timeout was seen at 14:51 on 2026-09-17; capture `$BUT,2` then `$ALCD` to root-cause it. `capture`.
- **F247 🟠 Needs Tony at the bench** A45 `no_fire` must not fire on a swap to an empty slot (the charge-rifle-hold half of the old F218 bench-passed today; this half did not run). `trigger`.
- **F248 ✅ CLOSED 2026-09-17.** The Rail Gun read "50%" on a two-round magazine, because the HUD's energy gauge followed A48's `weapon_class`. The gauge now follows **`rounds_per_charge > 1`** (`usesCellGauge()` in `app/src/hud/hud.js`), which is the question the gauge actually asks: a weapon that spends several rounds per charge gets the cell gauge and the reserve pills, and everything else gets pips or the big-magazine bar. `isEnergyWeapon()` keeps only the wording (RELOAD against RECHARGE, HOLD TO RECHARGE, OUT OF ENERGY). The Energy Rifle's 300-round magazine correctly moves to the big bar with it. `app/tools/screens.mjs` asserts both classes, and a pre-A48 bundle falls back to the named charge-cost constant.
...
- **F5 🟡 decision** the AR's fire rate and reserve are balance choices, not the capture: `weapons.json` ships it at the native **100 ms** with **reserve 192** against the captured 384 (the 2026-09-17 arsenal rework restored the native cycle; the reserve is still half). Keep or restore the captured numbers, and say which. See **F253**: the compiled reserve reaches the gun halved again. `decision`.
...
- **F162 🟠 Needs Tony at the bench** characterise the remaining venue controls after the 2026-09-13
  ...
  **→ 2026-09-17: t41's emitted-power effect is MEASURED and it is nil in outdoor mode (F231/Q15); t2 `gunRangeOutdoor` is the live control.** `$GSET` t3's effect remains unmeasured: `RANGE_ENV_OVERRIDE` stays a no-op
  ...
  is F169; indoor reflections are F198. See `archive/HANDOFF-gset-t2-2026-09-13.md` for the field evidence. `bench`.
...
- **F234 🟡 `RANGE_ENV_OVERRIDE` IS STAGED ON THE WRONG TOKEN.** `mc/compile.py` carries a venue → range map (`gun_range_pct`, `RANGE_ENV_OVERRIDE`) deliberately parked as a no-op until a bench confirmed which token moves emitted range. It is wired to **t41**, which 2026-09-17 proved inert in outdoor mode (Q15/F231); the live token is **t2**.
  ...
  **✅ CLOSED 2026-09-17.** The second item this row carried (the brx MCP `connect()` alias bug) is now **F249**: one row, one item.
...
- **F259 🔴 THE RECOIL WRITER ERASES SHOTS: THE GUN NEVER RUNS DRY.** Bench 2026-09-18, Tony on a live match: "the ammo was out of sync, it was rendering 31/32 back and forth", then "it just settled on 32 but that isn't right". Measured over CDP: the phone writes `$WEAP,0,...` to the gun **every 250 to 500 ms** while a player shoots. A mid-match `$WEAP` RESETS THE MAGAZINE (main's own bench, 2026-09-17), and `_recoilWrite` follows it with `$AMMO` from **`_prevAmmo`, its last KNOWN count**. So: the shot takes the gun to 31, its `$ALCD` is still in flight, the writer sends `$WEAP` + `$AMMO,32`, and the shot is erased. With the shipped `recover_ms` of 150 ms and `per_shot` 10 the writer fires continuously while anyone shoots, so **the magazine never depletes and the HUD's ammo means nothing**. It is S42's writer meeting the gun's own reset, and the `$AMMO` restore that was supposed to cover it restores a stale number. Fix shape: do not restore a count the node cannot know is current (skip the restore while a shot is in flight, or carry the freshest `$ALCD` and never a remembered one), and reconsider a writer that fires on every accuracy step during a fight. Mitigation for a bench or a game today: `config.recoil = false` (done on the bench, and the ammo counted honestly afterwards).
  **The way forward, decided with Tony at the bench 2026-09-18, and it is simpler than what shipped:**
  **(1) One step, not a walk.** Recoil does not need a continuous ladder. Sustained fire drops accuracy ONCE (crisp → degraded) when it crosses a
  threshold, and it recovers once the trigger goes quiet. That is two `$WEAP` writes per burst instead of about twenty, and it keeps the feel that
  matters: hold the trigger and your shots wander. Recoil must still bite DURING full auto, so 'only write when quiet' is not an option.
  **(2) Restore from the node's own shot account, never a remembered `$ALCD`.** The node sees the trigger press (`$BUT,0,1`) the instant it happens,
  before the gun fires, and it already models which presses produce no shot (empty, overheat, swap, stun). So the restore is `mag - fired`, and a
  round fired inside the write window is already counted. Reconcile the account against `$ALCD` whenever no write is in flight, and let `$ALCD` win:
  a drifting account must lose to the gun, never fight it. `build` + `bench`.
- **F258 🔴 THE GUN PICKER LISTS EVERY BLUETOOTH DEVICE IN THE ROOM.** Bench 2026-09-18, Tony: "pixel5 is locked up on bt selection again". It was not locked: the picker says **SCANNING FOR TAGGERS** and then lists, in signal order, two televisions, a QLED, a Hatch Rest, a WL_SCU, bare MAC addresses, and the two actual taggers at positions **7 and 12**. A player at a muster cannot find their own gun in that, and the screen reads as hung because it churns. ⚠ The raw scan must STAY unfiltered: `brxlink.scan()` deliberately sends no service filter because Android misses taggers whose UUID rides in the scan response (bench-proven, the comment is on the line). So the fix belongs in the PICKER, not the scan: rank a hit that advertises the Nordic UART service first, then a name that matches an enrolled gun, and put everything else behind an "other devices" fold. Better still, the phone already knows its player's `gun_id` from MC, so the assigned gun should be offered by name at the top before any list at all. `build` + `eyes`. ⚠ **It is worse than a noisy list: touch does not work at all.** Measured over CDP while Tony could not scroll: the JavaScript thread is alive and answers instantly and the DOM is not blocked, but the list is REBUILT AND RE-SORTED on every scan hit (`allowDuplicates: true`, about two dozen devices in the room). Four samples a second apart gave 12 rows, 12 rows, **2 rows**, then 5 in a different order. The row under a finger is replaced between touchstart and touchend, so no tap or scroll lands, and a synthetic click on the right row did not take either. Tony recovered it by swiping the app away and reopening it from recent apps, which tears the scan down. So the fix is three things: coalesce the re-render (2 Hz is plenty), sort on a STABLE key (`deviceId`, never live RSSI), and patch rows instead of rebuilding the list. Tony adds that the freeze continued through the pick itself ("it did lock up for a bit when I clicked the robas, was like the app was frozen") and cleared once the gun was connected, so the scan is still running across the connect and contending with it: stop the scan before the connect starts, not after it finishes.
...
- **F198 🔴 Needs Tony at the bench** the reflection theory behind `$GSET` t2 is untested and may INVERT the
  ...
  everywhere. (filed 2026-09-13 from archive/HANDOFF-gset-t2-2026-09-13.md §3) `bench`.
