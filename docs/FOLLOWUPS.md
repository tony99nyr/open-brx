...
**Ids.** One capital letter + number. Never renumbered, never reused. **Next free: B32 · D6 · E8 · F249 · G11 · H9 ·
K9 · P19 · Q20 · R4 · S52.** (2026-09-17 merge of the brx-weapons arsenal rework: this branch's thirteen
verification rows moved from F230-F242 to **F235-F247**, because main had already taken F230-F234. **F248** taken for the merge's one visible consequence.)
...
- **F235 🟠 Needs Tony at the bench** the operator menu (RESYNC GUN, FORCE RESPAWN, RELINK GUN, A47) on hardware: confirm each action against a real gun, including the two refusal cases (a wrong match or player, a stunned or reconciling phone). `trigger`.
...
- **F238 🟠 Needs Tony at the bench** the energy gauge (cell pills, NOT ENOUGH ENERGY, HOLD TO RECHARGE) on the charge rifle. `trigger`.
- **F239 🟠 Needs Tony at the bench** the OVERHEAT display on the charge rifle, and on the energy rifle once its t38 ships. `trigger`.
...
- **F242 🟠 Needs Tony at the bench** the results overlay. `eyes`.
- **F243 🟠 Needs Tony at the bench** HARDWARE READY with the ENABLE BACKHAUL button, against a real `cloudflared` tunnel. `build`.
- **F244 🟠 Needs Tony at the bench** MARK ALL READY for the operator. `build`.
- **F245 🟡 Needs Tony at the bench** the charge rifle has no overheat sound: `$PLAY,C19` sounds like the charge-cancel cue, not an overheat cue. `ears`.
...
- **F248 🟢** the Rail Gun's ammo gauge now reads as a percentage, not two pips. A48 gave the catalogue a formal `weapon_class`, and the HUD's energy gauge follows it instead of the old "energy or charge in the id" guess. The class is wider than the guess: `plasma_sniper`, `rail_gun`, `ion_sniper` and `laser_cannon` are energy too. Three of the four are hidden and the Rail Gun is pickup-only, so nobody sees it in a normal kit, but a 2-round weapon reading "50%" is worse than two pips. Decide whether the gauge should follow the class or the magazine size. `eyes`.
