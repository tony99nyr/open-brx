...
- **F134 🟡 `scoring.win_by` IS AN OPEN VOCABULARY.** `state.py _merge_config` stores any string; every reader (`scoring.py` ~485/504/741, `compile.py` ~471) branches on `not in (None, "", "kills")`, so a typo such as `"kils"` silently disables the frag cap. `station_source` got a closed vocabulary for exactly this shape (`STATION_SOURCES`); do the same: a `WinBy` Literal on `types.Scoring`, refused at PUT and by `validate()`, generated into the contract. Found by the pyright-pass delta review 2026-09-12.
...
- **F42 🟡** **the DRY-review backlog** (2026-09-07 Python review, agent team). Six bugs from that pass are FIXED
  ...
  **F42.9** (filed 2026-09-12, contract-DRY phase 1) the ~29 UI-only view types (`State`, `LiveView`,
  ...
  cleanup's job, F42.10). **F42.10 ✅ CLOSED 2026-09-12:** pyright gates `mcp/` in CI (`standard` mode,
  `mcp/pyproject.toml`; first run 2026-09-12 found 249 errors, 81% of defs already annotated). **F42.11** `webapp/mc`'s
  `tsconfig` is not `strict: true` yet. **F42.12** `app/src/transport` (`envelope.js`/`transport.js`) has no
  JSDoc + `checkJs` reading a generated `.d.ts`. **F42.14** `brx_mcp/stage/stage.py` is excluded from the
  pyright gate (56 errors on 2026-09-12, the hot file above) — it's the raw-JSON bench stage that must keep
  mirroring the phone engine, so it gets typed as its own job rather than folded into F42.10's cleanup. None
  of F42.9, F42.11, F42.12, F42.14 block anything; contract-DRY phase 1 (types.py/envelope.py → generated
...
- **F135 🟡 Needs Tony at the bench** B4 (2026-09-12 field session: all four guns ended `bleUp:false`)
...
- **20-minute two-node soak** (Pixel + iPhone): screen-lock one at T+5, background the other at T+10, walk out of Wi-Fi;
  BLE held, engine reconciled on unlock, outbox flushed on return, timed end fired locally on both. **F135** wants the
