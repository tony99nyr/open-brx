...
**Ids.** One capital letter + number. Never renumbered, never reused. **Next free: B32 · D6 · E8 · F217 · G11 · H9 ·
K9 · P19 · Q20 · R4 · S42.** (2026-09-16 write-up of the 2026-09-13 evening playtest: **F206-F216** taken, all from
...
- **F134 🟡 `scoring.win_by` IS AN OPEN VOCABULARY.** `state.py _merge_config` stores any string; every reader (`scoring.py` near 494/513/758, `compile.py` near 585) branches on `not in (None, "", "kills")`, so a typo such as `"kils"` silently disables the frag cap. `station_source` got a closed vocabulary for exactly this shape (`STATION_SOURCES`); do the same: a `WinBy` Literal on `types.Scoring`, refused at PUT and by `validate()`, generated into the contract. The server uses exactly three values, `"kills"`, `"survival"`, `"objective"` (measured 2026-09-13). Found by the pyright-pass delta review 2026-09-12; ordered and sized in [`HANDOFF-dry-2026-09-13.md`](HANDOFF-dry-2026-09-13.md).
