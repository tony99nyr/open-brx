...
- **F134 🟡** HUD gun picker does not refresh live: a gun powered on while the list is open updates the RSSI bars but never appears until "Set my gun" is tapped again (field 2026-09-12). `build`.
...
- **F159 🟢** HUD loadout detail pane: the name + role + EQUIPPED line overflows sideways for "Assault Rifle" / "Assault" (`.nm` 323 > 320 px), pre-existing, found by screens.mjs during the field-fix lane 2026-09-12. `build`.
