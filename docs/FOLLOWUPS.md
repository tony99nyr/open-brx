...
- **F42 🟡** **the DRY-review backlog** (2026-09-07 Python review, agent team). Six bugs from that pass are FIXED
  ...
  **F42.9** (M, then L for `State`) batches 1-4 are done: generated leaf,
  arsenal, saved-game, live-row, node, station, tunnel, presentation, mode, recap and history shapes replace
  their console copies, and their producers are checked. Only the `State.snapshot()` decomposition remains,
  ordered in the handoff. **F42.12** (M) `app/src/transport`
