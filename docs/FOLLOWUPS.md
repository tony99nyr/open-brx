...
| B10 | **Synchronized multi-gun start** | 🔴 NEW | Found in the G-2 two-gun test (exp-log #33): configuring guns **sequentially** makes their starts **~10 s apart**. Fix: **config all guns first, then broadcast `$SPAWN` to all together** (or config in parallel). The engine/driver (M0.2, `tier0-plan.md`) must spawn as a synchronized barrier, not per-gun. `arena` may already partly do this — verify. |
...
| P11 | **Shield-regen a settable weapon field?** | 🟡 (host path ✅) | **Host-driven health-write is CONFIRMED** (exp-log #33, G-2): `$LIFE`/`$BUMP` change a live tagger's HP/armor/shield (armor restored 16→70, shield 0→16). So shields/overshield/medic/Syphon are buildable via host writes **today**. Remaining question is only the *native* path: is a **regen delay/rate** a settable `$WEAP`/`$PSET` field (→ zero-host-logic Halo shields)? Diff a `$WEAP` with regen on/off. **Also nail:** `$LIFE` (add delta) vs `$BUMP` (absolute set) semantics + note that **writes don't self-emit `$HP`** (value shows on next hit/refresh). |
...
| G6 | What objective state does the gun expose over BLE? | 🔴 | For the state display (B8): during a grenade CTF/KotH/Assault game, what `$`-messages does the gun emit (flag held, point owner, timer)? Capture a grenade game and watch the BLE stream. **Also gates the CS-with-grenade "auto-enable Plant in range" feature** (`phone-app-spec.md`): if the grenade's IR beacon/hit surfaces in the gun's BLE stream, a player's node can auto-detect it's at the bomb site. Also confirm the **dead-gun-can't-fire** gate holds through a full death→respawn cycle (the hardware eligibility gate for shoot-to-arm). |
