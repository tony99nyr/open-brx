...
**Ids.** One capital letter + number. Never renumbered, never reused. **Next free: B32 · D6 · E8 · F243 · G11 · H9 ·
K9 · P19 · Q20 · R4 · S48.** (2026-09-17 verification bench: F215, F217 and F218 closed; F230-F242 taken for the
verification bench and one folded-in residue row; the brx-weapons session on main has taken F225-F229 and
S42-S47, which are not this branch's to spend, so new ids continue at F230 and S48.) (2026-09-17 bench pass on
...
for Tony. 2026-09-16 write-up of the 2026-09-13 evening playtest: **F206-F216** taken, all from
...
- **F230 🟠 Needs Tony at the bench** the operator menu (RESYNC GUN, FORCE RESPAWN, RELINK GUN, A47) on hardware: confirm each action against a real gun, including the two refusal cases (a wrong match or player, a stunned or reconciling phone). `trigger`.
- **F231 🟠 Needs Tony at the bench** confirm match resume after an MC restart mid-match, and adopt with no snapshot, against a real running match. `build`.
- **F232 🟠 Needs Tony at the bench** confirm the Pixel 5 BLE flood is gone: no Choreographer skipped frames, taps land instantly, and stations are still detected in a scanner-respawn game with the 25-per-second scan guard in place. `trigger`.
- **F233 🟠 Needs Tony at the bench** the energy gauge (cell pills, NOT ENOUGH ENERGY, HOLD TO RECHARGE) on the charge rifle. `trigger`.
- **F234 🟠 Needs Tony at the bench** the OVERHEAT display on the charge rifle, and on the energy rifle once its t38 ships. `trigger`.
- **F235 🟠 Needs Tony at the bench** NIGHT OPS dims the LEDs only, and each player's HUD skin is correct. `eyes`.
- **F236 🟠 Needs Tony at the bench** the shot-ready cue. `eyes`.
- **F237 🟠 Needs Tony at the bench** the results overlay. `eyes`.
- **F238 🟠 Needs Tony at the bench** HARDWARE READY with the ENABLE BACKHAUL button, against a real `cloudflared` tunnel. `build`.
- **F239 🟠 Needs Tony at the bench** MARK ALL READY for the operator. `build`.
- **F240 🟡 Needs Tony at the bench** the charge rifle has no overheat sound: `$PLAY,C19` sounds like the charge-cancel cue, not an overheat cue. `ears`.
- **F241 🟡 Needs Tony at the bench** an energy-weapon reload timeout was seen at 14:51 on 2026-09-17; capture `$BUT,2` then `$ALCD` to root-cause it. `capture`.
- **F242 🟠 Needs Tony at the bench** A45 `no_fire` must not fire on a swap to an empty slot (the charge-rifle-hold half of the old F218 bench-passed today; this half did not run). `trigger`.
