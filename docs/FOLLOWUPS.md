...
- 🔴 **B26** · **F198** · **F231** · **F232** · **F257** · **F261** · **F264** · **F275** · **K4** · **Q15** · **S10**
- 🟠 **B31** · **F50** · **F59** · **F152** · **F158** · **F162** · **F171** · **F183** · **F219** · **F226** · **F237** · **F269** · **F271** · **F272** · **F274** · **F277** · **G9** · **P8** · **Q16** · **S9** · **S33**
- 🟡 **B28** · **B29** · **B30** · **D1** · **D4** · **F3** · **F13** · **F21** · **F26** · **F27** · **F28** · **F30** · **F39** · **F63** · **F66** · **F67** · **F68** · **F75** · **F76** · **F82** · **F88** · **F128** · **F131** · **F167** · **F168** · **F169** · **F195** · **F214** · **F216** · **F227** · **F229** · **F233** · **F262** · **F267** · **F268** · **F270** · **F282** · **F285** · **G3** · **G10** · **H7** · **H8** · **K1** · **P3** · **P15** · **S1** · **S2** · **S7** · **S-A12** · **S36** · **S49** · **U11′**
...
- **F283 🟠 `soak --phone-pacing`: THE SOAK MUST PACE LIKE THE PHONE.** Filed 2026-09-18 night. The screamers sheet (Phase C) named this tool change, but no row held it. The MCP instrument (`ble.py`) sleeps 20 ms after every 20-byte chunk and adds no frame gap. That is about 0.84 B/ms, against the phone's peak of about 1.5 B/ms, so a soak under-stresses long frames by about 1.8 times. Do: add a `--phone-pacing` mode to `mcp/brx_mcp/soak/runner.py` that copies `WRITE_PACING` in `app/src/brxlink.js` (8 ms per chunk, 18 ms per frame, and the block pause once F269 turns it on). Test it with the fake clock that the runner already takes. A screamers Phase C run counts only with this flag. `build`.
...
- **F279 🟠 ONE COMMAND WORD SPANS OUR SAFEST READ AND ONE OF OUR MOST DESTRUCTIVE WRITES.** Filed 2026-09-18 at the request of the lane that had just guarded it, because the guard's reach is narrower than the ambiguity. `$LIFE` with all-zero pool tokens moves nothing and returns `$HP` from a live gun AND a dead one (bench 2026-09-18), which makes it the cheapest, safest question we can ask a tagger. `$LIFE,<hp>,0,0,1,*` REVIVES a dead gun. **The two are separated by one token**, and the word itself cannot say which you meant. ⚠ **It cannot be fixed at the command level and never will be**: we do not modify firmware, the command set is fixed, and `$LIFE` is the ONLY command that returns the pools and changes nothing (`$PING` has no pools, `$VERSION` none, `$QUERY` holds a dead gun's print loop for 2 s, which is why it came off the timer). The distinction IS real and permanent, because an all-zero add moves nothing by arithmetic rather than by convention; it is simply not in the word. **What exists today:** `isPoolProbe()` in `app/src/engine.js` and `protocol.is_pool_probe` as its Python twin, both semantic rather than an equality check, plus a source scan in `engine.test.mjs` that fails on any new `$LIFE` string in `app/src` or `app/test` that does not go through the predicate. **What that does NOT reach, which is why this row exists:** every Python reader outside `protocol.py`, `mc.diag`, a frame-ring scan, MC, and any log analysis written next year. The first symptom was five shield tests going red because a regen grant and a probe are the same word; the next reader will be counting `$LIFE` frames in a session file with no tests at all. Do: use the predicate everywhere a `$LIFE` is counted, matched or classified, and extend the scan beyond `app/` if a second instance appears. `build`. ⚠ **An unfalsifiable branch was found INSIDE the predicate while writing its guard** (a clause the prefix match had already made unreachable), which is the same defect the guard exists to catch, one layer down.
- **F278 🟠 A RUNTIME PERK CAN WALK STRAIGHT THROUGH A CATALOGUE GUARD.** Filed 2026-09-18 from brx-weapons, before anyone builds the perk that would trip it. The arsenal owner shipped a rule this evening that a weapon carrying a second IR word may not also carry a crit chance: the crit flag rides on BOTH words while the multiplier reaches only the barrel word, so the combination is a damage split nothing models, and it has never been measured. **That guard reads `weapons.json` at TEST time.** The bench has since found `$TMP` t10 = crit-chance points, so a crit perk would write crit chance at RUNTIME, never touching the catalogue. A player taking that perk while holding the Shotgun, the Plasma Sniper or the Rocket Launcher creates exactly the combination the guard exists to refuse, **and the guard stays green while it happens**. ⚠ **The general shape is the finding, not the instance**: every compile-time guard we have assumes the catalogue is the whole truth, and a live modifier system makes that false for any property a perk can write. `$TMP` can now write accuracy (t4), pool maxima (t1-t3), magazine (t9), incoming damage (t8), fire interval (t5), word damage (t7), reload time (t6) and crit chance (t10), so every catalogue rule touching any of those has the same hole. Do: audit the compile-time guards against that token list, and for the crit case add a RUNTIME refusal beside the compile-time one (a crit perk is ineligible on a weapon declaring `wire.headset_dmg`), because it is one condition and it fails safe. Measuring the two-word crit split is a separate question and must not block the refusal. `build` + `decision`.
...
- **F280 🟠 THE RECOIL RUNGS DO NOT SCALE WITH THE MAGAZINE, SO A DEEP-POOL WEAPON IS AT HEAVY FOR ITS WHOLE LIFE.**
  Filed 2026-09-18, and it is a fault in the numbers I supplied rather than in the writer. The table sets
  `after_shots` 3 and `after_heavy` 6 on every weapon that degrades, chosen from the assault rifle's 32-round
  magazine where 6 of 32 is a fifth of the pool. Applied unchanged to the **Energy Rifle, whose magazine is 300**,
  it puts rounds 7 to 300 at `heavy`: **98% of the magazine spent at the floor**, so the two-step ladder is not a
  ladder at all, it is "degraded briefly, then heavy forever". Measured across the ladder (playtest lane): heavy owns **84% of the AR's magazine, 93% of the SMG's and 98% of
  the Energy Rifle's**. ⚠️ **And the rungs are wrong underneath the numbers, so re-picking them does not fix it.**
  `after_shots` derives from the ladder's DEPTH, `ceil((crisp - heavy) / per_shot)`, which has nothing to do with
  how fast a held trigger should stop rewarding a player. That is why the Stinger is the only weapon with a real
  two-stage feel, and it got there by accident. Whatever numbers are chosen, the derivation needs replacing. A rung count that is an absolute number of rounds cannot mean the same thing to a 32-round
  weapon and a 300-round one. ⚠️ **DO NOT RE-DERIVE THE OPTIONS HERE: F268 carries the analysis and it supersedes this row's list.** A
  fraction of the magazine was the obvious answer and it is WRONG, because it inverts the incentive at both
  ends: the Energy Rifle would need 4.2 s of held fire before degrading at all, and the Stinger would floor by
  its third round. Magazine depth is not what recoil is about. The live proposal, with Tony, anchors the rungs
  on TIME-TO-KILL instead: every weapon kills in 12 to 15 hits over 1.2 to 2.0 s, so one kill's worth of held
  fire is the natural point to start punishing, which lets a player win a fight at full accuracy and penalises
  only over-holding. ⚠️ This is a NUMBERS row, not a mechanism row: it
  needs no bench, only a decision, and it should be settled in the same pass as F268's two judgements. Cross-ref
  F268, S54. `decision`.
...
- **F284 🟡 THE PHONE'S OFFLINE GIVE-UP POLICY (180 s): FIND IT, THEN DECIDE IT.** Filed 2026-09-18 night. `bench-plan.md` has listed "the 180 s offline give-up policy for a phone" as a decision for Tony since 2026-09-18, with no row and no source for the number. Do: find where 180 s comes from in the node or MC code, write down what the phone does when it gives up, and ask Tony to accept or change it. Do this before the match sitting's offline checks. `decision`.
- **F285 🟡 ONE TABLE FOR HOW EACH `$TMP` TOKEN TAKES A WRITE.** Filed 2026-09-18 night. The `$TMP` row of `protocol/brx-protocol.md` gives the write behaviour in prose: t4 and t8 ABSOLUTE, t9 ADDITIVE and one-shot. Every per-life owner (S55, the F121 spawn-protection rebuild, S50's perks) must know this before it re-sends. Do: turn it into one column per token (absolute, additive, or unmeasured). ⚠️ The same row also calls t1 and t2 absolute, but no experiment-log entry measures them, and levers §21 steps 12-14 (t1-t3) are unrun: mark t1, t2, t3, t5, t6, t7, t10 and t11 unmeasured until a bench says otherwise. t6 is the gap that matters most (F281): send t6 = 50 twice and time one reload. `build` + `bench`.
...
- **F121 🟡 HIT RECEPTION IS ARMED AT CONFIG PUSH AND NEVER DISARMED** — so you take damage during the countdown and before your gun arms at spawn. `compile.py:934` ships `sir_rows` in the SAME head as the pregame team colour, and per F11 the `$SIR` table IS the arming. No spawn-protection concept exists anywhere. Fix mechanism in-tree: `_SIR_NO_POOL` (`compile.py:135`) pregame, real table at `$SPAWN`; `$SIR` swaps are proven safe (`_revive` re-sends every life). [`archive/game-test-2026-09-11.md`](archive/game-test-2026-09-11.md) C1. `decision` + `build`. **→ 2026-09-12:** BUILT (A23): the head ships every `$SIR` cell on fn 28, the live table rides `spawn`/`revive` (or the `sir_pool` take), three compile guards (`assert_spawn_protected`, `assert_arms_at_spawn`, `assert_rearms_every_life`); `test_spawn_protection.py`. **Bench gate:** push, take a hit during the countdown — no headset reaction, no pool movement; after `$SPAWN` the same shot lands. **→ 2026-09-13 field, FIRST look, and it does not hold:** hits landed +1.6 s, +2.9 s and +3.0 s after a respawn, and the weapon comes online ~3 s after `respawned` — hit reception returns before the trigger does. See **F209**. **→ 2026-09-18:** the respawn half is confirmed on hardware (F217, closed: no damage in the first ~2.4 s). Left: the countdown half of the bench gate, and the `$STOP`-until-spawn alternative in `bench-firmware-levers-2026-09-19.md` §12 then §23. **→ 2026-09-18, §23 confirmed the lighter design.** The `$SIR` table survives `$SPAWN` (a fn-1 row sent once at arm still registered hits after a `$SPAWN,,*` with no `$SIR` re-send), and `$TMP` t8 = -100 sent right after `$SPAWN` protects the life with no fn-28 twin table at all: three hits registered as `$HIR` with zero pool movement. So a life's per-spawn protection cost can drop from the 28-frame fn-28 twin table to `$SPAWN,,*` plus one `$TMP` write, and `$SPAWN,,*` itself zeroes any earlier t8, so the protection write must land AFTER `$SPAWN`, never before. **→ 2026-09-18 (third sitting), §23 steps 1-5 run clean and add two rules.** `$STOP` blocks a hit's damage but not the hit itself (`$HIR` still arrives, no `$HP`); refutes the sheet's "no `$HIR`" prediction on that point. `$STOP` also SURVIVES `$SPAWN`: a run that left `$STOP` in effect stayed blocked through a `$SPAWN` and a t8 write, and only `$START,*` plus a `$GSET`/`$TID` re-send restored damage. **Design rule: anything that sends `$STOP` must send `$START` before the next life.** The clean design (`$SPAWN,,*`, `$TMP,,,,,,,,-100,,,,*`, `$TID`) is otherwise fully confirmed. **→ 2026-09-18 night: no bench is left.** The rebuild is desk work: ship `$SPAWN,,*`, `$TMP` t8 = -100 and `$TID` in place of the 28-frame fn-28 twin table, remove `sir_spawn_protected()` and the per-life `sir_pool` take, and design the t8 writer with S55's owner, because S50's damage-resist perk also wants t8. `build`.
- **F289 🟡 A PHONE THAT DIES INSIDE THE SPAWN-PROTECTION WINDOW MAY STAY PROTECTED ALL MATCH.** Filed 2026-09-18 (polish review #2, F121 gate work). F121's fix writes `$TMP` t8 = -100 right after `$SPAWN`, then the phone writes t8 = 0 (`spawn_protect_off`) on the first shot or at `SPAWN_PROTECT_MAX_MS` (2100 ms, `engine.js`), whichever comes first. The off-write is the PHONE's job, not the gun's own timer. If the phone loses power, crashes, or drops its BLE link inside that 2.1 s window, the off-write never goes out, and the gun keeps t8 = -100 for the rest of the match: that player takes no damage from anyone, and nothing on the MC board says so. Out of scope for this review's fix (`_bind`/`_hydrate`/`start()` app-version gate); this is a DIFFERENT hole in the same mechanism. Do: choose between a board flag that marks an OFFLINE player as POSSIBLY PROTECTED while they were mid-life when they went offline, and a gun-side timer (independent of the phone) that clears t8 on its own after a fixed delay. Until then, an operator should treat any OFFLINE player as possibly still protected. `decision`.
...
- **S16 🟠 Damage over time, on the node** (unblocked by the 2026-09-09 bench; raised to 🟠 2026-09-17, Tony: "we don't have any damage over time weapons, like a poison gun"). `$LIFE` takes negatives, so
  ...
  **The spec section is written**: `spec/node.md` §3.17 (recognise, tick, refresh never stack,
  end on death, the unattributed lethal tick, and the HUD's duty). ✅ **ALL THREE DECISIONS ANSWERED by Tony 2026-09-18, so only the code is left.**
  **(1) Kill credit goes to the player who applied the poison** ("yes the hit"). The victim's node emits the
  fact itself: it read the `$HIR`, it owns the tick clock, and it is the only thing that knows who applied it.
  `$BHIT` (levers §3) stays a candidate if a wire-side credit proves preferable.
  **(2) Poison does NOT survive a respawn** ("no") -- death clears it. Respawning still poisoned is the
  spawn-walk misery twice over.
...
mode; **P14**; **S48** what "super indoor" means; **F284** the 180 s give-up policy; **F268** and **F280** the recoil rung basis.
