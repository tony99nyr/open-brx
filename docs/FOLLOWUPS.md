...
**Ids.** One capital letter + number. Never renumbered, never reused. **CLAIM AN ID BY WRITING ITS ROW FIRST, before doing the work** -- a stub row and the bumped "next free" below, committed, then the investigation. Ids collided FOUR times on 2026-09-18 alone and every one was two sessions reading "next free" at the same moment and both working for an hour before either wrote anything down. The loser renumbers, which is tedious and loses cross-references. This is a process fault, not anyone's mistake. **Next free: B32 · D6 · E8 · F314 · G11 · H9 ·
...
- **F313 🟡 THE DEATH SCREEN'S FIELD CHECK AND ITS LOWS.** Built 2026-09-23 (S56). Field: in one real match, read a few DOWN screens against the victims' own TOOK lines and MC's board. Lows from the three polish rounds: a NaN `$HIR` sensor reads ON YOUR HEADSET; a num-0 final hit has no age check; a gun-restart death can still tag a KILLER row; YOUR MATCH's age label also covers the phone's own deaths; FFA ties rank by order; a two-word shot names only its first sensor; the dead `.down .lf` rule; the `:has` offset under the level 2/3 warning needs Chrome 105 or Safari 15.4 (use a `data-warn` attribute instead); the old `down-recap` stage has no step. Per-life accuracy is left out on purpose: the gun counts rounds, not trigger pulls, so it needs a pull count first. `bench` + `build`.
...
- **F310 🟡 BALANCE RULES ON THE SHIELDS PRESET.** Tony 2026-09-23: the balance rules should hold on Shields (45 HP + 105 shield = 150) too, "looser but generally yes". Add a health-preset parameter to `balance_sim.py`'s duel scenario, run every rule in the Balance rules table on Standard, Shields and Hardcore, and guard Shields in CI at a looser bar than Standard's 65% (propose the bar with the numbers). Report Hardcore only: one-shot kills are that preset's point. Watch the Charge Rifle (charge + 5 taps on 150) and the Shotgun (4 pulls). `build` + `decision`. **→ 2026-09-23, built:** `balance_sim.py --health-preset standard|shields|hardcore` on the recoil, range and R10 duels; every rule holds on Shields at a proposed 60% (lowest cell R8 63.0%), gated in `test_balance_sim.py`; Hardcore reported only. Waiting on Tony: the Shields bar, and whether heavies (115 a pull) should need two pulls on Shields' 150. Numbers in `weapon-design.md` §7.5f. **→ Tony 2026-09-23, heavies decided:** the Rocket Launcher's headset word adds 35 (a close hit kills on Shields; the word's reach is unmeasured, F275), the Rail Gun does 149 (a kill on Standard and Hardcore, 1 HP left on Shields); bench check `bench-2026-09-24.md` 4.9. The Shields bar (60% proposed) is still Tony's.
