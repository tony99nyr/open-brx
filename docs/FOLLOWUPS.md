...
K9 · P19 · Q20 · R4 · S53.** (2026-09-17 night: **S52** taken, the missing Easy Reload hint on the HUD; the fix/playtest-2026-09-13 branch renumbered its own rows and has
...
**🔴 blocking, at a glance (2026-09-16):** **F206** team modes register NOTHING (116 shots, 0 hits) ·
...
- **F253 🔴 THE HOST-DRIVEN STUN (A20) PUTS fn 24 ON THE EMP CELL, SO IT CARRIES THE PHANTOM-HIT BUG.** The 2026-09-18 bench proved fn 24 applies no damage and leaves the victim's gun manufacturing a fake `$HIR` every 5.07 s until the next `$SPAWN`, with sound, vibration and a headset flash (P18, closed). We fixed the Energy Launcher's row the same day, but `A20`'s stun deliberately moves fn 24 onto the EMP cell (`test_spawn_protection.py::test_the_delayed_blast_family_never_reaches_a_pregame_table` pins both configurations). So **any game with `config.stun` enabled gives every stunned player a phantom hit every five seconds for the rest of that life**, which reads as being shot by nobody. Nobody has reported it because stun has not been used in a real match. Do: pick a different function for the stun cell and bench it (fn 28 registers silently and is the obvious candidate, but the stun needs SOMETHING audible, so this is a design choice, not a swap), or accept the phantom and have the node suppress it, which it cannot do reliably because the phantom is indistinguishable from a real hit on the wire. Until then, do not enable stun in a real game. `trigger` + `build`.
