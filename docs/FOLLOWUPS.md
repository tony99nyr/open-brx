...
- **Q15 🔴 → THE RANGE LEVER IS REAL, BUT IT IS `$WEAP` t2 `gunRangeOutdoor`, NOT t41 (garden, 2026-09-17).**
  ...
  **The design intent below stands and is unaffected by the token correction.** Range is per VENUE, not one number
  ...
  it scales with the venue like play volume does, and the compiler's `RANGE_ENV_OVERRIDE` hook lives there — **but it
  is currently wired to t41 and must move to t2** (F234). Target reaches, ratios first, Tony's shotgun number set at
...
- **F234 🟡 `RANGE_ENV_OVERRIDE` IS STAGED ON THE WRONG TOKEN.** `mc/compile.py` carries a venue → range map (`gun_range_pct`, `RANGE_ENV_OVERRIDE`) deliberately parked as a no-op until a bench confirmed which token moves emitted range. It is wired to **t41**, which 2026-09-17 proved inert in outdoor mode (Q15/F231); the live token is **t2**. Also here: the brx MCP `connect()` refuses to re-open an alias whose link has dropped ("alias already connected", then "Service Discovery has not been performed yet" on the next write) — `disconnect` then `connect` is the workaround and it cost several minutes across the session; make `connect` reconnect a dead alias in place. `build`.
