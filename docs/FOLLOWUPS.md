...
7 pink · 8 orange**; 9/10 dark), and **`t4=5` blanks** (Callsign's `$GLED,,,,5,,,*`). Solved
2026-08-30; palette completed and the blank value corrected 2026-09-02 (`t4=3` does **not** blank).
...
| P17 | **How to turn the LEDs OFF (night mode)** | ✅ **CLOSED 2026-08-30 · value corrected 2026-09-02** | **Token 4 = 5 turns the LEDs off**, and only 5. The frame is Callsign's own death frame `$GLED,,,,5,,,*`, which is what `gameconfig._led_frames` already ships — **the product was never affected, only the documented value was wrong.** ⚠️ **RETRACTED 2026-09-02: `$GLED,,,,3,,,*` does NOT blank.** A full t4 sweep 0-10 with the colour held at green renders **solid green for every value except 5**, and an A/B from a lit state left a red gun red through t4=3 ([91,29,36] → [94,35,50]) while t4=5 dropped to ambient. Nuance kept: with **empty** colour tokens, 5, 6 and 7 all blanked; with **explicit** colours, only 5 did and 6/7 stayed lit. That difference is not explained, so the reliable off frame is the Callsign one and nothing else. ⚠️ The older shipped frame `$GLED,0,4,0,0,0,,*` was built on the retracted "index 0 = off" reading — index 0 is **red**, so it never turned anything off. Fixed 2026-08-31. |
