...
- **F124 🔴 THE FRAG LIMIT IS NEVER ENFORCED.** `next_kill_wins` announces at cap-1 and nothing ends the match: the end lives in `modes/deathmatch.py:110` and was never ported to `mc/scoring.py` (its only use of `frag_limit` is the announcement); not enforced node-side (`engine.js:2379` is display only) nor gun-side (`gameconfig.py:286` is never read). Field 2026-09-11: ROCCO hit 9 against a limit of 7, play continued. Intent documented at `compile.py:1434`. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) B1. `build`.
- **F125 🔴 END REPORTS "REACHED 2 OF 2" AND THE MATCH KEEPS RUNNING.** The node log proves it ended by `time-expiry` ~18 s later, not by the control. `state.py:2003-2021`: the push loop sets `reached` BEFORE the branch, and `if cmd == "end" and self.scorer:` falls through to the recall/panic `else` which never sets `phase`. Third time this count has misled (2026-09-01, F31). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) B2. `build`.
- **F121 🔴 HIT RECEPTION IS ARMED AT CONFIG PUSH AND NEVER DISARMED** — so you take damage during the countdown and before your gun arms at spawn. `compile.py:934` ships `sir_rows` in the SAME head as the pregame team colour, and per F11 the `$SIR` table IS the arming. No spawn-protection concept exists anywhere. Fix mechanism in-tree: `_SIR_NO_POOL` (`compile.py:135`) pregame, real table at `$SPAWN`; `$SIR` swaps are proven safe (`_revive` re-sends every life). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) C1. `decision` + `build`.
- **F127 🔴 MC's CONTINUE TO LOBBY LOCKS OUT EVERYONE STILL KITTING.** `state.py:540-548` documents "kit to lobby advances only when EVERY rostered player is ready", and `Kit.tsx:193` bypasses it with a bare `api.setPhase('lobby')` (the guarded route, `POST /api/lobby/push`, refuses on reds). `greens`/`roster_size` are already computed at `state.py:1627`. Wants a ready count on the button plus a warning. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) B6. `build`.
- **F123 🟠 easy_reload FAILS ON THE SHOTGUN, AND THE HUD RELOAD ANIMATION IS A BLIND TIMER.** Three faults: the shotgun's per-shell chain needs a HELD handle; `engine.js:1850-1852` discards every `$BUT` release although the gun streams `state 0` (hardware-confirmed in the frame ring); and `_reloadPulled` (`engine.js:1971`) writes NOTHING to the gun, so any failed reload animates as success on every weapon. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) C2. `build`.
- **F116 🟠 MEDALS NEVER REACH THE SCORE ROW, AND THE ROW SHOWS THE WRONG STREAK.** `honors()` returns empty below 3 players (so a 1v1 is always empty); the Halo per-kill medals are fired as alerts and discarded; `ScoreRow.streak` is the CURRENT streak (0 for whoever died last) while `best_streak`/`multi_best`/`first_blood` are tracked and never exposed. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) B3. `build`.
- **F122 🟠 THE DEBUG PANEL RE-RENDERS EVERY ~1 s: SCROLL RESETS AND TAPS ARE EATEN**, so SHARE LOG is nearly unpressable — nine uploads from the Android, ZERO from the iPhone. `hud.js:731` replaces `innerHTML` on every state change, so a tap spanning a re-render fires no click at all. Cost evidence on 2026-09-11. Fix: preserve `scrollTop` plus a sticky button row. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A3. `build`.
- **F119 🟡 MC's ACC SWINGS WILDLY** — hits arrive per event (`ingest_batch`) while shots are sampled ~2 s apart (`ingest_status`), so accuracy can exceed 100%. `st.shots_t` already makes a stale denominator detectable and `scoring.py:576` already gates SHARPSHOOTER at 10 shots; there is no per-metric settling concept. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) B5. `build`.
- **F118 🟡 MC's FEED PRINTS PLAYER-FACING TEXT AND A RAW PLAYER ID** — "Your Team Takes The Lead (002803e7)" in an FFA, from `presentation.py:157` (HUD copy) with the id unresolved, while kill rows in the same feed resolve names correctly. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) B4. `build`.
- **F113 🟡 THE GUN LED STRIP FREEZES AT A PARTIAL LEVEL FOR THE WHOLE DEATH.** Documented A16 behaviour (`engine.js:2179`, "death is deliberately hands-off") whose assumption the field refuted: it reads "a sliver of health left" while you are dead. Worse the bigger the per-shot damage. Overturning it is a decision; `test_led_invariants.py` must stay green. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) C3. `decision`.
- **F110 🟡 THE BRIEFING'S TITLE AND DESCRIPTION ARE SHEARED OFF AT THE BOTTOM.** `.bf .bfbody` (`index.html:629`) is a fixed-height flex column, so the title and description shrink below their content height and their own `overflow:hidden` clips them; the body never overflows, which is why nothing caught it. Measured at three viewports. NOT the `max-height:360px` query. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A1. `build`.
- **F115 🟡 THE DOWN COUNTDOWN DIGITS TEAR AND OVERLAP ON iOS.** Saira Condensed has NO tabular figures, so the `tabular-nums` request is a lie ("11"=113px vs "88"=178px) and the centred number re-lays-out every tick under a running transform; `line-height:.8` at 190px also leaves glyphs 73px taller than their box. MC assumes the same of Oswald — measure before animating (S25). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A5. `build`.
- **F117 🟡 THE POST-MATCH READY BUTTON IS STYLED AS THE INERT ONE.** `hud.js:228` labels the `onReady` control "MATCH COMPLETE" (a statement) in muted grey at 20px with the SAME `ready wait` classes as the genuinely inert STANDING BY button (`:231`). Players do not know they must tap it. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A2. `build`.
- **F126 🟡 THE iPHONE DEBUG PANEL FONT IS INFLATED** — `-webkit-text-size-adjust` is set nowhere in `app/www/index.html`, so WKWebView auto-inflates text in `#diag` (a scroller). One line to fix; compounds F122. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A4. `build`.
...
- **S26 🟠 PASSIVE BACKGROUND LOG SYNC — stop making the operator tap SHARE LOG on every phone.** Tony 2026-09-11: "game sync has priority. once that is completed, we should ask MC if ready and we should safely patiently background sync for the game logs. maybe an option on MC for this. For almost all of my games I am going to want the logs from all phones." ⭐ **HALF-BUILT ALREADY:** `pull_log` is a defined envelope kind on both sides (`mc/envelope.py:59`, `transport/envelope.js:43`), **MC already pushes it to every player node at recap** (`state.py:2062`) and again on every `log_offer` (`:1543`, capped ~1 MB per node), and the reference `mock_node.py:406` answers it. **The real app has NO handler** — grep finds `pull_log` only in transport plumbing, so both phones ignored MC all night and the only logs that arrived were manual taps. ⚠ Log frames **bypass the retry ring** (`app.js:335`), so an out-of-coverage phone loses its log silently; patient sync needs the ring or a re-request on reconnect. Blocked-on nothing; makes F122 far less painful. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D6. `build`.

- **S23 🟠 "FINAL / MATCH RESULTS" — one job, six reports.** Win/lose, mode stats (hill time, team scores), a match history, muster-time stats, per-team AND per-player views, animated in on sync. Most needs NO new plumbing (kd/streak/medals already pushed and discarded; `score.board`; `this.hold`; the 50-match `localStorage` history; the sync-complete condition already computed). Two things do: the OUTCOME push (win/lose CANNOT be inferred node-side — silence means "lost" and "out of coverage" identically) and per-player rows in team modes (`_push_scores` already loops every row and sends each node only its own). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D3. `build`.
- **S22 🟠 WEAPON BALANCE: SIDEARMS MUST NOT OUT-KILL RIFLES.** Model confirmed exact across all 22 weapons: ttk equals (htk-1) times `wire.fire_ms`. Deagle 1500 and USP 1600 both beat the FASTEST rifle (rifles span 1650-1800); only the Glock complies. Tony also wants the USP faster-and-weaker, which only holds if damage drops enough to raise htk. Root cause is the pools: 45 HP plus 70 armour equals 115 is why everything takes many hits. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D2. `decision`.
- **S20 🟡 COLLAPSE THE TRY-OUT INTO SELECTION.** The tick means "equipped with the host" while "armed on the gun" hides behind TRY IT — two states that look like one. Decided: selecting arms immediately (debounced ~400 ms); TRY IT becomes REVIEW KIT, with weapon details on the row. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D1. `build`.
- **S24 🟡 MC MATCH PAGE LEGIBILITY** — 9px headers over 14-16px values, six numeric columns at a 10px gap with no rules or grouping, K/D/A three identical 40px dim columns. Below the repo's own 11px floor. Also the STK column shows the current streak (F116). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D4. `build`.
- **S25 🟡 A SPECTATOR / BROADCAST VIEW FOR MC** ("espn quality"). Resolve the tension first — MC has been pushed toward calm and scannable; a scoreboard wants big and animated. `Live.tsx` mixes score display with END MATCH EARLY and RECALL, which must not be on a screen pointed at a room. Measure Oswald's tabular figures before animating numbers (see F115). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D4. `decision`.
- **S21 🟢 DROP THE CAM LOOK-THROUGH, KEEP THE QR SCANNER.** Decided 2026-09-11: remove the button and the `camera-preview` dependency; KEEP the iOS and Android camera permissions or QR joining silently breaks too. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D5. `build`.
