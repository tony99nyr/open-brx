...
- **F260 🟡 THE GUN'S NATIVE SHIELD-HIT SOUND IS WRONG SOMETIMES.** Bench 2026-09-18, Tony, stripping a 120 shield 20 at a time: "it played the wrong shield hit sound sometimes". The `$HIR` trace for that run shows the shots landed on TWO different sensors, some on the headset (token 1 = 0) and some on the gun body (token 1 = 4), so the suspicion is that the native sound tracks the SENSOR struck rather than which pool absorbed the hit. The shield hum and the shield-hit sound are native and we do not write them (S45), so if this is real the fix is either a `$SIR` row per sensor or a node-played cue over the top. Cheap to settle: fire ten shots at the headset alone, then ten at the gun body alone, with a shield up, and listen. `bench` + `ears`.
