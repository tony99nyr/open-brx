...
- **F253 🔴 THE HUD IS TOLD TWICE THE SPARE ROUNDS THE GUN IS GIVEN.** `mc/compile.py` `resolve()` writes the catalogue's `reserve` to **t17** and `reserve // 2` to **t40**, and `spawn_ammo()` hands the phone the same `reserve`. F207 proved the gun's own `$ALCD` reserve mirrors **t40**. So an assault rifle whose catalogue says 192 compiles to `t17 192 / t40 96`: the HUD opens the life showing **192** spare rounds and the gun holds **96**, until the first `$ALCD` corrects the HUD downwards mid-fight. Every host-facing number derived from `reserve` (the MC stats panel, `views.weapon_view`, the TTK model) is doubled the same way. **Two ways to fix it and they are not the same game:** write the catalogue number to t40 and twice it to t17, which DOUBLES what every player carries, or keep the wire and halve what the HUD and the host are told, which leaves the balance alone. `test_mc_compile.py` pins the current reading (`spawn_ammo()[1] == t17`), so the guard moves with the decision. Needs the bench first: fire a full magazine, reload to empty, and count what the gun actually spends. Found by the 2026-09-17 polish review, out of the same range that corrected the site's Reserve column. ⚠ **The published figures are not in doubt:** the site and the manual publish each weapon's own CAPTURED frame, where the assault rifle's t40 is 192, so they describe Callsign's stock gun correctly. This row is about the SHIPPED weapon, whose catalogue 192 reaches the player as 96. `bench` + `decision` (brx-weapons owns the balance half; the bench sheet `bench-perks-2026-09-18.md` counts the refills, in a section that is on **main**, so merge main into this branch before the bench).
- **F254 🔴 Needs Tony at the bench: THE `$SIR` TABLE IS NOW ELEVEN ROWS, AND TEN IS THE MOST EVER PROVEN.** S50's armour-piercing perk adds a permanent fn-2 cell, so every head and every `sir_pool` take now carries **eleven** rows. `docs/manual/dev.md` records an A/B of one row against ten; nothing has ever tested eleven. If the firmware caps the table, the eleventh row drops **silently**, and `assert_armor_piercing_armed` still passes, because it reads the compiled text and not the gun. Bench, two minutes: `$QUERY` the table back after a head write and again after a `sir_pool` take, count the rows, then fire an armour-piercing shot and confirm it moves HP and leaves armour alone. Then confirm the fn-2 row lands at fn 28 in the head, so an AP shot inside the 2100 ms spawn-protection window does nothing (A44). Found by the merge review 2026-09-18. `bench`.
