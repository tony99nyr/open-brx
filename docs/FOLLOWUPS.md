...
- **F259 🟢 SHOULD `$PB*` AND `$AS` JOIN THE DENY LIST?** Four of the six blocking audio waits sit in the gun's own game paths (the mode announcement, game-over audio, two channel-4 waits), which `$PBSTART`/`$AS,1` enter. They stay on the known list (bench tools use them; no compiled bundle carries them). Screamers A1-A3 test only
