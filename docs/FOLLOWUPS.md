...
- **P18 🔴 IS fn 24 A STATUS FUNCTION OR THE DELAYED BLAST? Two 2026-09-11 findings disagree.** A20 (host-driven stun) ships `$SIR,8,0,,24` on the charge-rifle cell and calls fn 24 "a STATUS function: `$HIR` fires, no pool moves, no `$HP` follows" (bench-proven chain); the same night's F69 bench found the hill's chip damage "manufactured INSIDE the gun by the fn-24 blast row" and set the rule "never ship fn 24-27 on any cell". `compile.py` now carries both comments (~284 and ~383). Bench: arm a stun game, take one EMP word, watch the pools for ~10 s — if health moves late, the stun row needs another function (fn 28 registers silently; which status fn also plays the EMP sound is the question).
