...
| B2 | **Native phone app** to replace Callsign | 🟢 **BUILT** | Native Capacitor app (Android + iOS), the per-player game engine + HUD, reporting to Mission Control over the LAN. Web Bluetooth is dead — see `docs/adr/0003-native-app-over-web-bluetooth.md` + `app/README.md`. Field path (MC↔phone) still VERIFIED for the live path (2026-08-25/26 real phone→MC→gun sessions); soak/scale still unverified on hardware (verification-checklist). |
| B3 | **Mission Control** (scan → assign games/teams/weapons → live scoreboard) | 🟢 **BUILT + TESTED** | `docs/spec/mission-control.md`. Operator console built + tested **with operator auth**; compiles per-player `FrameBundle`s (`mcp/brx_mcp/mc/compile.py`) and drives the live game over the LAN (WebSocket). MC↔phone field path still VERIFIED for the live path (2026-08-25/26 real phone→MC→gun sessions); soak/scale still unverified on hardware (verification-checklist). |
...
| P9 | **Max native team count** (`$TID` range) | ✅ **RESOLVED (bench exp 4, 2026-08-26)** | `$TID` is **masked to 2 bits** — effective team = `$TID & 3` (tok4 read 4→0, 63→3, 100→0). **All four masked teams (0–3) are usable** — a re-run with a properly re-armed victim (spawned, team 1, full `$SIR`) caught **5 clean `$HIR,4,0,0,2,24,0,0`** registrations from a `$TID,2` shooter; the earlier team-2 silences were a **bench-script re-setup race** (shooter caught mid-`$CLEAR`), not a team-2 limit. So **native team count = FOUR (0,1,2,3)**; larger squad counts → MC logical teams (FFA + armbands). **FF sub-open now CLOSED:** friendly fire is **NOT IR/firmware-enforced** — same-team damage lands under `$GSET` token1=**0 and 1** (FF=1 probe: 7 registrations `$HIR,4,0,5,1,9,0,0`). So "FF off" is **app-side bookkeeping** (our MC scorer already tracks `friendly_kills` separately — architecture already right), and `$GSET` token 1's on-gun function is now **UNKNOWN**. P9 fully closed. See exp-log + `brx-protocol.md` `$TID` row. |
| P10 | **IR damage value in the hit payload** | ✅ **RESOLVED (bench exp 2, 2026-08-26)** | `$HIR` **token 5 = the applied damage, EXACT** across 4 weapons (AR 9, Shotgun 45, Sniper 80, Rocket 115) — it equals the `$WEAP` `t5` field, so **damage-weighted scoring and heavy-weapon balance are BLE-native** (no IR-payload bit-decode needed). **Token 2 = the shooter's IR protocol** (0 standard, 10 on the rocket) — the explosive/tag-type signal Jay described; **token 7 = weapon subtype** (sniper 1). **Armor model pinned:** armor absorbs 1:1, overflow spills to HP, **no per-hit cap** (feeds the engine/sim — B15). See `brx-protocol.md` §7r. |
...
- ~~SUPERSEDED (see U0-closed below: t20=fire mode + t23=burst PROVEN 2026-08-26)~~ **Capture the burst-fire token.** 2026-08-26 field: Burst Rifle fired single heavy shots (no burst) —
...
- ~~Fire-mode token 'likely does not exist' — **WRONG, SUPERSEDED same day: t20 IS the fire mode, PROVEN by one-field flip** (see the U0-closed entry below)~~ Original text: Range session 2026-08-26:
...
- **`$GSET` token1's on-gun function is UNKNOWN.** Labeled "friendlyFire" by the teardown, but the bench
  proved FF is not firmware-enforced under EITHER value (2026-08-26) — the token does something else or
  nothing. Probe by diffing behavior with it flipped across other mechanics.
