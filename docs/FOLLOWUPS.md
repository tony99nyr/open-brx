...
- **F258 🟠 THE LOCK-UP DETECTOR: SILENCE PLUS NO `$PONG` MEANS POWER-CYCLE.** A screamer answers nothing and its BLE link may stay up (the radio is a separate chip), so "connected" proves nothing and F208's gun sat byte-identical for 105 s. Design: while LIVE and alive, after `GUN_SILENT_MS` (about 8 s, needs the idle `$VOLTS` cadence confirmed) send one `$PING`; no `$PONG` after two pings 3 s apart = `gun_locked`: a HUD takeover that tells the player to hold power 3 s and power on, `status.gun_locked` for the board, and a relink that marks the player down and revives on the normal timer instead of keeping stale pools. Replaces the disabled B4 watchdog reading (F163). The HUD half belongs to the `brx-hud` session. Screamers sheet A13 and A1 (does the link stay up on a hang?). `transport-hardening.md` §7. `build` + `bench`.
- **F259 🟢 SHOULD `$PB*` AND `$AS` JOIN THE DENY LIST?** Four of the six blocking audio waits sit in the gun's own game paths (the mode announcement, game-over audio, two channel-4 waits), which `$PBSTART`/`$AS,1` enter. They stay on the known list (bench tools use them; no compiled bundle carries them). If screamers A1-A3 show a `$PB*` start can hang a gun, they move to `protocol.DENIED_COMMANDS`. `bench` + `decision`.
