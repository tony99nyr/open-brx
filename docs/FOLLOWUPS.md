...
- **F71 🟠 The three headset weapons may do far more damage than we publish.** The catalog derives `dmg`, `htk`
  ...
  **→ 2026-09-18 capture (cap30): TWO WORDS, MEASURED ONCE.** One Callsign Shotgun pull from shooter id 1 landed
  ...
  (the cycle is 900 ms, so it cannot be two pulls). Distance unrecorded. Which emitter sent which word is
  SOURCED, not settled (LaserTagMods/Jay, 2026-09-18, one expert statement): the gun sent the 45, the headset
  the 70 (`docs/weapon-design.md` §7). A capture cannot show which emitter fired, so this needs one bench run
  covering a single emitter at a time; F254's run answers it in passing.
