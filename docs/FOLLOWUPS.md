...
- **F129 🟡 M2 UI POLISH RESIDUALS (round-2 review; the fix lane was cut off by the session limit 2026-09-12).** Live/Spectate at 393 px scroll sideways with no affordance → a fading right edge or `▸ SCROLL` hint; Armory headset copy `PROVEN BY LINK` vs `CONNECTED` are not parallel → `CONNECTED (LINK)` / `CONNECTED (ECHO)`; the notice toast squeezes `NEW MATCH ▸` onto three lines at 393 px → its own row; the `#spectate` latch is never releasable and its comment/test say a reload returns to the console → release on a fresh load with a non-spectate hash; Kit keeps a stale refusal line after the roster becomes ready; the LOGS button's "never gated" and the AUTO meaning live only in `title` → a one-line legend; the roster `READY` tag clips at 393 px; e2e step 12 shoots LOBBY under a KIT name; on the phone the rack hero says EQUIPPED while the row is still ⟳ → `ARMING…` until the ack.
