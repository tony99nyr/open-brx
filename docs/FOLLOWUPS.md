...
**Ids.** One capital letter + number. Never renumbered, never reused. **Next free: B32 · D6 · E8 · F260 · G11 · H9 ·
K9 · P19 · Q20 · R4 · S54.** (2026-09-18 evening, the drive integration on branch `jay-drive-integration`: **F255-F259** taken, the transport-hardening measures of `spec/transport-hardening.md` §8, renumbered at the merge from F254-F258 because main had taken F254; F206 root-caused and its fix shipped, row kept open for the bench.) (2026-09-18: **F254** taken: at what `t13` value does the headset word (t12) stop
...
**🔴 blocking, at a glance (2026-09-16):** **F206** team modes register NOTHING (116 shots, 0 hits; **root cause found in the V4_31 firmware and the fix shipped 2026-09-18**, one bench run to confirm) ·
**F207** the START echo refusal is a false positive on every gun · **F208** a gun can die with the HUD holding
the player alive and no way back · **F209** the respawn delay collapses in a burst and spawn protection does not
hold — all four from [`game-test-2026-09-13.md`](game-test-2026-09-13.md), all root-caused from the MC store.
*Earlier (2026-09-11):* **F68** a miss permanently kills the headset team colour ·
...
names the bench step in [`bench-firmware-levers-2026-09-19.md`](bench-firmware-levers-2026-09-19.md) that settles it.**

- **F255 🟠 ARM PACING: A BLOCK PAUSE, AND THE RUNT `$SIR` ROWS.** The gun reads ONE serial byte per main-loop pass from a 1 KB buffer, and a lost `*` corrupts the NEXT frame (V4_30/V4_31). An arm is 43 frames / 1012 B / 75 packets; 13-14 of its fn-28 registrar rows are 21-22 bytes, a full packet plus a 1-2 byte runt, the packet most exposed to a loss. `brxlink.WRITE_PACING` now holds the field pacing (8 ms per chunk, 18 ms per frame) and a block pause that ships OFF (`blockFrames = 0`); turning it on is one line. Do not move it on a reading: bench §14.1 (200 arms, count the frames that did not apply), §14.2 (bursts with and without a pause) and §19 give the number, and one gun with the trailing empty token trimmed off the registrar rows against one with the full row says whether the runt can go (`$SIR` is positional; F11 makes this a real risk). `transport-hardening.md` §3. `bench` + `build`.
- **F256 🟡 WRITE WITH RESPONSE FOR MULTI-PACKET FRAMES.** Every write is `writeWithoutResponse`, so a lost packet is invisible. Jay's ESP32 and the 2018 BC app both wrote every chunk with response. Cost: an arm goes from about 1.2 s to about 2.5 s. Worth it only if §14.1 shows frames going missing; then one line in `brxlink.write` and `ble._write`, head and spawn only. `transport-hardening.md` §5. `bench` + `build`.
- **F257 🟠 `$QUERY` READ-BACK AFTER ARMING.** The V4_30 `$QUERY` reply starts `<playerId>,<team>,<hpMax>,<armourMax>,<shieldMax>`. One `$QUERY,*` after the head echo, parsed into `ack_config` beside `gun_echo`, lets MC compare the gun's TEAM against the pushed `$TID` and refuse `start` on a mismatch: the check that would have caught F206 on every gun at the lobby. Needs claim 19 (§18) first: the token map is a firmware reading, and the 2026-08 capture proves only the shape. `transport-hardening.md` §6. `bench` + `build`.
- **F258 🟠 THE LOCK-UP DETECTOR: SILENCE PLUS NO `$PONG` MEANS POWER-CYCLE.** A screamer answers nothing and its BLE link may stay up (the radio is a separate chip), so "connected" proves nothing and F208's gun sat byte-identical for 105 s. Design: while LIVE and alive, after `GUN_SILENT_MS` (about 8 s, needs the idle `$VOLTS` cadence confirmed) send one `$PING`; no `$PONG` after two pings 3 s apart = `gun_locked`: a HUD takeover that tells the player to hold power 3 s and power on, `status.gun_locked` for the board, and a relink that marks the player down and revives on the normal timer instead of keeping stale pools. Replaces the disabled B4 watchdog reading (F163). The HUD half belongs to the `brx-hud` session. Bench §14.3 and §14.4 (does the link stay up on a hang?). `transport-hardening.md` §7. `build` + `bench`.
- **F259 🟢 SHOULD `$PB*` AND `$AS` JOIN THE DENY LIST?** Four of the six blocking audio waits sit in the gun's own game paths (the mode announcement, game-over audio, two channel-4 waits), which `$PBSTART`/`$AS,1` enter. They stay on the known list (bench tools use them; no compiled bundle carries them). If §14.4 shows a `$PB*` start can hang a gun, they move to `protocol.DENIED_COMMANDS`. `bench` + `decision`.
...
- **F206 🔴 TEAM MODES REGISTER NOTHING. 116 SHOTS, ZERO HITS.** The 2026-09-13 TDM match recorded not one `hit_taken` and not one `death` in 42 s while the status stream counted 116 shots (103 + 13); both FFA matches on the same guns in the same half hour registered normally. Teams were pushed correctly (`$TID,1` vs `$TID,2`, both phones advertising the right team) so this is NOT team assignment. The one variable across the three heads is `$GSET` **t1 `friendlyFire`**: FF ON → hits, FF OFF → nothing, and `compile.py:978` is `friendly_fire=(mode == "ffa")`, so **every non-FFA mode we ship sends the value that registers nothing**. This is **F49** at game scale, and the reading both fit: t1=1 disables the team gate, t1=0 registers only a MATCHING team. **And the store says where the mismatch comes from:** all 38 `hit_taken` facts carry `shooter_team: 0` while both shooters were on `$TID,1`, and `protocol/brx-protocol.md` §5 documents `$HIR` t4 as the shooter's effective `$TID & 3` — it should read 1. The emitted word is not carrying the team we set. ⛔ NOT `$PSET` t2: that token is bench-proven inert (0/1/7 identical, protocol §3). **Leading suspect, untested:** we send `$TID` as the LAST frame of the head (`compile.py:1238`) and `$SPAWN` arrives later, with A23's live `$SIR` table riding it; nothing re-sends `$TID` at spawn (`compile.py:1397` is the only re-send anywhere) and protocol §3 already notes `$SPAWN` clears a painted LED colour. Two repo facts frame it: Callsign's own arm contains **no `$TID` at all** yet its victim stream reads team 1 (`protocol/captures/2026-08-23-two-tagger-combat.txt`), while our bench 2026-09-07 says `$TID` DOES reach the word (tid 4 → team 0, tid 5 → team 1) — so our 0 on `$TID,1` contradicts our own measurement. **First rung, five minutes and two guns:** one gun `$TID,1`, one `$TID,2`, friendly fire ON so a hit is guaranteed to register, read `$HIR` t4 on the victim. If it is 0 both ways, `$TID` never reaches the emitted word and F206, F49 and Q13 collapse into that; then re-send `$TID` right after `$SPAWN` and read t4 again — if it becomes 1 the fix is one frame. **Then** push TDM with `friendly_fire=True` + opposite TIDs; if hits land, ship t1=1 and score friendly fire in MC (per **Q13** the gun's gate buys us nothing we cannot do better server-side). [`game-test-2026-09-13.md`](game-test-2026-09-13.md) A1. **→ 2026-09-18: ROOT CAUSE, from the V4_31 firmware image (LaserTagMods' drive).** The gun keeps ONE team byte: `$TID`, `$TEAM` and `$PSET` t2 all write it, the outgoing word reads it, the friendly-fire compare tests it, and the last writer wins. The "⛔ NOT `$PSET` t2, bench-proven inert" line above was measured with `$TID` sent AFTER the `$PSET`, so it could not see this. `gameconfig._pset` hard-coded t2 = 0 and the node writes a `pset_pool` `$PSET` in the same burst as every `$SPAWN` (`engine.js` `_spawn`/`_revive`), so every gun went live as team 0; FFA works because it ships t1 = 1. **Fix shipped** (`d0f4c729`): every `$PSET` carries the `$TID` team, `$TID` is re-sent right after every `$SPAWN`, an infection flip revives onto the new team, and `assert_team_byte_consistent` refuses any bundle whose `$PSET` disagrees with its `$TID` (`test_f206_team_byte.py` breaks the builder and watches it go red). **Bench confirmation pending:** `bench-firmware-levers-2026-09-19.md` §1 (runs a-f; run f is the end-to-end TDM). The `$QUERY` read-back that would have caught this at the lobby is **F256**. `trigger` (one run) + `build` (done).
- **F207 🔴 `GUN ECHO ≠ CONFIG` IS A FALSE POSITIVE, AND IT REFUSED START ON HEALTHY GUNS.** Three weapons, six acks, 2026-09-13: mag correct every time, reserve **exactly half** every time (sniper+extended_mags `8/24` vs `8/48`, amr `14/28` vs `14/56`, burst_rifle `36/108` vs `36/216`). That half is not an approximation — it is **the t40 value MC itself wrote into the same `$WEAP` frame** (`compile.py` keeps the captured-frame invariant `tok17 == 2 * tok40`). The gun's `$ALCD` reserve field mirrors **t40**; `frames.py` compares it to `_WEAP_RESERVE = 18` (**t17**), so the check **can never pass on any weapon on any gun**. **This ANSWERS F201** (96 vs 192 was t40, not a clamp and not a halving). Fix: compare against `t17 // 2`, with a test built on a real captured head — `test_mc_config_proof` proves the check against MC's own synthetic frames, which is exactly why it stayed green. Shipping the refusal FORCEABLE is what saved the session. [`game-test-2026-09-13.md`](game-test-2026-09-13.md) A2. `build`.
- **F208 🔴 A GUN CAN DIE WHILE THE HUD HOLDS THE PLAYER ALIVE, WITH NO WAY BACK.** 2026-09-13 18:47:34, the incident that ended the session: a 45-damage hit, then ROCCO's status is **byte-identical for 105 s** (`alive=True hp=45 armor=5 ammo=36 shots=25 src=gun`) while his node log shows no `☠ down` and the frame ring shows `$BUT` press/release pairs still arriving — the trigger being pulled, nothing firing. Two separable faults: (a) **there is no staleness detector on the pool anywhere**, so a gun silent for two minutes looks exactly like a healthy idle one on the HUD and on the MC board; (b) the phone ended with `alive:false, respawnIn:0, respawnGate:"trigger"` under an `auto` config (**F212**), i.e. gated on a pull that the ring proves was arriving. Fix: a pool-staleness watchdog that says so instead of showing a confident stale number, plus an operator mid-match resync (today the only exits are a whole-lobby RE-PUSH or ending the game — Tony ended the game). [`game-test-2026-09-13.md`](game-test-2026-09-13.md) A3. **→ 2026-09-18:** the V4_31 firmware has six audio waits that block the main loop with the serial port unread and no hardware watchdog, so a gun CAN lock with its BLE link up and answer nothing (the "screamer"). The watchdog half of this row is designed as **F257** (`spec/transport-hardening.md` §7: silence plus a `$PING` with no `$PONG` = locked, tell the player to power-cycle, re-arm on the relink). `build`.
- **F209 🔴 THE RESPAWN DELAY COLLAPSES TO ZERO IN A BURST, AND SPAWN PROTECTION DOES NOT HOLD.** "you can die and get shot before the weapon comes online during respawn" (Tony). In a game with `delay_s: 5`: **five deaths and five respawns inside one second**, every respawn at +0.0 s after its death (18:38:17 ROCCO; again 18:40:34 TONY), and outside the bursts hits land at **+1.6 s, +2.9 s, +3.0 s** after a respawn. Nothing on the death path has a refractory period, so a burst of `$HIR` is processed as N independent hit→death→respawn cycles with the delay skipped rather than queued. The post-respawn hits are the first field look at **F121/A23**'s spawn protection, whose bench gate has never been run: either it is not landing, or it ends at `$SPAWN` while the weapon needs ~2 s more (`write gun take` lands ~3 s after `respawned`). The asymmetry IS the bug: **hit reception returns before the trigger does.** Fix: one death per life until the respawn completes, and a protection window that ends when the weapon is armed. [`game-test-2026-09-13.md`](game-test-2026-09-13.md) B1. `build`, proof needs two guns.
- **F210 🟠 CONNECTING THE PHONE HUD BEFORE THE HEADSET GETS STUCK IN A LOOP** (Tony, 2026-09-13). Not captured — every node log in the session opens with the gun already paired. Needs one repro with the debug panel open: launch, link the gun, leave the headset off. Note `preflight.headset_ok` is already reported to MC and was `false` in this session's first statuses, so the state is known to the app and is evidently not gating whatever loops. [`game-test-2026-09-13.md`](game-test-2026-09-13.md) C1. `build`.
- **F211 🟠 THE HUD DOES NOT CHECK WHETHER BLUETOOTH IS OFF** (Tony, 2026-09-13): "otherwise list doesnt load and its confusing". The picker simply never populates, with no message. Both platforms expose adapter state; add a `bluetooth_on` beside the `preflight` fields the phone already sends (`gun_linked`, `ssid_ok`, `mc_reachable`, `cellular_off`, `dnd_on`) so MC's ITEMS board can see it too, and one line of copy on the picker. [`game-test-2026-09-13.md`](game-test-2026-09-13.md) C2. `build`.
- **F212 🟠 `respawnGate: "trigger"` UNDER AN `auto` RESPAWN CONFIG.** Both phones end both of the last two 2026-09-13 matches with `respawnType:"auto"`, `respawnIn:0`, `respawnGate:"trigger"` against configs that say `respawn {type:"auto", delay_s:15}`. Either the gate is mislabelled or a trigger-gated respawn is reachable in a mode that never asked for one; **F208** is what that costs when it goes wrong. Read `engine.js`'s respawn gate against `contracts.md`'s respawn table — no hardware needed to decide which. `build`.
- **F213 🟡 THE HUD SHOWS ARMOUR ABOVE ITS OWN MAXIMUM.** Both phones, two matches, 2026-09-13: `"armor":120, "maxArmor":70`. `body_armor`'s `max_armor_add: 50` is baked into the pushed `$PSET` (120) but `maxArmor` is still the game's 70, so every armour bar and every percentage dividing by it is wrong for anyone carrying the perk. `compile.py` knows the real ceiling; it just is not sent as the max. `build`.
...
- **F215 🟡 THE SNIPER'S EXTENDED-MAGS AMMO READOUT DOES NOT MATCH THE GUN** (Tony, 2026-09-13): "the hud lies, it isn't accurate with the number". `extended_mags` is `ammo_mult: 2` and `_mods` scales mag AND reserve, so the sniper's 4/24 compiles to 8/48. Per **F207** the echoed 24 is the head's own t40 and proves nothing about the real reserve, so the store cannot separate the two readings: the doubling landed and the HUD is right, or it landed on the magazine only and the player really has 24. **The test is one minute:** push the sniper with `extended_mags`, fire dry counting rounds, compare against 48 — that count settles F207's residue, this row and **A10c** at once. Same pass: ROCCO ended a match holding `reserve: 201` against `loadReserve: 216` with a **shotgun** (own reserve 24) in hand, i.e. the HUD's reserve appears to track the primary while the secondary is out. `trigger`.
...
- **F225 🔴 THE CHARGE RIFLE LANDS HALF ITS DAMAGE: `$SIR` FUNCTION 38 IS A FLAT ×0.5.** Bench 2026-09-17, two guns, headset front dome with the gun sensor covered: a full charge (magnitude 100) landed **50** and a tap (magnitude 20) landed **10**, every time, at `$GSET` t7 0 and 50 (t7 was sent mid-life, so treat the t7 half as provisional). MC ships `$SIR,8,0,,38,…` for the Charge Rifle's `<t3,t4> = <8,0>` key, so in every game a pre-charged ambush leaves the target at 65 of 115. Re-pointing the key at function 1 (`$SIR,8,0,,1,0,0,1,,*`) landed the full value (charge 90 → 90, taps 20 → 20); a second row for the same key REPLACES the first. The 85 the playtest bench read is unexplained (different table). Fix: function 1 for `<8,0>` in the compiled table, and a compile guard that no damage weapon keys to a multiplier row by accident. `build`.
- **F226 🟠 A CHARGE WITH FEWER ROUNDS THAN A FULL CHARGE LOCKS THE GUN.** Bench 2026-09-17: a full charge spends **10** rounds (measured 4 times; a tap spends 1). With 6 rounds left, holding a charge fired nothing, heat jumped to **156**, and two reload-lever pulls did not refill; every later charge attempt read heat 156 and fired nothing until `$AMMO` refilled the cell. From exactly 10 rounds a charge fires normally. A player reads this as a broken gun. Fix: a catalogue `rounds_per_full_charge` (10) that the one-magazine guard and the ammo sums divide by, a HUD "NOT ENOUGH ENERGY" state below 10, and a bench recheck of whether the reload is refused or only delayed by the lockout. `build` + `trigger`.
...
- **F113 🟡 THE GUN LED STRIP FREEZES AT A PARTIAL LEVEL FOR THE WHOLE DEATH.** Documented A16 behaviour (`engine.js:2179`, "death is deliberately hands-off") whose assumption the field refuted: it reads "a sliver of health left" while you are dead. Worse the bigger the per-shot damage. Overturning it is a decision; `test_led_invariants.py` must stay green. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) C3. `decision`. **→ 2026-09-12:** BUILT: `_death` writes one `gun.blank` frame at once (nothing when the bundle has no gun table), `_gunTake` repaints at respawn, `$HLED,,6` still never in play; led-language.md amended; stage mirrored. **Bench gate:** a fast two-shot kill mid-animation → the strip goes dark immediately, repaints at respawn.
- **F126 🟡 THE iPHONE DEBUG PANEL FONT IS INFLATED** — `-webkit-text-size-adjust` is set nowhere in `app/www/index.html`, so WKWebView auto-inflates text in `#diag` (a scroller). One line to fix; compounds F122. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A4. `build`. **→ 2026-09-12:** BUILT: `-webkit-text-size-adjust:100%` on html/body; screen-truth pins the computed value. Confirm on the iPhone (WKWebView cannot be reproduced in Chromium).
- **F112 🟡 THE CAM BUTTON CRASHED THE iPHONE** — `NSCameraUsageDescription` was absent, and iOS hard-terminates on camera access without it; `android-setup.sh` had carried the permission all along, only iOS was missing. Fixed in `scripts/ios-setup.sh` 2026-09-11, **not yet verified on device**. The QR join scanner shares the permission (`app.js:429`), so it stays even though the look-through is dropped (S21). Why Android also failed was never diagnosed. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) A6. `build`.
- **F111 🟢 TRY-OUT OVERLAP NOT REPRODUCED** — "READY UP overlaps the damage line" was reported on the iPhone, but at 891x411, 667x375 AND 812x375 with the demo payload there is 27px of clearance and no overlap. Hypothesis: the real MC-pushed `WeaponView` renders more stat rows than the demo's four. Needs a device screenshot or the real bundle in the harness. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) C4. `eyes`.
- **F120 🟢 THE UNSTOPPABLE MEDAL PLAYS NO AUDIO.** It fires, the bundle carries its `$PLAY` frame, and VX0U is `on_gun`. Both cheap theories refuted (both guns were in earshot; 0.1.8 postdates the medal path by 6 days). Narrowed by a success: the voice slot demonstrably works (kill lines play through the same form), so the fault is VX0U itself — the only medal sound that is `voice:game_mode` not `voice:medal`, with in-repo precedent for clips silent over BLE (`compile.py:1148`). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) E1. `ears`.
- **F114 🟢 WRONG SOUND ON A DEAGLE KILL** — "groaning heavy breathing", identified as VA6 (5.96 s, the male `hurt_loop` slot). First ear ever on A15.3. `low_health` REFUTED (all nine kills at hp=45/armor=70) and a bad kill-line id REFUTED (the male kill pool is clean). Leading untested theory: it came from the VICTIM's gun. The store holds no sound envelopes, so this cannot be settled from the desk. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) E2. `ears`.
...
- **F49 🔴 UNEXPLAINED, and it contradicts our reading of polarity.** Bench 2026-09-07, gun on `$TID,1`, armed by
  ...
  **→ 2026-09-13: REPRODUCED IN A GAME (F206).** A TDM match with `$GSET` t1=0 and correct opposite TIDs took 116
  shots and registered ZERO hits, while two FFA matches with t1=1 on the same guns in the same half hour registered
  normally. Every `hit_taken` all session carries `shooter_team: 0` and no gun was ever on TID 0. The reading that
  fits both this and the bench: t1=1 disables the team gate, t1=0 registers only a MATCHING team. All 38 hits that
  session read `shooter_team: 0` on guns set to `$TID,1`, where protocol §5 says `$HIR` t4 is the shooter's
  effective `$TID & 3` — so the emitted word is not carrying the team we set. Read t4 back with FF ON first.
...
- **F65 🟡 `$BUMP` is inert on v4.32 — is that the command or our shape?** Bench 2026-09-09: `$BUMP,-5,0,0,*` on
  ...
  **→ 2026-09-18: ANSWERED, it was our shape (the arity).** The V4_30 firmware, BC's own sheet and the 2018 BC app
  agree on `$BUMP,<amount>,<hp 0/1>,<armour 0/1>,<shield 0/1>,<sound>,*`: tokens 2-4 pick the pools, so our probes
  asked for nothing (`$BUMP,0,5,0,*` put 0 in the amount and set no flag). The same day's Callsign capture (cap30)
  confirms the armour flag on the wire: the shield recharge sends `$BUMP,12,,1,,,*`, and every frame adds 12 armour,
  clamping at the maximum. Firmware reading, not yet measured: a negative amount cascades shield → armour → HP and
  kills at 0, and a positive one heals HP and spills outward. That is the one-command zone damage `$LIFE` cannot do.
  Still to bench on v4.32: the hp and shield flags, negatives and the cascade (`bench-firmware-levers-2026-09-19.md`
  §5). `trigger`.
...
- **S16 🟠 Damage over time, on the node** (unblocked by the 2026-09-09 bench; raised to 🟠 2026-09-17, Tony: "we don't have any damage over time weapons, like a poison gun"). `$LIFE` takes negatives, so
  ...
  **per-pool with no spill**, so the node walks shield → armour → health itself; the pool **floors at 0**, so
  ...
  11 = gas). ✅ **2026-09-18 bench: the NATIVE route is DEAD.** A single fn-24 shot does no damage at all, it only
  manufactures a phantom hit every 5.07 s (P18, closed). So the node tick clock in `spec/node.md` §3.17 is the ONLY way to build
  poison, exactly as designed. The proc mechanism it wanted also arrived the same day: `$WEAP` t6 makes the gun roll a crit on a
...
- **Q15 🔴 → THE RANGE LEVER IS REAL, BUT IT IS `$WEAP` t2 `gunRangeOutdoor`, NOT t41 (garden, 2026-09-17).**
  ...
  **Shipped 2026-09-17 (outdoor only, `docs/weapon-design.md` §4.2):** Sniper 100; AMR/Charge Rifle 85; AR/Burst 70;
  ...
  headset word has its own reach, `t13` 80 outdoor / `t42` 30 indoor (35.5 / 29.25 kHz under `weapon-design.md`
  §6.5's carrier formula), so a far target takes 45 and a near one can take 115 (F71). And with the app's venue on
...
- **F162 🟠 Needs Tony at the bench** characterise the remaining venue controls after the 2026-09-13
  ...
  **→ 2026-09-17: t41's emitted-power effect is MEASURED and it is nil in outdoor mode (F231/Q15); t2 `gunRangeOutdoor` is the live control.** `$GSET` t3's effect remains unmeasured: `RANGE_ENV_OVERRIDE` stays a no-op
  ...
  is F169; indoor reflections are F198. See `HANDOFF-gset-t2-2026-09-13.md` for the field evidence. **→ 2026-09-18 (V4_31 trace, LaserTagMods' drive):** the mechanism is the IR **carrier frequency**, not power: the gun emits at `38000 − 125 × (100 − t2)` Hz (≥ 100 = 38 kHz; 75 = 34.9 kHz; the 13–31 window is 27.1–29.4 kHz), and the receiver's 38 kHz band-pass is the curve; emitter power changes only with the indoor/outdoor level (duty 20 % / 38 %); t41 replaces t2 only indoors AND when non-zero, which is why it was inert outdoors; `$GSET` t3 is stored and never read. Calibrate in kHz (S48/S49). `bench`.
...
- **F231 🔴 `$WEAP` TOKEN 2 IS `gunRangeOutdoor`, THE EMITTED-POWER CONTROL, AND NOTHING SHIPS IT.** Garden 2026-09-17, outdoor mode, `$GSET` t2=0: t2=5 landed **0 of 38** shots (15 at 3 m, 23 with the muzzle on the dome); t2=100 is the stock value and reaches 200 ft. Ladder between: 13 → 3/8 at 15 m on precise aim, 25 → 15/32 at 10 m, 32 → 15/32 at 10 m, 50 → 11/11 at 3 m, 21/23 at 10 m, 100% at 20 m, 90%+ at 30 m, ~33% at 40 m. Shape: a floor where nothing arrives at any distance, a transition roughly **13–26**, then a flat shelf from ~31 to 100 where every value behaves the same at any distance we can pace out. Position derived from the APK field order (`callsign-extract/protocol-classes.md`: `slotType, iRSource, gunRangeOutdoor, primaryDamageType, …`) against the bench anchors t5=damage and t14=fire-rate; `weapmap.py` calls t2 `(scale/const)` because both derivation frames held it constant. The shipped catalogue proves it is a live field: **every gun 100, melee 90**. ✅ done 2026-09-17 (this lane): t2 is promoted to `docs/manual/dev.md` beside the t41 row, named in `weapmap.py` FIELDS, and `mc/compile.py` now drives t2 from `wire.range_outdoor_pct` (F234 closed). Still open: whether t2 fences a weapon to a distance above the shelf, and the shaded indoor/outdoor re-run below. ⚠ **Every number above was shot into a dome in DIRECT SUN with the light moving, and the misses came in streaks of 5-6** — re-run the ladder shaded before shipping a value. `bench` + `build`.
...
- **F233 🟡 THE GUN HOLDS AT LEAST FOUR `$WEAP` SLOTS BUT ALT ONLY CYCLES TWO.** Bench 2026-09-17: `$WEAP,0..3` each acked with its own `$ALCD` and took an independent `$AMMO`; the ALT button toggles slots 0 and 1 only, so 2 and 3 are configured and unreachable. Callsign only ever used two. Worth knowing for (a) a four-weapon loadout and (b) bench A/B work — four configs per walk instead of two, if a `$BMAP` can be found that selects a slot directly. `bench` + `build`.
- **F234 🟡 `RANGE_ENV_OVERRIDE` IS STAGED ON THE WRONG TOKEN.** `mc/compile.py` carries a venue → range map (`gun_range_pct`, `RANGE_ENV_OVERRIDE`) deliberately parked as a no-op until a bench confirmed which token moves emitted range. It is wired to **t41**, which 2026-09-17 proved inert in outdoor mode (Q15/F231); the live token is **t2**.
  **The range-plumbing half is ✅ CLOSED 2026-09-17 (this lane):** `resolve()` now writes only `t2` (`gun_range_outdoor_pct`, fed by each weapon's `wire.range_outdoor_pct`), `t41` is never written and stays byte-identical to the capture, a floor guard refuses to compile `t2` under 13, and the shipped per-weapon outdoor table is in `docs/weapon-design.md` §4.2. Indoor is left untouched (still F231). `docs/manual/dev.md` and `weapmap.py` `FIELDS` now name t2 `gunRangeOutdoor` too.
  **Still open here:** the brx MCP `connect()` refuses to re-open an alias whose link has dropped ("alias already connected", then "Service Discovery has not been performed yet" on the next write) — `disconnect` then `connect` is the workaround and it cost several minutes across the session; make `connect` reconnect a dead alias in place. `build`.
...
- **F201 🟠 Needs Tony at the bench** every gun echoed `$ALCD,32,100,0,96,0` against a compiled reserve of 192 —
  ...
  refusal can become force-proof. (filed 2026-09-13 from HANDOFF-gset-t2-2026-09-13.md §3) `capture`. **→ ANSWERED 2026-09-16 by F207:** 96 was the head's OWN t40 (`tok17 == 2 * tok40`), and the gun's `$ALCD` reserve mirrors t40 while the check reads t17. Neither a clamp nor a halving — the comparison reads the wrong field. Four weapons agree. Yes, the refusal can be force-proof once it compares the right two numbers.
