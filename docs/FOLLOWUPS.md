...
- **S59 🟢 HALO SHIELD HUD.** Tony 2026-09-24: BUILT, variant C "Visor": a 16 px bar on the top edge, Halo blue, red when low or broken with a red frame tint, a lime overshield layer by day (no fade, only hits), red-only at night; no words, numbers or timers (DOWN means waiting for respawn); health only bottom left; no armour number or bar in a game without armour. The gun LED keeps one teal pool. Lows: the night creep rail is 1.86:1 against its fill (raise the gate to 1.8); a hit during a stun leaves the frozen creep stale; demo.js `$145` replace idiom. `build`.
- **F331 🟢 POWERUPS LOWS (polish 2026-09-24).** The station screen says BY PLAYER 7 (roster name); the armour number still shows 0 at max_armor 0; a restored item is not re-validated (spawn_every_s 0 would hang the tick); the AdvertGate does not rate-limit state-bit flaps; a stunned player keeps claim_ready while walking away; the console strip shows restored items with the flag off; the Python stage decoder ignores byte 15 `taker`; the overshield `$LIFE` mode 2 on a dead gun is unmeasured (bench 3.3 item 7). `build`.
