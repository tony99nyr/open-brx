...
- **S23 🟠 "FINAL / MATCH RESULTS" — one job, six reports.** Win/lose, mode stats (hill time, team scores), a match history, muster-time stats, per-team AND per-player views, animated in on sync. Most needs NO new plumbing (kd/streak/medals already pushed and discarded; `score.board`; `this.hold`; the 50-match `localStorage` history; the sync-complete condition already computed). Two things do: the OUTCOME push (win/lose CANNOT be inferred node-side — silence means "lost" and "out of coverage" identically) and per-player rows in team modes (`_push_scores` already loops every row and sends each node only its own). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D3. `build`. **→ 2026-09-12:** SPEC A24 written (results push to every node, `score.rows`, `welcome.node.result`, the node never infers win/lose). Tony 2026-09-12: the match ends at the TIMESTAMP of the confirmed winner's cap kill; late-flushed facts can move that moment earlier and MC RE-SCORES by replaying the stored facts; facts after the moment ride the recap as an unofficial "after the whistle" block, never counted. A31: a pre-game "a win is confirmed at Mission Control" line for every MC-decided end state unless full coverage or all phones have backhaul.
- **S22 🟠 WEAPON BALANCE: SIDEARMS MUST NOT OUT-KILL RIFLES.** Model confirmed exact across all 22 weapons: ttk equals (htk-1) times `wire.fire_ms`. Deagle 1500 and USP 1600 both beat the FASTEST rifle (rifles span 1650-1800); only the Glock complies. Tony also wants the USP faster-and-weaker, which only holds if damage drops enough to raise htk. Root cause is the pools: 45 HP plus 70 armour equals 115 is why everything takes many hits. [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D2. `decision`. **→ 2026-09-12:** research in `docs/reference/ttk-model.md`: ideal `(htk−1)·fire_ms`, expected `(htk/p−1)·fire_ms`; at p=0.7 the Deagle is the SLOWEST sidearm and a 13-hit USP empties a 16-round mag before the kill 3 times in 4 → USP mag ≥ 20. Tony's ordering (USP fastest + weakest, Glock middle, Deagle slow + heavy) overrides `test_pistol_identities_keep_the_counter_strike_ordering`.
...
- **S24 🟡 MC MATCH PAGE LEGIBILITY** — 9px headers over 14-16px values, six numeric columns at a 10px gap with no rules or grouping, K/D/A three identical 40px dim columns. Below the repo's own 11px floor. Also the STK column shows the current streak (F116). [`game-test-2026-09-11.md`](game-test-2026-09-11.md) D4. `build`. **→ 2026-09-12:** prerequisite MEASURED: no product font has tabular figures (Oswald 500 spreads 12 px on 2 digits at 40 px; Chakra Petch 20 px), so digits need fixed-width cells before any count-up. Carry the F116/F119 UI residuals here: STK shows `best_streak` (Live.tsx/Recap.tsx), ACC dims while `acc_provisional`, `FeedEntry.kind 'alert'` + tags ALERT/WITHHELD/ROLE in types.ts; `r.error` on END is already shown.
