...
- **F71 🟠 The three headset weapons may do far more damage than we publish.** The catalog derives `dmg`, `htk`
  ...
  **→ 2026-09-18 capture (cap30): TWO WORDS, MEASURED ONCE.** One Callsign Shotgun pull from shooter id 1 landed
  ...
  covering a single emitter at a time; F275's run answers it in passing.
  Our compiled Shotgun ships `wire.dmg` 20 + `wire.headset_dmg` 20 with `t13`/`t42` locked at 100 (always lands,
  no range falloff). Still open: the range at which a headset word WOULD drop out, see **F275**.
