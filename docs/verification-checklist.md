...
- ✅ **Night mode / LEDs off — PASSED 2026-08-30 (P17 closed; mechanism corrected 2026-09-02).** `gameconfig` ships Callsign's own `$GLED,,,,5,,,*` and it turns all three LEDs off. ⚠ **Not because "token 4 = 5 is off"** — token 4 is an **apply gate** (0/6/7/8/9/10 apply the frame's colours at full brightness, 5 applies them at ~1/3, 1/2/3/4 are no-ops), so this frame blanks because **its colour tokens are empty and t4=5 applies them**. The "t4=3 blanks" reading stays **retracted**: 3 is a no-op, so it left the previous colour lit. ⬜ **Remaining:** confirm a blanked gun stays dark once **spawned** — a spawned gun runs a native health gauge on those same LEDs.
