...
(b) Every fix carries a paired CONTROL assertion in the house style. (c) `/polish-loop` was asked for and is not
a skill this environment has; the nearest available pass (`/code-review` on the branch diff) ran instead.
