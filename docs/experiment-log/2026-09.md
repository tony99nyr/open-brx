...
### 2026-09-07 (bench, cont.) — 🎨 EIGHT TEAMS WORK END TO END; the headset palette is 0-7 like the gun; a paint holds 8 min with no traffic
...
- **`$TID,4/5/6/7` are all accepted and the firmware BREATHES the matching palette colour** (purple, teal,
  ...
  So **eight teams work end to end** — gun body, headset and `$TID` agree on one palette. A fourth team is
  PURPLE (tid 4), which the A16 design wanted specifically to keep green off a headset (green there reads as
  the native hit/out flash at range).
