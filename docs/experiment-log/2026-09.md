...
6. 🔴 **A lethal negative really kills, and says so with the wrong frame.** `$LIFE,-45,0,0,*` at HP 45 produced a
   ...
   corrected 2026-09-10 by a code reviewer:** `engine.js:1265` books a death inside `case 'LCD'`
...
helpers in four test files and `__main__._hir()` all defaulted the shooter's wire id to **0**, a value the
platform never puts on the wire. The suite had been proving that the engines correctly credit kills to a shooter
