...
PA/LNA + 3× AMS1117 adapters, overnight). Full BOM + power + wiring in `docs/archive/hardware/bench-shopping-list.md`;
...
## 2026-08-26 (overnight) — no-hardware fix batch for the bench punch-list (commit 5278561)
...
**Compiler bug found + fixed (a5dbec2).** `compile.py` had been writing `fire_ms` into `tok15` — the
...
The re-based catalog (71cd827) ships each weapon's own t20/t23 verbatim.
...
Full range on the re-based catalog (71cd827): **fire modes all correct in the field** — bursts burst
...
`6d74728` and verifies clean on the Windows venv against `mcp` 2.0.0. Marked done.
...
and one that has to be re-run. `7bc32f6` cannot settle the sensor question precisely because it
recorded no tok1.
...
3. **`7bc32f6`'s "fn 24 damages on protocol 7 (armour 70 -> 30)" does NOT reproduce.** fn 24 is
