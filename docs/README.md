...
- [`mode-extensibility.md`](mode-extensibility.md) — critical review of **opening the project up**: what JSON can/can't customize for game **modes** (why a new ruleset needs Python across ~4 files; the E1-E4 plan to make a mode a drop-in plugin + JSON block) and for **sound packs / custom announcers** (the gun plays only its on-gun bank by id — no audio-over-BLE; the phone speaker is the unbuilt clean path; E5-E7).
