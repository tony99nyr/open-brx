...
**Hill timing (A68).** MC sends `duration_ms` from LOAD onward and in ARMED/LIVE when the game has a time limit. Adopted matches omit it, and so does END's recap push. A duration lets a Stick that missed START anchor go-live itself. The anchor is the first alive player advert with the Stick's exact game byte, at any RSSI, from a player the Stick has already heard DOWN in this game. A phone in the lobby advertises down, so a Stick armed at MC hears every lobby player. A phone still alive from the last match cannot anchor: a welcome gives it the new game byte while it is still alive (`engine.js`). A Stick that heard nobody down waits for a death and a respawn in its range. Until the anchor the hill shows WAITING, stays neutral and counts nothing; F374's 60 s offline go-live applies only to a config without a duration. The hill counts from the anchor to anchor + duration, then freezes its owner and hold tally (MATCH OVER). A START config overrides the fallback when heard. A new game byte clears the anchor. The Stick saves the time left in NVS at the anchor, then at most every 30 s (`HILL_CLOCK_SAVE_MS`), and as 0 at the whistle. A restart resumes the whistle from that save, so it is late by at most 30 s plus the time the Stick was off. A restart before the anchor waits for a new one. A same-game config without a start (LOBBY, or an abort) stops an anchored or resumed hill and clears the saved time left. The start error is the delay until the Stick first hears a spawned player. Untimed games and operator or objective ends cannot reach an offline Stick. MC sends no `station_update` for a hill.
...
|---|---|---|
| **`held`** (the MVP field mode and the boot default, serial `LINK HELD`) | joins at MC and **stays linked for the whole match**, reconnecting per contracts §5 backoff | A timed hill can receive a START correction while in coverage |
| **`muster`** (serial `LINK MUSTER`, saved in NVS `assoc`) | joins at muster, takes `station_config`, then drops the association | Field stations that rely on offline fallback |

`held` is the MVP field model, because its timed hill must receive MC timing when coverage allows. A Stick with no saved mode boots in `held`; a saved `LINK MUSTER` still wins. The case for `muster`:
...
| 2 powerup | yes | **built, ON by default** (docs/spec/powerups.md, A56, F372): MC stores the item, compiles the pickup slots, runs the spawn schedule and sends `station_update`; `--no-powerups` turns it off |
