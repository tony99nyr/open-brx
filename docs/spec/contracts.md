...
```
**[A38]** `unrostered_phones` = connected companion phones with a gun set, claimed by nobody on the roster and not
...
|---|---|---|---|
| A38 | 2026-09-13 | `ReadinessSnapshot` gains **`unrostered_phones: int`** — connected companion phones with a gun set that nobody on the roster has claimed and that are not parked on STANDBY either (field 2026-09-12: "4 guns connected, only 2 in lobby" — MC only ever surfaced this on ARMORY). Computed by `state.py unrostered_phone_count()`, the same matcher ARMORY's own NodeCard claim form uses (`_find_player_for_gun`), now parametrized over an optional `roster` dict so it can be asked of `self.standby` too instead of the claim form re-deriving the standby exception on its own. Never blocks `go`. KIT and LOBBY each show a "N CONNECTED PHONES NOT IN THE ROSTER" banner when the count is nonzero, linking to the ARMORY claim card. | `mcp/brx_mcp/mc/state.py` (`_find_player_for_gun`, `unrostered_phone_count`, `readiness`); `types.py ReadinessSnapshot` (+ `contract.gen.ts`); `webapp/mc/src/screens/Kit.tsx`, `webapp/mc/src/screens/Lobby.tsx`; `mcp/tests/test_mc_state.py`; `webapp/mc/test/unrostered-phones.test.tsx` |
