...
`reach == "backhaul"` (`on_backhaul`). **Its `level` is always `"zones"` (F256, below) until a node reports an
independent transport (F309)**; the rules that follow describe what a derived `"full"` will do then. It rides on the snapshot as `coverage: { level, on_backhaul,
