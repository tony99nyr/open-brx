...
|---|---|---|---|
| A56 | 2026-09-24 | POWERUPS, PROPOSED (docs/spec/powerups.md; built behind a flag, off until bench Sitting A proves the spare weapon slots and the `$BMAP` cycle). A powerup station's `StationAssignment.item?` `{weapon_id, charges, cooldown_s, name, color}` (the same object in the player's `config.stations[]` entry); `GameConfig.powerups?` `[{weapon_id, slot}]`, the pickup weapons MC arms at start in slots 2 and 3 with an empty magazine and out of the ALT cycle; a player fact `pickup {match_id, station_id, weapon_id, t}`; MC's `station_update {id, depleted_for_ms, depleted_until?}` to the station (time remaining: a Stick has no synced clock; `cooldown_s` 1-255, `name` at most 12 characters, `color` `#rrggbb`; agreed with brx4 for H8). The grant is a mid-life `$AMMO` plus a `$BMAP` ALT-cycle write, never a config re-push. | powerups.md (proposed) |
