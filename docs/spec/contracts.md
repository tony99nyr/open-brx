...
FrameBundle {                       // per (config_id, player_id); pushed in `config`, re-pushed on assign change
  ...
  spawn:   string[],   // go-live tail at T-0: DISARMED $SIR×n (fn 28) → $PLAYX,0 → $SPAWN,, → $AMMO per slot → $BMAP,0,0   [A23, A44: the live table is NOT here; the node writes one sir_pool take once the gun can fire]
  revive:  string[],   // respawn: DISARMED $SIR×n (fn 28) → $SPAWN,, (+ loadout-correct $AMMO) → $BMAP,0,0 (a live resync re-writes the held head first)   [A23, A44]
  ...
  sir_pool?: string[][],             // [A17, A44] one full LIVE $SIR table per take; the rows differ ONLY in their sound tokens.
                                     ...
                                     //   off, the rolled takes when on. The node writes ONE take after every spawn, revive and
                                     //   infection flip: on the gun's first shot, or SPAWN_PROTECT_MAX_MS (2100 ms) after the write,
                                     //   and again at the end of a rejoin reconcile. Re-sending $SIR rows is the F11 REPAIR path.
