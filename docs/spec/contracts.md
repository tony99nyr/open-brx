...
FrameBundle {                       // per (config_id, player_id); pushed in `config`, re-pushed on assign change
  ...
  spawn_protect_off?: string, // "$TMP,,,,,,,,0,,,,*": the node writes it on the gun's first shot, or SPAWN_PROTECT_MAX_MS
                       //   (2100 ms) after the spawn/revive write, and at a rejoin reconcile end or an operator resync.
