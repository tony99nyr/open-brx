...
| `station_update` | `{ id, available, next_spawn_in_ms? }` | **[A56]** MC → a **powerup** station, on a pickup and at each spawn time, and again on a reconnect. `next_spawn_in_ms` is time remaining (a Stick has no synced clock). The station advertises state 1 (available, `value` 0) or 0 (taken, `value` = seconds to the next spawn, capped at 255). Behind the powerups flag (docs/spec/powerups.md). |
