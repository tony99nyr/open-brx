...
|---|---|---|---|
| F2-1 | Headset domes never registered a hit | 🔍 | **Re-counted 2026-09-01 after Tony noted the headset has FOUR sensors: `$HIR` tok1 0,1,2,3 are ALL headset, 4 is the gun.** I had counted only 0 and 1, halving the headset total. Correct figures for that session: **headset 108, gun 62 — the headset caught 64% of all 170 hits**, first one 17 s in. So the headsets were working throughout. What remains unexplained is narrower: the 5–10 min window ran 0 headset / 11 gun, and the deliberate point-blank test failed then and worked later (30–40 min: 92 headset, 0 gun). ⛔ Refuted: `outdoorMode`, daylight, gun uptime (cycled before every game). Protocol doc corrected — it documented only 0/1/4. See V1. |
