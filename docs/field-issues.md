...
|---|---|---|---|
| F2-1 | Headset domes never registered a hit; nozzle held on the dome, both directions, both headsets | 🔍 | **The live question.** Ours: 10/10 `$HIR` on sensor `4` (gun body), zero on the domes. Callsign's capture: domes caught 3 of 23. Tony has played natively outdoors in direct sun with working headsets, so daylight is NOT the cause. **`$GSET` token 2 `outdoorMode` is the ONLY token where we differ from all 19 captures** (we send 1 outdoors; Callsign sends 0 in every capture we hold). Next: run with venue INDOOR and repeat the dome test. |
