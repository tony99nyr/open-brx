...
|---|---|---|---|
| F2-1 | Headset domes never registered a hit | 🔍 | **The timeline contradicts "never": the first dome hit landed 17 s in, and the first five minutes ran at 6 dome / 50 body ≈ 11%, matching Callsign's native 13%.** The 30–40 min block (75 dome / 17 body) is the deliberate nozzle test once it worked. ⛔ Refuted so far: `outdoorMode` (domes returned with the venue still outdoor), daylight (native outdoor play works), and gun uptime (they were power-cycled before every game, so it cannot explain the bad period or the recovery). The open question is narrower: **why point-blank on the dome failed at that moment**. Aim vs a transient link fault — see V1. |
