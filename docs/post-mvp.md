...
## 1. Modes, extensibility and spectating
...
- **F314 🟠 THE STICKS3'S ONBOARD IR RECEIVER CANNOT DECODE A GUN SHOT; AN EXTERNAL ONE CAN.** Root cause found (2026-09-24): the onboard receiver (G42) decodes an ordinary TV remote fine, so its optics and RMT path are sound, but it does not pick up BRX-style IR, gun or emitter, at any distance tried; M5's own docs and a matching forum report agree the hardware is the limit, not the firmware or the gun's carrier. Workaround proven: an external VS1838B wired to the Stick's Grove G9 with an internal pull-up (`RXPIN 9`, `0f50605e`) decoded 3 of 3 real gun shots at 1 m as whole 25-bit words, sharing power with another receiver already on the bench. Remaining: build the standalone wiring (the VS1838B powered from its own Grove pin, not shared) and run a HILL capture test with real shots on a standalone Stick. Keep this row open until that test passes. `bench`.
...
- **F359 🟡 HUD VISUAL QA ROUND 2: THE LOWS.** Filed 2026-09-24 from the phone HUD VQA round 2 (the Critical, Highs and Mediums on main are branch `hud-vqa2`; R2-01/03/10/11/22 belong to the alert redesign). R2-22: ENEMY DOWN by a teammate uses the same styling as your own KILL CONFIRMED (alert redesign). R2-23: with Location off, two filled primary buttons compete on the picker. R2-24: during the death flash the slot label reads "PICKUP ASSAULT RIFLE 02/0". R2-25: the scan rows still show raw dBm values. R2-26: the team dots stay at night. R2-27: the `live-shields` stage label says "spawn at 0", which F348 superseded. Also open from R2-07: the pre-game, loadout, result and diag labels are still 11 frame px (8.7 on the SE screen); the in-game screens are gated at 11 on screen. `build`.

- **F360 🟡 F356/F357 EDGE CASES (narrow).** Filed 2026-09-24 from the F356 final review. (a) After an MC restart from a LIVE snapshot whose post-whistle write failed, `_arrival_cap_recv` derives the cap moment by sorting stored facts on `t_recv`; a `t_recv` tie between two synced nodes breaks by the eff order, not true arrival, so it can land on the wrong side of the tie. Carry the store row id as the tiebreaker. (b) `_end_on_frag_limit` sets `cap_recv` after the whole batch: a same-node multi-kill flush whose second kill lands in the tie band still gets a kill confirm. Owner brx3. `build`

- **F362 🟢 INTEGRATION REVIEW X1-X10, THE LOW FINDINGS LEFT OPEN.** Polish loop 2026-09-24 on the X fixes. (a) X9 guards only the alert and pool cards: an MC `medal` item that went silent and is displaced by a kill fires `_eventLeds` again when it comes back. (b) A spree fold marks a folded medal-only item as said, so its IR twin stays silent and nobody voices that kill. (c) `adopt_orphan` breaks an even split of game bytes towards the higher byte, not the newest report. (d) The death scream enters the audio model before `_death` clears the fill window, so it counts as stuck for up to one tick. (e) A must-hear line in the first seconds of a Shields life flushes the spawn line and the klaxon too (normal must-hear behaviour). (f) A pool repair inside the fill window sets `_prevShield` to the maximum, so the fill's answer does not end the window (it expires after 5 s). (g) An overshield grant before the fill's answer bases its write on shield 0. (h) The stage ends the fill window outside the live phase and in the tutorial; the engine does not. **→ 2026-09-25, the MC/contract review's Lows left open:** (i) `_arm_station` sets `restarts` to 0 before the station's first status arrives; (j) the station snapshot still leaves out `assoc` (the tally is now in it); (m) Polish Lows: a suppressed kill moves `last_kill_t` but adds no chain entry; one tunnel bucket per IPv6 address (key by /64?); the refusal-log count is lost if a flood stops; the frozen-kill oracle reads MC's own `cap_recv`; a crash inside the 2 s snapshot debounce can lose the last lead change, so the first kill after the resume says it again. (n) Pre-existing, for Tony beside A63: a late death inside the tie band makes `_reconcile_end`'s `t`-order probe move a frag-cap end back to a crossing the board passed only for a moment (a team kill took it back), the kind of crossing F363 now ignores on a resume. (i) and (j) stay because each changes what the ITEMS panel flags: (i) a first heartbeat that carries only a `boot_count` is judged at its arrival time, so a reboot before the lock can count as a restart; (j) a restored `assoc` would flag every held station OFFLINE until its first heartbeat after an MC restart. (k), (l) and three (m) Lows are closed (2026-09-25). `build`.
...
- **F317 🟡 MISSION CONTROL HAS NO OUTDOOR OR HIGH-CONTRAST THEME.** MC visual QA 2026-09-23 (M25): the console has one dark theme, and a laptop in daylight at a field may be hard to read. A design question for Tony: whether MC needs an outdoor or high-contrast theme, and who switches it. `decision`. **→ Tony 2026-09-25: POST-MVP** ("would be good to get some daylight gameplay experience before I weigh in on that anyway").

## 7. Protocol, firmware and research
...
The bench-sheet aliases that used to sit in FOLLOWUPS §9, for post-MVP rows. The running order is
[`bench-plan.md`](bench-plan.md); these lines keep the method.
