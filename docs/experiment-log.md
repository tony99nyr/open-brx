...
| 2026-09-06 | 2026-09 | LED language review (four lenses) → `docs/led-language.md`, S10, F33, F34; docs consolidation executed (status layer → 3 files, archive/, hygiene test) (2) |
| 2026-09-06 | 2026-09 | voices: soundboard on the gun stage, 24 characters, slot 2 = gas death, rolled `$PSET` takes + per-kill pools (A15/A15.1), empty cry field + node-played spawn line (A15.2) |
| 2026-09-07 | 2026-09 | repo hygiene: keep one repo (the coupling + cadence case), raw Callsign assets removed, binaries purged from history (93 MB → 28 MB), CONTRIBUTING + code of conduct, UI contract test caught three real drifts (1) |
| 2026-09-07 | 2026-09 | **the LED night**: the native death flash was never missing, our own `$HLED,,6` was disabling it (deletes A11.8's pulse scheme); `$HLOOP` is the down signal; apply-gate 5 is OFF, not ~1/3 (retracted); token 5 is the dimmer and is GLOBAL; A16 + A16.2 + A16.3 built (dark rest, seven-level pool bar with a drop animation, night overlay, headset roles); `$TID` is 0-3 only (F35); the ~600 ms hit-to-LED lag removed; three polish rounds, seven defects, four of them stage-vs-phone divergences (14) |
| 2026-09-09 | 2026-09 | the manual website rebuilt around what it costs to edit (plain markdown, one file per page), then the rewrite reviewed: nine guards that could not see their own fault (2) |
| 2026-09-10 | 2026-09 | **the hill nights**: F69 was the hill SCORING, not a blind spot; rung S and the objective heartbeat blocking regen; CAPTURE PROVEN END TO END and the capture pair is not a same-burst transition; `mag=53` means the point was neutral; a live team switch moves hit resolution but not the LEDs; a third capture settles n=2 plus a magazine-count trap; rung Y negative (fn 28 ignores the `$SIR` sound field); holding a hill does not buff rate of fire (8) |
| 2026-09-11 | 2026-09 | King of the Hill built in one night; the first real game test on the Mac (1v1, two taggers, 25 new ids); two followups triage passes; the site refactor (two doors, one source); dogfooding "start Mission Control" as a fresh agent; and three bench sittings — F69 REFUTED (the chip damage is manufactured inside the gun by fn 24), F23 CLOSED (fn 36/37 scale on the HEADSET only), and the sound pass ($PLAY has an interrupt slot and a queue slot, seven rows closed by ear) (9) |
| 2026-09-12 | 2026-09 | the game-test sheet worked at the desk: 14 fixes landed, spawn protection, and the WSL crash was one spinning test (1) |
