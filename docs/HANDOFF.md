...
2026-09-25 desk review on `brx4/stick-sitting-b`: A68 now freezes timed hills at the whistle and on MC END, RECALL or PANIC. A live marker starts hills after the runway; MUSTER waits for it, with a 60 s offline fallback. The Stick protects a restored timed hill until MC supplies a fresh clock, and RESET cannot restart a finished tally. F399 now uses a 100 ms claim tie window. The requested host suites and simulator are green. Bench work remains for F383's -78 dBm radius, F386-F388 and F399 latency. F382's phone tick remains open under the `app/src` restriction. See `experiment-log/2026-09.md` for the desk evidence.
