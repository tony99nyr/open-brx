...
artefact, and the `/arsenal` page. The recoil rungs ship per weapon (S54, F268, F280 closed, `aa7b08b9`); the balance
decisions live in the Balance rules table at the top of `weapon-design.md`. F260 is closed: dual-emitter words share a `shot_group`, so MC counts one physical pull for accuracy while retaining both damage facts.
...
3. **Decisions before more code:** F291's open duel rule and F310 (brx1's balance lane); B21 waits for Tony's
