...
- 2026-09-18 **S42** Stance, flinch and node-driven recoil: **cut by Tony, not built further.** The recoil writer re-sent the active slot's whole `$WEAP` frame plus an `$AMMO` restore up to every 250 ms under sustained fire, the largest BLE load on a gun, and long split BLE frames are a suspected cause of gun lock-ups. The writer, `GameConfig.recoil` and the `weapons.json` `recoil` blocks are removed. Stance and flinch are cut with it, because both were designed to act through the same accuracy writer. The mid-life `$WEAP` bench facts stay in `spec/node.md` §3.15.
