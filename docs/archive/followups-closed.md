...
- 2026-09-26 **F403** built (brx5): the phone BRIEFING shows `PICKUPS` with each active item once, in station order and its own colour (night: the night accent); MC's `game_brief()` carries `pickups` and re-sends `assign` when an item edit changes the line; no line with no items. The demo's RAIL GUN and OVERSHIELD colours were swapped against MC; fixed, with a guard.
- 2026-09-26 **F410** closed: MVP is outdoors only. `default_config()` and `compile.play_volume()` already defaulted every new game to `outdoor` (volume 90) from the first M-MC commit, and the builtin Silenced Sniper preset inherits it, so no server default changed. `docs/platform/index.md`, `docs/platform/platform.md`, `docs/platform/modes.md` and `docs/manual/sound.md` now say plainly that MVP games run outdoors only and indoor is a post-launch feature, while keeping the fact that an unrecognised venue still falls back to the quieter indoor volume as a safety net. Hiding the venue picker from the MC console, and the presets store rework, are F411's own work (brx3, branch f411-games) and are not duplicated here.
- 2026-09-26 **F221** closed: Tony approved the MC alert colours gallery (`C:\Users\Tony\brx-mc-alerts\index.html`) built to his red/amber/neutral rule; the catalogue is `webapp/mc/src/alerts/`.
- 2026-09-26 **F372** closed: Tony overrode the row's old bench gate ("rockets, railgun, overshield as powerup/pickups. yes lets enable them"). MC's `powerups_enabled` now defaults True; `--no-powerups` is the opt-out and `--powerups` stays a no-op for an old command line. The item presets (Rockets, Rail Gun, Overshield) are unchanged. The bench steps in `bench-2026-09-25.md` and `spec/powerups.md` "Bench gate" stay open, now as verification of the mechanism and the claim-threshold calibration, not as a condition for the default.
- 2026-09-26 **S32** closed: Tony picked direction C ("team colour") from the mode-art storyboard (`C:\Users\Tony\brx-mode-art\index.html`). All six modes (tdm, ffa, koth, extraction, infection, lms) now ship direction-C 640x350 JPEGs in `app/www/assets/modes/` and `webapp/mc/public/assets/modes/`; `mcp/tests/test_ui_contract.py`'s koth mode-art gap is removed and `webapp/mc/src/modeArt.ts` lists koth (its own `test/modeArt.test.ts` gap reminder removed with it). Melee stays the one open weapon-art gap, unchanged: Tony's call is that it is an ability every gun has, not a weapon pick. Verified live against a fresh build: the phone BRIEFING stage harness and MC's GAMES `?mock` screen both draw the real koth art, not the glyph/emblem fallback. Two storyboard findings are NOT fixed by this row: the 76 px MC mode cards still clip the top of the mark (F411 is rewriting that screen); the night skin still hides all BRIEFING art (`[data-env="night"] .bf .bfart{display:none}`): its own 2026-08-27 commit comment ("night: no art, no glow, no white, the red ramp only") confirms this is deliberate, so no new row is filed for it.
- 2026-09-26 **F382** closed: bench part 1, both halves of the row's own pass rule CONFIRMED. Match `7ad0b738cc`: the gun tick ran every 3 s held, 1.5 s losing, and went silent on both guns while CONTESTED; `e.hold` for the holding team read the same frozen value (28749 ms) 11 s apart while CONTESTED, proving the possession clock itself pauses, not only the audio.
- 2026-09-26 **F384** closed: bench part 1, CONFIRMED. A recapture at -66 dBm (GREEN LOSING → BLUE CAPPING → BLUE HOLDS) announced "hill captured" and fired the HUD event on the capturing gun.
- 2026-09-26 **F385** closed: bench part 1, CONFIRMED (Tony's eyes) for the plain capture-then-clear path; the row's down-then-respawn half was not repeated this sitting.
- 2026-09-26 **F387** closed: bench part 1, the row's stated pass rule (RANGE opens at 5 s, not about 2 s) CONFIRMED with a clean serial-logged hold (opened at exactly 5000 ms). The first control run that sitting (a 2.8 s hold that opened nothing) turned out to be confounded, cut short by a Stick idle-timeout bug found the same sitting (F386's own extension, below), not a clean "short hold does nothing" reading.
- 2026-09-26 **F333** closed: bench part 1, Tony walked every Stick screen (EMPTY, countdown, READY, TAKEN, CAPTURING, HOLDS, CONTESTED, LOSING, MATCH OVER) at arm's length; all read clearly. Only F398's known SPAWN-screen overlap stood out, already filed and still open.
- 2026-09-26 **F426** closed: the phantom team-2 hill total was the raw `hold_ms` a control station reports (`state.py _keep_station_tally`), which carries every tid the station ever saw, never filtered to the match's rostered teams — so a hill's tid-2 neutral time (F82) rode the tally straight into the recap and the ITEMS panel as if it were a real team. `Scorer.possession()` already excludes an unrostered tid from every team's total (`test_a_hills_neutral_time_is_nobodys`); `_keep_station_tally` now applies the same rule before a report's `hold_ms` is merged or shown, so an unrostered tid never reaches either surface. New test: `test_mc_stations.py::test_f426_a_hills_neutral_tid_is_dropped_from_the_stations_recap_not_shown_as_a_phantom_team` (fails without the fix; reproduces this row's own 122744 ms bench number).
- 2026-09-26 **F423** closed: team 3 is renamed PURPLE end to end, matching what the gun and headset actually paint (F35's wire identity, GREEN, is unchanged — see `contracts.md` A69). `state.py TEAM_DEFS`'s tid-3 entry is now `team_id: "purple"`, `"PURPLE TEAM"`, `#7b2cbf`; KOTH's default roster is `["blue", "purple"]`. Every team-colour-key map that must track MC's roster followed: `app/src/engine.js` (`TEAM_KEY`/`TEAM_NAME`), `app/src/utility.js` (`TEAM_KEYS`/`TEAM_NAMES`/`TEAM_ABBR`), `app/src/hud/hud.js` + `deathscreen.js` (`TEAM_COLOR`/`TEAM_INK`/`TID_KEY`), `app/www/index.html` + `utility.html` (`--team-purple`, `[data-team="purple"]` and the STATION setup button), `webapp/mc/src/tokens.ts` (`TEAM`), and `webapp/mc/src/screens/Items.tsx` + `Recap.tsx` (`TID_NAME`/`STATION_TID_NAME`, one name-map line each). `presentation.PALETTE[3]` stays "green" on purpose (the wire/LED palette, a separate fact); `test_team_color_consistency.py` now pins tid 3 to "purple" for the display-facing sources and checks the wire palette entry separately. The generated contract needed no regeneration (`Team`'s shape carries no literal colour enum). `docs/manual`/`docs/platform` already said the gun paints purple (F33, closed 2026-09-10) and needed no change; F413's row text corrected its colour list from green to purple.
