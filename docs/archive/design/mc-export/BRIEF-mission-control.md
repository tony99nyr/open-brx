...
- Per gun: **sticker name** (`GUN-A`), **powered?**, **headset connected?**, **battery %**, **link /
...
The `screenshots/` folder holds the official Callsign app's operator screens (local-only
reference — the UI to match and beat). Use them for layout/flow cues; their data is already
distilled into `weapon-roster.md`. Don't reproduce Callsign's exact look — we're improving on it.
