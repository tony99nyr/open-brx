...
time. It is not repeated here, and it has grown since this sheet was written. `$PY` below = the Windows venv python,
...
3. **`tutorial_frames()` also ships `$BMAP,0,0` only** — trigger fires, nothing else is mapped. No
   ...
   **staying connected** — the pattern `ff_ab.py` already uses. ⚠ `bench_common.AR`
...
5. **A STITCHED IR word can be parity-valid and WRONG in exactly the field you are measuring.**
   ...
   `proto=4`, `proto=1`, `mag=137`, `mag=41`, `sub=1`, `sub=2`, `player=16`. **BQ-C3 counts words by
   PROTOCOL and BQ-C1 counts MAGNITUDE 0**, so an ambiguous stitch manufactures the very signal those
...
Highest value per minute in the queue: BQ-A1 gates a whole feature class and validates an assumption
the syphon code already depends on.

| # | item | run | read | control |
|---|---|---|---|---|
| BQ-A1 | ✅ **DONE 2026-09-09** — `$LIFE` takes negatives, `$BUMP` is inert. Do not re-run; the ordering lesson it cost is in *Answered rungs* below | — | — | — |
| BQ-A2 | **P4** | `$PY mcp/tools/sendframes.py <addr> '$AS,1,0,0,0,0,0,0,99,*' '$UP,1,*'` on a live gun. ⚠ Also try LaserTagMods' actual shape, `$UP,100,<n>,0,*` followed by `$UR,*` — `$UP,1,*` matches nothing any source documents and may just be an 8th silent shape rather than a probe | not "does it reply" (known: no, across 7 shapes) but does the GUN's behaviour change after: fire, reload, LEDs, anything | fire and reload normally for a minute BEFORE sending, same gun same session, so "no change" is established rather than assumed |
| BQ-A3 | **1.5a (gates 1.5)** | `$PY mcp/tools/ally_remeasure.py <victim> COM8 10,11,9,15,31,32,34` | per-fn pool deltas against a **depleted** pool | fn 10/11 are the known heals: if they do not move, the rig or the team polarity is wrong, not the fn |

**Why BQ-A1 was first, and what it returned:** the firmware has no damage-over-time function, so a node applying
...
WSL→Windows boundary. **BQ-B0 first:** `$PY -m brx_mcp sounds ids:H06 <addr> --audit` — if the prompt
does not take your keystroke, run it from a Windows terminal instead of losing an hour.

| # | item | run | note |
|---|---|---|---|
| BQ-B1 · BQ-B2 · BQ-B3 · BQ-B4 · BQ-B5 · BQ-B6a · BQ-B6b · BQ-B7 · BQ-B8 | ✅ **ALL CLOSED 2026-09-11** (the sound sitting). Do not re-run; every pick and every rejection is in *Answered rungs* below | — | — |
| BQ-B6 | **S9** the events that failed the walkthrough | `sounds ids:U100,U13,U41,V114,VX0R,VQ8,VA3J,VR7,VS7,V8Y,VHV,VX0U,VA7F,VA7M,VA7K,V125 <addr> --audit` | **Confirmed 2026-09-11 (evening)**, promoted from TRIAL: `extraction_closing` = V114 "10 seconds" (VX0R rejected, numbers/menu announcer), `extraction_complete` = VS7 "Objective complete" (VQ8 rejected: Nexus commander is a zombie/creature voice, wrong register; VR7 kept as a note), `unstoppable` = VX0U "Domination", `killing_spree` stays VA7K (matches the shipped VA7H/VA7E/VA7Q family). **`extraction_tick` changes to `JAS`, queue slot (token 4)** — Tony wants it to loop for extraction, and the tick already re-fires every ~10 s so JAS queues cleanly (F42.1); U100 goes back to being F44's shield-loop trial. `presentation.py` now carries a per-event `slot` field; `extraction_tick`'s is `"queue"`. JAQ is the not-yet-assigned window-open candidate. Still open: the mode-preset sweep |
...
wants ONE filmed rung, not a theory: **BQ-B9** phone camera at 240 fps on the gun, stage `raw` `$GLED,6,6,6,0,10,,*`
...
| BQ-C11 | **A10d** Easy Reload | **full arm**; kit the `easy_reload` perk (policy forces no secondary), or by hand `$BMAP,1,97,,,,,*` in place of the default ALT row. Empty the mag, press ALT not the handle | `$ALCD` reload burst — same shape and timing as a handle reload? | pull the reload HANDLE on the same gun immediately before and after, as the known-good baseline |
...
| **BQ-D2 / K4** melee | the slot-4 guard in `engine.js` `_onAmmo`: an isolated `$ALCD` with slot 4 currently sets `activeSlot = 4`, overwrites `ammo`/`mag` from the melee slot (maxClip 1), and can count a swing as a shot. Fix it, and mirror it in `stage.py`, or the swing corrupts the HUD you are reading it on |
...
Four rungs and the whole Block B sound sitting are done; their rows in the tables above are one line each now.
What is worth reading in the four is why three of them gave the WRONG answer on the first attempt. The findings
themselves live in `experiment-log/2026-09.md` and, where they are wire facts, in `protocol/`.

- **BQ-A1 ✅ 2026-09-09 — `$LIFE` takes negatives, `$BUMP` is inert** (F61 closed; F64 and S16 opened; three
...
- **BQ-C1 ✅ 2026-09-09 — the simulated-recoil model is REAL** (F46 closed; F66/F67/F68/S17 opened; t21/t22
...
- **BQ-D7 ✅ 2026-09-10 — holding a hill does NOT change `$ALCD` cadence** (the "KotH rate-of-fire buff", a clean
...
### Block B — the 2026-09-11 sound sitting (every pick and every rejection)
...
- **BQ-B6b ✅ CLOSED 2026-09-11** — **F42.1** does the announcer slot pre-empt the effect slot?. Run: stage `raw`: `$PLAY,H02,4,6,VA33,,,,*` · then `$PLAY,H02,4,6,,,,,*` followed 300 ms later by `$PLAY,,4,6,VA33,,,,*` · then the reverse order. Answered: neither cuts nor overlays uniformly — token 1 always INTERRUPTS (either slot), token 4 always QUEUES, regardless of fx vs voice id (six trials, `experiment-log/2026-09.md` → *the sound pass*). `presentation.py`'s token-4 `game_over` is the one to keep; `compile.py`'s `Compiler.cues()` copies are dead code
- **BQ-B7 ✅ CLOSED 2026-09-11** — **S1** the `fx:hit` audit is complete (122/122: 105 in the audit file, 17 in `hitaudio.py`'s 2026-09-07 comments). Second sitting 2026-09-11 evening covered H32-H58 and the HM block: H35/H39/H40/H54 "hammer on metal", H52/H53/H58 "metal bucket", H44 "falling down, hitting the ground", H55/H56 darts, H41 breaking open + scatter, HM10-HM12 small sand explosion, HM13/HM14 small glass, HM1B cartoon boing + accelerating tick, HM1F equipping a heavy weapon, HM1O/HM25 launcher shots
- **BQ-B8 ✅ CLOSED 2026-09-11** — **F44** the shield loop is A10 (heard looping live; A08/A09 are the on/off pair; U100/U13 rejected as "a bomb ticking"). Method that worked: arm with the golden bundle over the MCP, rewrite the `$PSET` foot's last token, `$LIFE,0,0,20,*` up / `$LIFE,0,0,-20,*` down. No stage needed
