...
5. **`firemode_probe.py` disconnects when it is done** (config survives the drop). So it can never
...
| # | item | run | read | control |
|---|---|---|---|---|
| C1 | **F46** t21/t22 accuracy (the simulated-recoil model) | design already written: [`bench-weap-tokens-discovery-2026-09-04.md`](bench-weap-tokens-discovery-2026-09-04.md) §t21/t22, the three-count design. `firemode_probe.py <addr> assault_rifle 23=0`, then `22=0`, then `23=50`, control `22=100 23=100` between | three counts per run: pulls (`$ALCD` mag), words on board A (length, trailer, magnitude), victim `$HIR` tok5 | the 100/100 control between every rung. 10 pulls / 0 words = the shooter suppresses; 10 words at magnitude 0 = the D=0 miss; 10 clean words = inert |
...
| C3 | **t7-t11 secondary fire** (no id yet) — the per-shot damage-type question | `firemode_probe.py <addr> assault_rifle 8=100 9=5 10=5 11=50`, board A capturing with `native_capture.py sec-t7 COM7 60`. Then repeat under `$GSET` t6=0 vs t6=1 | count captured words **by decoded protocol**: does a protocol-5 word ever appear, and does its rate move with `$GSET` t6 | t7 empty (every stock frame) as the control: zero protocol-5 words, proving nothing already leaks there. Read on the RIG, not the victim — protocol 5 has no `$SIR` row, so a victim would discard it silently |
...
| D6 | **1.5 U11′** the status functions | after A3's keyboard pass, fire enemy 8, 24-28, 35 and ally 31, 32, 34 at a held gun | what you hear, see, or cannot do — only a human holding the gun can name these | fn 1 (plain damage) and fn 10/11 (heals) as the two known ends |
| D7 | **1.6** the "KotH rate-of-fire buff" — **not a separate test**: nothing in the repo defines this as a feature (no code in `modes/driver.py` or `compile.py` implements one). It is D6 with an added step | hold the trigger through each of fn 31/32/34 landing | `$ALCD` frame interval vs a stopwatch | the un-buffed AR cadence measured immediately before and after each fn |
