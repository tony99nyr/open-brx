...
| D6 | **0.5 U11′** the status functions — ⚠ **MOSTLY DONE (F73, 2026-09-10)**: enemy **8, 24, 25, 26, 27, 28** are swept and written up (fn 28 registers with NO sound, flash or vibration — the row to ship for beacons; fn 8 is silent but still flashes and buzzes; fn 24-27 fire a long grenade-ish clip). **Only enemy 35 and ally 31, 32, 34 are still open** | fire enemy 35 and ally 31, 32, 34 at a held gun — one row at a time, protocol verified per trial, 3 ft of separation (see the F73 method note) | what you hear, see, or cannot do — only a human holding the gun can name these | fn 1 (plain damage) and fn 10/11 (heals) as the two known ends |
