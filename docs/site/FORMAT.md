...
- A paragraph containing only images renders as a row of screenshots (one image is a wide shot, two
  or three are a row). Image paths are `/shots/<name>.jpg` (generated into `site/shots/` by `site/shots.mjs`,
  which the `site-shots` CI job re-runs whenever the UI source moves past them) or
