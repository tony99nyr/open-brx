...
`wrangler.toml`, and **a push to `main` deploys it**. The root `npm run build` is the Cloudflare
build command, so the pages are generated at deploy time rather than committed. `webapp/mc/` and
`webapp/download/` are hand-committed and the generator refuses to write into them.
