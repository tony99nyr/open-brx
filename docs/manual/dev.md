...
- **`energyShieldLoop` is a real loop.** It runs for as long as the shield is up and survives a `$PSET` rewrite. Callsign's stock `A10` is a ticking clip, so it plays under every shield-band hit. Refuted 2026-09-25: `$PLAYX,0,*` does not stop it while the shield is up. Only the shield reaching zero stops it, and it can restart after a delay of 15 seconds or more once the shield is up again. Treat it as blocking the audio queue for the rest of the shield's life, not just until the next `$PLAYX`.
