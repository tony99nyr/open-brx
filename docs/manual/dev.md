...
| `$STOP,*` | >> | n/a | Stop. First frame the official app sends on every (re)connect; also part of the end-of-game tail. Whether `$STOP` also disarms IR reception is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.1 (levers sheet §12, claim 13). |
...
| `$START,*` | >> | n/a | Begin the configuration sequence. Gun echoes `$LCD,0,0,0,0,0,0,*`. Whether `$START` also arms IR reception is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.1 (levers sheet §12, claim 13). |
...
| `$PSET,…,*` | >> | id, team, HP, armor, shield, crit bonus, deathAlarm, voice-pack… | Player settings: **token 1 = player id (0-63)**, token 2 = team (an unconfirmed firmware reading, see the `$PSET` table), tokens 3-5 = HP/armor/shield pools, token 6 = crit damage bonus (Battle Company's sheet name), then a 17-sound voice pack at t7-t23. See the `$GSET` and `$PSET` section. |
...
| `$VERSION,*` | >> | n/a | Query firmware. Reply `$VERSION,v4.32,?,4,,devhost.03,*`. Token 2 is the **headset** firmware (`hds.59`) when a headset is linked. The meaning of tokens 3-5 is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.1 (levers sheet §18, claim 19). |
...
| `$QUERY,*` | >> | n/a | Over BLE returns a `$`-framed status array (`$QUERY,0,0,0,0,0,,1,0,,0,…`) plus a `$LCD`. **Not** the USB device record (that is USB-only; see the USB serial console section). The field map is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.1 (levers sheet §18, claim 19). |
...
| `$BUMP,<amount>,…,*` | >> | amount, then pool flags | **Live for the armour flag.** Callsign's shield recharge sends `$BUMP,12,,1,,,*`, and each frame adds 12 to the armour pool up to its maximum (capture, 2026-09-18). So token 3 is the armour flag. The rest of the shape (the hp and shield flags, negative amounts, a cascade across pools, the sound token) is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.2 (levers sheet §5, claim 5). `$LIFE` also works. |
| `$BHIT,<bulletType>,<playerId>,<team>,<damage>,<isCrit>,<powerLevel>,<irDirection>,*` | >> | 7 fields | A host-injected hit. The app's metadata gave the 7-field layout on 2026-09-04, and Battle Company's own sheet gives the same order: BulletType 0-15, PlayerID 0-63, TeamType 0-3, Damage 0-255, IsCriticalShot, PowerLevel 0-3, IRDirection. **Never sent in this shape on v4.32.** The four shapes tried earlier all used a 3-token guess, and each was echoed and applied no damage. What the gun does with the real shape is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.2 (levers sheet §3, claim 3). |
...
| `$IRTX,<Direction>,<BulletType>,<PlayerId>,<ImmuneTeamColor>,<Damage>,<IsCriticalShot>,<PowerLevel>,<IrRange>,<ToggleIRLoop>,<TimeFireLoop>,<FlashLED>,*` | >> | 11 fields | Raw IR transmit, named in Battle Company's own sheet. Direction 0 = front, 100 = all. Field 4 is **not** the shooter's team; BC's own note says the gun treats team 2 as the broadcast team. ToggleIRLoop: 0 = no loop, 1-99 = fire that many times, 100 = loop; TimeFireLoop = ms between pulses. Whether the gun relays the frame to the headset to emit is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.2 (levers sheet §16, claim 17). The 2018 BC app sent `$IRTX,0,14,<pid>,<team>,1,0,0,<range>,1,,1,*` once a second as a revive beam. A sample from BC's sheet: `$IRTX,100,15,63,0,6,1,0,100,100,5000,1,*`. Untested on v4.32; the earlier 4-field guesses emitted zero IR against a passing receiver control. |
| `$MELEE,<intensity>,*` | >> | intensity | `$MELEE,255,*` returns `$BUT,4,0,*` and fires no IR. The reply may be a coincidence: whether the gun has a `$MELEE` handler at all is an unconfirmed firmware reading (see `protocol/brx-protocol.md` §3.2). The firmware-levers bench sheet §2 runs a control (`$XYZZY,255,*`) to check. |
| `$STUN,<ms>,*` | >> | one token | A bare `$STUN,*` showed no effect on v4.32. The 2018 BC app sent `$STUN,2000,*`. Whether the token is a stun time in milliseconds is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.2 (levers sheet §4, claim 4). |
...
| `$VERSION,<gun fw>,<headset fw>,<n>,,<host image>,*` | e.g. `v4.32,?,4,,devhost.03` | Version reply; token 2 reads `hds.59` with a headset linked, `?` otherwise. `devhost.*` = developer/host image. Tokens 3-5: an unconfirmed firmware reading, see `protocol/brx-protocol.md` §3.1. |
...
| `$QUERY,…` | ~11 `value,,` pairs | Status array in reply to BLE `$QUERY,*`. The field map is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.1. |
...
5. **A dead gun ignores all incoming IR**: 448 distinct words, including every grenade-beacon shape, failed to revive one. Only the host can. The 2018 BC app's revive beam is fn 34 on protocol 14, counted by the phone. Our 448-word test had no fn 34 or fn 35 row in the table. Whether those two functions register on a dead gun is an unconfirmed firmware reading (see `protocol/brx-protocol.md` §5, levers sheet §10, claim 11).
...
| 1 | slotType / mode flag (BC sheet: `WeaponIRSource`) | n/a | n/a | `2` on exactly the three weapons carrying an extra-headset payload (t12/t13/t42 populated); `1` on melee; `0` rail gun; empty otherwise. Not fire mode. Battle Company's own sheet names this field `WeaponIRSource`, an enum of 0 GunLaser, 1 HeadSetOnly, 2 GunAndHead, 3 DoubleGun, 4 DoubleGunAndHead, 5 DRY_FIRE. Every stock melee row ships 1 (HeadSetOnly). What the fire path does with each value is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §6 (levers sheet §2, claim 2). |
| 2 | **gunRangeOutdoor** | 100 | 100 | **The range control (bench-proven 2026-09-17).** The range result below is measured. Why it works (a firmware reading says the token sets the IR carrier frequency, not the power) is unconfirmed: see `protocol/brx-protocol.md` §6 (levers sheet §20, claim 21). Every captured gun reads 100 here (melee 90). A garden ladder test found a floor below which nothing lands at any distance (5 landed 0 of 38 shots, including muzzle on the dome), a transition roughly 13-26, and a flat shelf from about 31 to 100 where every value behaved alike at any distance the test could pace. Whether it can fence a weapon to a chosen distance above the shelf is unresolved; the ladder was shot into a dome in direct sun and needs a shaded re-run. This corrects the earlier reading that named t41 as the range lever: t41 was tested the same day and found inert outdoors. Open BRX ships a per-weapon starting value outdoors from this token (docs/weapon-design.md §4.2) and never compiles a value under 13. |
...
| 3 | gunLaserRegion | 1 | APK name only. Its effect is untested on the bench. A firmware reading says the gun stores it and never reads it (unconfirmed, see `protocol/brx-protocol.md` §3.1 and levers sheet §20). |
...
| 2 | **team** | 0 | Our earlier reading, "0/1/7 gave byte-identical behaviour, inert", was measured with `$TID` sent *after* the `$PSET`, so it could not show whether this token writes the team: `$TID` was the last writer in every trial. That it does (one team byte, the last writer wins) is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §3.1 (levers sheet §1, claim 1). |
...
| 6 | **CriticalDamageBonus** | 50 | Field name from Battle Company's own sheet. Shipped as 50. The bench measured a crit as x1.5 with `$GSET` t7 at 0 (2026-09-18). Whether this token is the source of that x1.5 is an unconfirmed firmware reading: see `protocol/brx-protocol.md` §5 (levers sheet §9, claim 10). The 2026-09-07 sweep of this token had no crits in it, so it could not see a crit effect. |
| 7 | **deathAlarm** | (empty) | Empty in every Callsign capture; the 2018 BC app sends `NA0` here. This is the first slot of a **seventeen**-sound voice pack that runs t7-t23, not sixteen from t8: Battle Company's own sheet names 17 fields at t7-t23 (a firmware reading agrees, see `protocol/brx-protocol.md` §3.1). |
...
A firmware reading maps more function numbers than the bench has. That unconfirmed reading, with its evidence levels, is in `protocol/brx-protocol.md` §5, and the firmware-levers bench sheet §8 and §10 test it (claims 9 and 11).
...
- **Dead guns accept no IR at all** over every row we have tested. Whether fn 34 and fn 35 register on a dead gun is an unconfirmed firmware reading (see `protocol/brx-protocol.md` §5, levers sheet §10).
...
No, over the documented rows. A dead gun ignores all IR; stations *arm* a living tagger's respawn path. The 2018 BC app's revive beam used `$SIR` fn 34 on protocol 14. Whether fn 34 and fn 35 register on a dead gun is an unconfirmed firmware reading (see `protocol/brx-protocol.md` §5, levers sheet §10).
...
A firmware reading lists more `SETUP` menu options than the ones above; it is unconfirmed and is in `protocol/brx-protocol.md` §9. We do not record any code, PIN or unlock value here.
