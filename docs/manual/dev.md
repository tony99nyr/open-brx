...
| `$BUMP,<amount>,<hp>,<armor>,<shield>,<sound>,*` | >> | amount, hp 0/1, armor 0/1, shield 0/1, sound | **Live for the armour flag.** The firmware's shape is `$BUMP,<amount>,<hp 0/1>,<armor 0/1>,<shield 0/1>,<sound>,*` (the V4_30 firmware, Battle Company's own sheet and the 2018 BC app all agree): tokens 2-4 pick which pools take part. Callsign's shield recharge sends `$BUMP,12,,1,,,*`, and each frame adds 12 to the armour pool up to its maximum (capture, 2026-09-18). That confirms token 3 as the armour flag. Our earlier probes (`$BUMP,-5,0,0,*` and `$BUMP,0,5,0,*`, 2026-09-09) set no pool flag, so they asked for nothing. From the firmware, not yet measured: a negative amount drains shield, then armour, then HP, and kills at HP 0; a positive amount heals HP, then spills into armour, then shield. The 2018 app used `$BUMP,<n>,1,,,N94,*` for out-of-bounds damage. The hp and shield flags, negatives and the cascade are not yet verified on v4.32 (firmware-levers bench sheet §5). `$LIFE` also works. |
| `$BHIT,<bulletType>,<playerId>,<team>,<damage>,<isCrit>,<powerLevel>,<irDirection>,*` | >> | 7 fields | A host-injected hit that runs the firmware's own hit path. The app's metadata gave the 7-field layout on 2026-09-04, and Battle Company's own sheet and the V4_30 firmware agree: BulletType 0-15, PlayerID 0-63, TeamType 0-3, Damage 0-255, IsCriticalShot, PowerLevel 0-3 (the IR word's subtype), IRDirection (the sensor). In the firmware it runs the same hit handler as a received IR word, and a dead gun drops it. **Never sent in this shape on v4.32** (firmware-levers bench sheet §3). The four shapes tried earlier all used a 3-token guess, and each was echoed and applied no damage. |
