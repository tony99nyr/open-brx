...
> **Credit, first.** The BRX serial protocol was discovered and proven by **LaserTagMods** (the JEDGE / JBOX / NRFL-Bases projects, github.com/LaserTagMods). Everything on these pages is an independent, clean-room restatement. We verified it on our own taggers, decoded it from public app metadata, or captured it off the air. It contains no Battle Company code or assets. If you build on this, credit them too.
...
- **What bounds a custom weapon?** The firmware's behaviour vocabulary: the DamageType and PowerType enums, ReloadType, six slots, and the 2,477 on-gun sound ids. Any *combination* with arbitrary numbers is buildable; a brand-new damage *behaviour* is not. 🔍 src: protocol/callsign-extract/protocol-classes.md ("What's moddable")
...
[diagram DEV-07] Damage pipeline: IR word (B,U,D,C) → victim `$SIR[B,U]` → function multiplier → ×(1 + `$GSET` t7/100) if crit → drain shields → armor → HP → emit `$HIR` + `$HP`. ✅ src: below
...
- **Which protocols are free?** Stock uses 0, 8, 10, 11, 13 and 15 (grenade beacon); the app's table also ships rows on 1, 2, 3, 6, 9. Truly unused: 4, 5, 7, 12, 14. Every cell is still re-definable per game, since you push the table. ✅ src: protocol/brx-ir-protocol.md

---

### Page: Events: what the gun tells you  (`/manual/dev/events`)
_Hits, health, HUD echoes, buttons and telemetry, plus the proof that the gun keeps no game state. Read this page to learn what the gun sends back to your code._
...
- **`$BRXSERVER` `$SSID` `$PASS` `$BRX`**: together they suggest a Wi-Fi/server mode in some firmware; unprobed.
