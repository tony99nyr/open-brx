...
| `$GLED,<led1>,<led2>,<led3>,<t4>,<brightness>,,*` | >> | three LED colours | **Gun LED colour, per LED.** Tokens 1 to 3 are the three body LEDs, each a direct palette index: **0 red, 1 blue, 2 yellow, 3 green, 4 purple, 5 teal, 6 white**. **Token 4 = 3 blanks all three** (night mode); Callsign sends `$GLED,,,,5,,,*` on death, which also blanks. Token 5 is brightness. On a spawned gun the set colour alternates with the team colour, because a spawned gun is also using these LEDs as its own health gauge. Verified on the bench 2026-08-30: a gun held on team 1 took six different colours on command, and `$GLED,3,2,1,0,10` was predicted and confirmed as green, yellow, blue. | ✅ |
