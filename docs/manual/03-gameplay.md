...
[callout:warn] **A dead gun is deaf.** At 0 health the tagger takes no IR at all. A respawn station cannot revive you with a beam; it arms the living. Pull the trigger while dead and all you get is the dead or out-of-ammo noise. ✅ `src: docs/reference/grenade.md §Can we add new modes, protocol/brx-protocol.md §7r (Resync)`
...
- **Hits-to-kill for non-standard damage types (5 weapons).** The captured 10-row `$SIR` effect table maps subtype 1 → fn 36 and subtype 3 → fn 37. **Those multipliers (×1.25 / ×2) are DISPUTED.** They did not reproduce in 24 controlled cells, so the hits-to-kill they imply cannot be published; the Energy Launcher's `<9,3>` maps to fn 24, a status function that changes no pool (0 damage, 3/3 trials). Whether the Callsign app pushes this same table in every game is not established, and neither is what Burst Rifle, Force Rifle, Bolt Rifle and AMR actually take to kill, nor whether the Energy Launcher kills at all in stock play. *Removed:* the plain-damage hits-to-kill cells for Burst (13), Force (13), Bolt (9), AMR (7), Energy Launcher (1) and the Energy Launcher "one-shot kill"; the Sniper Rifle stays at 2 because 80 and 100 both need two hits. `src: docs/weapon-design.md §0 + §6.2, mcp/brx_mcp/gameconfig.py _SIR_TABLE, docs/experiment-log.md 2026-08-26`
