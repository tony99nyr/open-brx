...
| Head shots? | The headset carries two sensors (front and back domes) and the gun body a third; a crit flag exists in every shot but no stock weapon sets it. A crit multiplies damage by `1 + $GSET t7/100` — a per-game setting, ×1.5 at the shipped t7=50, and t7=0 disables crits entirely. | ✅ |
