...
| Shotgun | cqb | 21 | 800 | 3 | **1.60** | 26.2 | 24.2 | 6 | 24 | 400 | 12% | — | **2026-09-18**: dmg 45→21 (`wire.dmg`, the gun word); OUR chosen 19-damage headset word (`wire.headset_dmg`, t12) stacks unconditionally on top, 40 real per pull. **F276 keeps the two words UNEQUAL (21 + 19, not 20 + 20):** the receiving gun may DROP a word identical to one it took within 159 ms, and the pair arrive about 88 ms apart carrying the same `$SIR` key, so equal damage could silently cost the second word entirely. The sum is what the ladder publishes; the second word itself is measured (Callsign's own 70, cap30), the split is a balance number we picked (htk/TTK unchanged, 2 pulls short, 3 kills either way), but `dmg`/`dps`/`sust`/one-mag % here are the GUN WORD ALONE, not the real per-pull total. Magazine deliberately left at 6/24: the AMR now covers it on every axis, and rather than buy a lead with an unexamined number, `test_ttk_band_and_no_strictly_dominant_weapon` names the `(amr, shotgun)` pair in `KNOWN_DOMINANCE`. The suite is GREEN, not red, and the exemption fails the moment the pair stops dominating, so it cannot outlive F275; §7 |
