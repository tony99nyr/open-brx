...
3. **Lock-up.** Replay the live-accuracy writer's pattern (a `$WEAP` plus an `$AMMO` every 250 ms) to B for up to
...
the live-accuracy writer (S42) must be throttled before the next playtest.
