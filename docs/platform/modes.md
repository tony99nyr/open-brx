...
- A phone can land in utility mode by accident (an idle player phone reads seven taps and a hold as the switch). If nobody has armed it as a station yet, a HOLD TO EXIT button on its screen takes it straight back to being your HUD, about a one second press. Once Mission Control has armed it as a real station, that button is hidden on purpose, and the operator brings it back with RELEASE on the items panel instead.
