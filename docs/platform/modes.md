...
- Mission Control arms utility phones from the muster items panel: a spare phone can be armed as a respawn station or a control point before the match. Dedicated station hardware (an M5StickS3) is on the bench and not yet proven in a match, and the Smart Grenade is set by its own button, not by Mission Control.
