...
| **0.3** | **Does the SENSOR STRUCK change the applied function?** The single cheapest test for the two results that would not reproduce (fn 36/37 x2, and fn 24 damaging on protocol 7). **All 18 of my hits landed on `$HIR` tok1 = 4, the gun body**; a held gun is struck at a different angle | Fire the **same word** twice: once at the **headset dome** (expect `$HIR,0` or `,1`), once at the **gun body** (`$HIR,4`). Use fn 24 on protocol 7, magnitude 20 x2, and read the `$HP` delta | different pool delta between sensors ⇒ **both anomalies explained by one mechanism**, and the function map needs a sensor qualifier · identical ⇒ sensor is ruled out and the cause is elsewhere (different gun, or gun state) |
