...
def test_put_keeps_stun_and_the_compiled_head_carries_the_emp_row():
    ...
    assert any(f.startswith("$SIR,8,0,,24,") for f in b["sir_pool"][0]), b["sir_pool"]   # F209: the live table is the sir_pool take
    ...
    assert not any(f.startswith("$SIR,8,0,,24,") for f in s._compile_rolled(p)["sir_pool"][0])
    ...
    assert not any(f.startswith("$SIR,8,0,,24,") for f in t._compile_rolled(next(iter(t.players.values())))["sir_pool"][0])
