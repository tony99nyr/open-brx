...
Mission Control will send; `app/src/transport/envelope.js` is what the phone node will ACCEPT. A kind
missing from the node's set is not a loud error: `validate()` throws `unknown_kind` and the message is
dropped before `onMessage` ever runs, so the feature simply never happens. That has now bitten twice
for real -- `alert` (A11.4, unlisted at the node until 2026-09-07: every alert MC sent was discarded)
and `station_config` (A13.5/F104: MC's arming message for a utility phone, same silent drop).
...
ENVELOPE_JS = REPO / "app" / "src" / "transport" / "envelope.js"
...
def _compare(direction: str, spec: list[str], py: set[str], js: set[str]) -> None:
    ...
    for kind in sorted(spec_set):
        if kind in PENDING:
            continue
        where = [n for n, s in (("mc/types.py", py), ("app/src/transport/envelope.js", js)) if kind not in s]
        if where:
    ...
    for kind in sorted(js - spec_set):
        problems.append(f"{kind!r} is in envelope.js {direction} but nowhere in contracts.md §5")
    assert not problems, (
