...
tests below still pin the FORMULA at t7=50 -- a real, still-reachable value, just no longer the
default -- and at the new default t7=0.
"""
from brx_mcp.mc.compile import headset_multiplier


def test_fn36_at_crit_modifier_50_matches_the_2026_09_02_reading():
    # t7=50 (the MC-compiled default until 2026-09-17): fn 36 -> 1.25, matching the earlier flat reading exactly.
    assert headset_multiplier(36, 50) == 1.25


def test_fn37_at_crit_modifier_50_matches_the_2026_09_02_reading():
    # t7=50 (the MC-compiled default until 2026-09-17): fn 37 -> 2.0, matching the earlier flat reading exactly.
...
def test_fn37_at_the_closing_control_crit_modifier_zero_is_unscaled():
    ...
    # headset, isolating t7 (not the function alone) as what drives the scale. t7=0 is also the
    # MC-compiled default since 2026-09-17.
