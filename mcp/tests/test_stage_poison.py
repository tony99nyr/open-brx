...
class Bench:
    ...
    async def set_pools(self, hp: int, armor: int, shield: int) -> "Bench":
        """Steer the gun's pools directly, and lift its own ceilings to match. The fake gun clamps every
        `$LIFE` to its `$PSET` caps (`fake.py`, the LIFE branch), and the default head is the Standard life
        preset, whose shield ceiling is 0 (S45, merge 2026-09-19). Without the lift, a shield injected here
