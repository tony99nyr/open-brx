...
Regression for the 2026-09-02 bench find: ALPHA was advertising as `ALPHA-3D4F-3D4F` because a
...
def test_strips_the_tail_this_address_actually_appends():
    assert strip_advert_tail("ALPHA-3D4F", ADDR) == ("ALPHA", True)


def test_repairs_an_already_doubled_name():
    # the exact string found on the bench
    assert strip_advert_tail("ALPHA-3D4F-3D4F", ADDR) == ("ALPHA", True)
...
def test_leaves_a_bare_name_alone():
    assert strip_advert_tail("ALPHA", ADDR) == ("ALPHA", False)
...
def test_macos_uuid_address_has_no_mac_tail_to_match():
    ...
    assert strip_advert_tail("ALPHA", uuid) == ("ALPHA", False)
...
def test_doubled_name_survives_truncation_and_is_repaired():
    # "ALPHA-3D4F-3D4F" is 15 chars; CALLSIGN_MAX truncation must not get there first
    assert name_for_rename("ALPHA-3D4F-3D4F", ADDR) == ("ALPHA", True)


def test_single_tail_is_repaired_too():
    assert name_for_rename("ALPHA-3D4F", ADDR) == ("ALPHA", True)


def test_the_wrong_name_we_actually_wrote_is_repaired():
    # what the ordering bug put on the gun, fed back in
    assert name_for_rename("ALPHA-3D4F-3", ADDR)[0] == "ALPHA-3D4F-3"


def test_plain_name_passes_through():
    assert name_for_rename("ALPHA", ADDR) == ("ALPHA", False)
