...
async def send(alias: str, command: str, confirm: bool = False,
               reply_window_ms: int = 500, allow_hang: bool = False) -> dict[str, Any]:
    ...
    list from brx-protocol.md require confirm=true. Denied commands (persistent
    state, pairing, DFU, factory tests) are refused whatever you pass; a
    hang-prone one ($DPLAY) goes only with confirm=true AND allow_hang=true,
    for a supervised bench run that wants a lock-up on demand."""
    err = protocol.validate_frame(command)
    if err:
        return {"error": f"malformed frame: {err}", "command": command}
    denied = protocol.deny_reason(command, allow_hang=bool(confirm and allow_hang))
    ...
    if protocol.command_name(command) in protocol.HANG_PRONE_COMMANDS:
        notes.append("HANG-PRONE frame sent on allow_hang=true: if the gun stops answering $PING, power-cycle it")
    if notes:
