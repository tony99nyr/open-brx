        ...
        if kind == "ack":
            ...
            if cmd in ("end", "recall", "panic"):
                ...
                self.alive = False
                  ...
                  and body.get("player_id") == self.player_id):
                # A47: the demo phone's operator actions (engine.js `_operator`), answered with the
                # `operator_result` fact MC writes the feed line from.
                if cmd != "relink" and self.arm_state != "live":
                    self.emit({"type": "operator_result", "cmd": cmd, "ok": False, "why": "not live"})
                else:
                    if cmd == "respawn":
                        self.alive = True
                        self.emit({"type": "respawn", "operator": True})
                    self.emit({"type": "operator_result", "cmd": cmd, "ok": True})
