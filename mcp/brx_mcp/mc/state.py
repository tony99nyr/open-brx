    ...
    def readiness(self) -> ReadinessSnapshot:
        ...
        for p in self.players.values():
            ...
            row.update({"battery_pct": nv.get("battery"), "battery_age_ms": (now - nv.get("last_seen_ms", now)) if nid else None,
                        ...
                        # A28.3/F144: the path MC is reaching this phone over right now, and the last one
                        # it was heard on. The Armory card renders the first as a tag and the second is
                        # what makes an unreachable row's reason honest (F155).
