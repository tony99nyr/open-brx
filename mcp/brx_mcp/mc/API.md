...
| `PUT /api/stations/{node_id}` | `{kind, team, id, threshold?}` → `StationView`. `kind` ∈ respawn · powerup · extraction · bomb · control; `team` = a `team_id`, a `$TID` 0-3, or `"any"` (255 — required for `control`, which starts NEUTRAL and is taken by presence); `id` 1..65535, unique on the field; `threshold` dBm −100..−30 (default −74 ≈ 10 ft at high TX). Pushes `station_config` `{kind, team, id, threshold, game, valid_ids}` to that phone at once and re-arms every other assigned station (the allow-list they echo changed); an offline phone is flagged and armed on its next hello. 400 in the operator's voice | muster/build/kit |
| `DELETE /api/stations/{node_id}` | → `{ok}`; drops the assignment (the survivors' `valid_ids` shrink). 404 for an unknown station | any |
