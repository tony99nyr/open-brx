...
| `POST /api/stations/unlock` | → `{ok, armed, pending: node_id[]}`; A58: `lock_s: 0` to every assigned station now (a muster station out of Wi-Fi hears it on its next hello). The next LOAD push or START locks them again | any |
