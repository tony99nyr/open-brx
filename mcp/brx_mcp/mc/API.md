...
State {
  ...
  lobby: { ready: number, total: number, pushed: boolean, all_acked: boolean,
           acks: { [player_id]: { ok: boolean, gun_echo?: string, err?: string, config_id?: string } } },
                                              ...
                                              // refuses even with `force`. Anything counting "acked" must ask the
...
| `POST /api/players/{id}/standby` | → `Player` (parked). **STANDBY (2026-09-12):** the player leaves the roster but is not forgotten — the record (callsign, team, gun, loadout, voice, `player_num`) parks under the snapshot's `standby: Player[]` with `node_id: null`, `ready: false`. Their phone is unbound and drops back to a plain connected node, so the readiness board, the kit/lobby counts and the next push no longer wait on them. 404 for an unknown id; 400 once the match is `armed`/`live`. **v1 gap:** nothing is pushed to the phone — an unbound node receives no frames, so its HUD keeps the last `assign` it was handed until PLAY or the next kit | ≤ lobby |
...
- **RECAP → NEXT MATCH is a deterministic reset (A36).** `POST /api/session/new` clears the acks, the
