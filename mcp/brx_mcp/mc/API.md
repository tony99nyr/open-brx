...
| `POST /api/session/new` | `{keep_roster?: boolean}` → `State` (back to muster). **A43:** from `recap` with `keep_roster` (NEW SESSION) the finished match keeps its late facts and its A42 end watch; `keep_roster: false` (FRESH SESSION) drops both | recap |
