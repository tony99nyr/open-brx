...
| `--bench-volume [N]` | bench run only: every `$VOL` MC compiles (match heads at any venue, try-outs) plays at N, default 55, 0-100. Without it, heads use the venue volume (80 indoors, 90 outdoors) and try-outs 69. The banner says `BENCH VOLUME N: not for a real game`, the state carries `bench_volume`, and the console header shows a BENCH VOL tag. `npm run mc -- --bench-volume` forwards it. |
