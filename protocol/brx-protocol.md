...
| `$BUMP,<hp>,<armor>,<shields>,*` | Adjust current pools — **INERT on v4.32** | **Bench 2026-09-09: it does nothing, in either direction.** `$BUMP,-5,0,0,*` on full HP and `$BUMP,0,5,0,*` on armour depleted to 61 both left the pools untouched, with a validated read either side (a real IR hit moved them, and `$QUERY`'s `$LCD` tracked it). The "same additive / clamped behaviour" this row used to claim was inherited from `$LIFE` and never measured. **Use `$LIFE`**, which does work. Whether `$BUMP` wants a different arity or a game state we did not have is open (F65) |
...
| **Delayed blast** | 24 | Registers on arrival (plays the long grenade-ish clip below, no IMMEDIATE pool change) then, **~4 s later, applies 1–3 damage ticks equal to the ORIGINAL word's magnitude**, each reported as its own ordinary `proto=0` `$HIR` carrying the original word's team. Bench 2026-09-11: a live `<15,0>` grenade beacon (mag 8, ~5 s period) produced 1–3 `$HIR,0,0,0,2,8,0,0` ticks ~420 ms apart per beacon, ~4 s after it, 8 armour each, for as long as the beacon kept arriving — zero further ticks in 60+ s once the source stopped (no self-replay). **Corrects the "no pool change" row below, which fn 24 was wrongly filed under until this bench session.** 25/26/27 share fn 24's clip (F73, bench 2026-09-10) and are presumed the same delayed-tick family but untested for the tick itself | enemy (measured on the grenade's words); the tick carries the triggering word's own team, so an ally-polarity trigger would presumably tick as an ally effect too — untested |
| Registers, no pool change | enemy 8, 25, 26, 27, 28, 35 · ally 31, 32, 34 | `$HIR` fires, pools unchanged **at the cell itself** — see fn 24 above for a function in this family that is NOT actually inert. ⭐ **They differ in PLAYER FEEDBACK, bench-swept 2026-09-10** (free cell, one row only, `$HIR` protocol verified per trial): **fn 28 registers with NOTHING — no sound, no headset flash, no vibration**, so a host can read an IR event the player never perceives (the row to ship for beacons). **fn 8** is silent but still flashes and vibrates. **fn 25/26/27** fire a long grenade-ish clip (hiss → timer → explosion; the "varied sounds" reported are that one clip truncated by the next event) — ⚠ the SOUND is measured; whether they also flash and vibrate was never observed and is marked `?` in the source tables, so do not read "additionally" into it; whether they also carry fn 24's delayed tick is UNTESTED. fn 35 and the ally trio unswept. **Polarity measured both ways on fn 28:** enemy-only under `$GSET` t1=0 (ally words silently rejected, 0/3), and with **t1=1 it registers ally words too** with the owner in `$HIR` token 4 — which is how a host reads who holds a control point | n/a |
...
| tok | Field | Proof |
|---|---|---|
| 2 | **gunRangeOutdoor = the emitted-power control** | ⭐ **BENCH-PROVEN 2026-09-17 (F231/F234)**, garden ladder, two guns, both in physical outdoor mode. Every captured gun reads **100** here and melee reads 90. t2 = 5 landed **0 hits from 38 shots**, muzzle on the dome included; 13 to 26 is a real attenuation band (13 gave 3 of 8 at 15 m on precise aim); about 31 to 100 is a flat shelf that behaved alike at every distance the garden could pace. ⚠ Whether t2 can fence a weapon at a chosen distance is **UNRESOLVED**: the whole ladder was shot into a dome in direct sun with the light moving, so every sub-30 number is a lower bound and the run needs repeating with the dome shaded. Below 26 the groups stopped being monotonic, so the values there cannot be separated. MC writes t2 only at an outdoor venue and refuses a value under 13 (`compile.gun_range_outdoor_pct`) |
...
| 41 | gunRangeIndoor: **a measured null outdoors** | 2026-09-17 (F231), same garden session as t2. Two slots differing only in t41, 5 against 75: the low slot fired 27 rounds and landed **27 hits** from 3 m to about 200 ft, the stock slot landed 55 of 57. t41 reads 75 on every gun and 20 on melee, and it changes nothing an outdoor game can measure. Indoor behaviour is still untested. Open BRX passes it through byte-for-byte from the capture |

Unknown or unverified: t19 (not simply reloadType), t25/t26 (Suppressor only), t6 crit chance.
(**t21/t22 accuracy left this list 2026-09-09**, and **t2 and t41 on 2026-09-17** — bench-proven, see the
`$WEAP` table.)
