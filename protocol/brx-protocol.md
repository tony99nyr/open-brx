...
| `$FIREX,<slot>,*` | Fire a slot with no trigger pull `[disasm]` | Stores a slot byte 0–11 (the byte `$LCD` token 4 shows) and calls the fire/select routine, so the slot's real IR word should leave (inferred, not traced to the emitter). The control for a melee that will not swing: levers sheet §2 step 5 |
