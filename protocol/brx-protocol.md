...
| `$GLED,...` | Set gun LED — fields `mid, effect, optionA, optionB` (from APK, `callsign-extract/protocol-classes.md`); `effect` = LedEffect enum (Solid/Glow/ChaseBack/ChaseForward/StopIR) | The old `<r>,<g>,<b>` reading is **disproven** (§7i) — but ⚠️ **CORRECTED 2026-08-30: `$GLED` token 1 IS a COLOUR INDEX and overrides the team colour.** Bench-swept on a gun held at `$TID,1` (blue): **0 = off · 1 blue · 2 yellow · 3 green · 4 purple · 5 teal · 6 white**, and 7-8 are two further distinct colours (observed changing, not individually named) — so **at least eight colours plus off**; whether the palette extends past 8 is untested. Token 2 is the `LedEffect` (effect 1 = Glow, visible brightness modulation). So colour is a shared palette that `$GLED` addresses directly and `$TID` also selects from — it is an index, not RGB. ⚠️ Which of the three LEDs is targeted depends on the trailing tokens, which are NOT yet isolated: this sweep moved only LED 1, while `$GLED,1,<eff>,1,0,10` moved the MIDDLE LED to green. e.g. `$GLED,1,1,1,0,10,,*` |
