...
| `$GSET,<t1>,…,<t8>,*` | Global game settings, 8 tokens | `friendlyFire, outdoorMode, gunLaserRegion, autoAmbientLight, gyroscope, secondaryBluetoothWeapons, criticalShotModifier, gameMods`. Captured `$GSET,0,0,1,0,1,0,50,1,*`. **Token 1 is firmware-enforced both ways**: 0 blocks same-team damage and enemy heals, 1 opens the gate. **Token 7 is the crit modifier in percent, and it is not a generic "crit" — it is the HEADSET-sensor multiplier on `$SIR` fn 36/37 rows only** (bench 2026-09-11, §5): fn 36 = magnitude × (1 + t7/200), fn 37 = magnitude × (1 + 2·t7/100); every other function, and every gun-body hit, is unaffected by t7. The `$HIR` crit bit (token 6) is a separate, unset axis. 0 disables the fn 36/37 scaling, 100 triples fn 37 (×3; fn 36 → ×1.5); the shipped/MC-compiled 50 is why the two functions read as a fixed ×1.25/×2. **No respawn, time, lives or score token**: those live in the host (§7). Tokens 2–6 and 8 carry the APK field names only; none has been flipped on the bench. **Tokens 2 and 3 are the two candidates for the venue's IR range** (t2 `outdoorMode` is the APK's name for the on-gun ALT-hold toggle, which persists across power cycles; t3 `gunLaserRegion` is a USA/International power profile and the only field that reads as a direct power control). MC sends t2 from the venue in every head and t3 as the captured `1` at both venues. ⚠️ **Whether either moves emitted IR range, and whether t2 is even the same persisted bit as the ALT hold, is UNMEASURED** — FOLLOWUPS **F162**, Runs C and D of the 2026-09-12 range entry in `docs/experiment-log/2026-09.md`; the t3 candidate is staged no-op behind `mc/compile.py`'s `DRIVE_IO_MODE` |
