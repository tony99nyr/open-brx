...
| `$TMP,<t1>,…,<t11>,*` | **Temporary modifier set** `[disasm]` | Each token optional (empty leaves it alone). t1/t2/t3 = HP / armour / shield **maximum bonus** (added to `$PSET` t3–t5 in the clamps; a change that leaves HP ≤ 0 kills); t4 = accuracy modifier (fn 23 writes −100 here); t8 = incoming damage % (`damage × (100 + t8) / 100`; `$INVU` writes −100); t9 = magazine % bonus (every slot's magazine += clip × t9 / 100); t11 = the default hit sound when a row has none; t5/t6/t7/t10 stored, use not traced. A native perk system. **Bench-confirmed 2026-09-18 for t4, t8 and t9 (levers sheet §21), none of them resetting the magazine or reserve on their own:** t4 sets `$ALCD` accuracy as an ABSOLUTE value at once, with no walk-back, and a `$WEAP` push does not clear it; t8 = -50 took 4 off a 9-damage hit, t8 = -100 took 0 (spawn protection with no `$SIR` table); t9's top-up is RAW ROUNDS (clip × t9 / 100) and a re-send adds again rather than replacing. `$SPAWN` zeroes every token, confirmed. ⚠️ **t4 is last-writer-wins against the gun's own fn 23 (smoke), and fn 23's own ~6 s timer resets t4 to 0 regardless of the last write** (bench-confirmed 2026-09-18): a host writer of t4 must not write while a smoke is active and must re-send its own value once the smoke ends |
