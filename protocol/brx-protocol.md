...
reconnect anyway: two 2026-08-26 readings of a bare `$SPAWN,,*` after a drop contradict each other, one reviving
the gun with its config intact and one not, and were never reconciled; the retractions table in
`session-findings-2026-08.md` has both); a power-cycle wipes everything except `$NAME`; switching the headset off makes
