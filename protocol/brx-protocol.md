...
**Status:** reference, current as of 2026-09-18. Each row states the current reading only. The dated
...
- The main loop reads **one byte per pass** from the radio UART (1 KB buffer). A burst faster than the loop drains
  fills the buffer; bytes past 1 KB are lost. The loop's pass time is not measured (levers sheet §14, §19).
...
- **Six audio waits block the loop** with `delay(10)` and no timeout, serial unread: `$DPLAY` (reachable over
  ...
  serial routine, so a blocked loop never recovers. `$DPLAY` is on the deny list (§3.3). Levers sheet §14.
...
| `$STOP,*` | Stop | First frame the app sends on every (re)connect; also part of the end-of-game tail. `[disasm]` **Disarms IR reception**: while the started flag is clear the receive routine drops every word ("not start"). Levers sheet §12 (F121) |
...
| `$LIFE,<hp>,<armor>,<shields>[,<mode>],*` | Grant **or drain** a pool | `[disasm]` **A 4th token is a mode**: 0 or empty = additive (what we send), 1 = absolute set, clamped at the maxima, 2 = absolute set with no clamp; a dead gun ignores `$LIFE` when token 1 is 0. The 2018 app revived a downed player with `$LIFE,30,0,0,1,*` and polled a silent gun with a bare `$LIFE,*` `[apk2018]`. ⚠️ BC's own sheet lists the tokens as **shields, armour, HP** `[sheet]`, the reverse of our bench-measured order (HP, armour, shield): our order stands (measured 2026-09-09 and 2026-09-11 on separate pools), but in set mode the wrong order would set HP to 0, so levers sheet §6 reads `$QUERY` before every set. **Bench-measured 2026-09-09** (first time either direction was ever measured; the old row was APK/spec-derived). Additive, clamped at the pool max, never an absolute set — and it **DOES self-emit `$HP`**, contrary to what this row used to say. ⭐ **It accepts NEGATIVES and drains**: `$LIFE,0,-5,0,*` took armour 66 → 61. A negative is **per-pool and floors at 0 with NO spill** — `$LIFE,0,-100,0,*` on armour 61 left armour 0 and HP untouched at 45, so it does **not** cascade shield → armour → HP the way IR damage does; a host applying damage must do its own pool arithmetic. ⚠️ **A lethal negative really kills** (headset green out-flash, trigger dead, `$BUT` with no `$ALCD`) but announces it with `$LCD,0,0,0,0,<mag>,<reserve>,*` and **never `$HP,0,0,0`** — see the `$HP` row |
| `$BUMP,<amount>,<hp 0/1>,<armour 0/1>,<shield 0/1>,<sound>,*` | Adjust current pools **as a cascade** | ⭐ **Our 3-token shape was inert, and the shape was wrong.** Bench 2026-09-09: `$BUMP,-5,0,0,*` on full HP and `$BUMP,0,5,0,*` on armour at 61 both left the pools untouched, with a validated read either side. `[disasm]` `[sheet]` `[apk2018]` The real shape is five fields (the V4_30 handler reads 5; BC's sheet header is `Amount, HP, Armor, Shields, Sound`; the 2018 app sent `$BUMP,<n>,1,,,N94,*` for zone damage and the negative of it to kill a player): tokens 2–4 are on/off flags that pick which pools take part, so our probes sent every flag as 0 and asked for nothing. A negative amount drains shield, then armour, then HP, and kills at HP 0; a positive amount heals HP, then spills into armour, then shield. That is the cascade `$LIFE` lacks (`$LIFE` floors each pool with no spill). Not yet verified on v4.32: levers sheet §5 (F65). Until then use `$LIFE` |
...
| `$BHIT,<bulletType>,<playerId>,<team>,<damage>,<isCrit>,<powerLevel>,<direction>,*` | Host-injected hit | `[disasm]` `[sheet]` Seven fields, and the two sources agree on the order (BC: `BulletType 0-15, PlayerID 0-63, TeamType 0-3, Damage 0-255, IsCriticalShot, PowerLevel 0-3, IRDirection`; the firmware: `proto, shooterId, team, damage, crit, subtype, sensor`, so PowerLevel = the word's subtype and Direction = the sensor). It calls the **same hit handler as a received IR word** (in app mode through the receive pre-filter first), so a host can apply any `$SIR` row, stun, fuse or splash with no IR; **a dead gun drops it**. Jay's ESP32 injects decoded IR into the gun as `$HIT,<proto>,<pid>,<team>,<dmg>,<crit>,<power>,0,*`, the headset's own frame with the same seven fields `[jay]`. Earlier probes used a 3-token guess and were echoed with no damage; **untested in the real shape**: levers sheet §3 |
...
`[disasm]` The V4_30 image dispatches **145 distinct `$` commands**; 85 of them are in no app capture. The names
...
word-format switch, `$IRLVL` and `$INDOOR` = the same variable as `$GSET` t2, `$VIBTOGGLE`, `$LIGHT`). Removed
...
the IR word format, runs a factory test, or blocks the main loop (`$DPLAY`), plus the USB console word `SETUP`.
The same list reaches the phone as `NODE_DENIED_COMMANDS` and the node drops such a frame at its one write path;
...
`allow_hang=true`, one frame, never a batch (levers sheet §14.4). Everything else needs `confirm=true`.
...
| `$DD,<killerId>,<killerTeam>,*` | **A death report from the gun** `[jay]` | Jay's ESP32 parses `$DD` FROM the gun (on BLE and on the SPP headset link) and builds its kill confirmation on it. Our doc called `$DD` a host-side convention; on v4.32 nobody has listened for it after a death. Levers sheet §13 |
...
on death, which Jay's code reads (§4 table, unverified on v4.32).
...
| 30 | normal damage, **doubled from sensor 1** (headset back); a kill from sensor 1 sets the silent-death flag (a backstab) | unmeasured; §10 |
...
| 36 / 37 | headset sensors 0–3 only: × (100 + t7/2)/100 and × (100 + 2·t7)/100; body sensor × 1 | ✓ exactly the measured formula |
...
| 39–49 | no-op | ✓ 39–45 |
| 50 / 51 / 52 | call the team-colour repaint only, no `$HIR`; 50 enemy, 51 ally, 52 anyone | unmeasured; the 2018 app ships `$SIR,15,1,NULL,50` |
...
**Protocol-15 station words** `[jay]`: JBOX and JHALO put a function code in the MAGNITUDE of a protocol-15 word:
6 = respawn (crit 1, every 3000 ms), 8 = perk / KOTH loop, 10 = proximity, 50 = capture notice (matches our
grenade capture word, magnitude 50). Other station words: perk box = protocol 3, subtype 1, magnitude 10 (key
...
| 2 | `GunRangeOutdoor` | 100 on guns, 90 on melee. **Bench 2026-09-17: t2 IS the range lever** (the range test). `[disasm]` **The mechanism is the IR CARRIER FREQUENCY, not power**: the gun emits at `38000 − 125 × (100 − range)` Hz (range ≥ 100 = 38 kHz exactly; 75 = 34.9 kHz; 31 = 29.4 kHz; 5 = 26.1 kHz), and a receiver band-passed around 38 kHz loses sensitivity as the carrier detunes. That is the bench curve: a sharp knee where the carrier enters the band (our 13–31 window is 27.1–29.4 kHz) and little change above. Emitter POWER changes only with the indoor/outdoor level (duty 20 % indoor, 38 % outdoor; `$DUTY` is test-only, every shot overwrites it). Calibrate in kHz, not percent (S48, S49) |
...
Both rates sit within about one standard deviation of the token value. **A crit is the magnitude x1.5, truncated** (9 → 13), the same multiplier our own emitter's C bit produces, and it is **not** the `$GSET` t7 headset scaling, which is a separate axis and ships at 0. `[disasm]` The V4_31 trace names the cause: on a crit roll the shooter multiplies the word's damage by (100 + `$PSET` t6)/100 (`CriticalDamageBonus`, shipped as 50), and the victim's `$GSET` t7 is a second layer on top (§5, the crit bonus; levers sheet §9). A "Critical Strike" perk is therefore one `$PSET` token on the shooter. The crit echoes on `$HIR` token 6, so a proc is visible to the victim's node: this is the mechanism a crit-chance perk, a high-variance weapon, or a poison-on-a-percentage-of-shots round would use.
