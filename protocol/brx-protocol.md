...
| `$LED,<big>,<small>,<effect>,<pulses>,*` | **The headset's SMALL flash LED** (the native "camera flash" on a hit is a separate bi-colour part next to the big RGB LED). APK: LED {LedColorType, isUsedGreenLed, ledEffectType, rateOfPulses} | Bench 2026-09-04 R0BQT, Tony calling: **`$LED,0,1,1,1,*` = ONE green flash "super bright like native"**; token 2 = small LED colour (0 red, 1 green); token 1 = 0 leaves the big LED alone, 1 paints it blue, 2 white, 3 green (alongside the flash); tokens 3/4 made no visible difference at 0-50 (one flash per frame). Used by A11.8 `events[ev].flash`. |
