...
| `$GLED,...` | Set gun LED — fields `mid, effect, optionA, optionB` (from APK, `callsign-extract/protocol-classes.md`); `effect` = LedEffect enum (Solid/Glow/ChaseBack/ChaseForward/StopIR) | The old `<r>,<g>,<b>` reading is **disproven** (§7i). **Token 1 IS a colour index and it overrides the team colour** (bench 2026-08-30: a gun held on `$TID,1` showed yellow, green, purple, teal and white on command), which disproves the older "colour is team-derived only" claim. The index matches the **community 0-8 map**: **0 red · 1 blue · 2 yellow · 3 green · 4 purple · 5 cyan/teal · 6 white** (7-8 observed changing, unnamed). ⚠️ **Index 0 is RED, not off** — an earlier capture records `$GLED,0,0,0,0,10,,*` as red (§below), and the "all dark" seen on 2026-08-30 came from the trailing tokens, not the index. **Do not build night mode on `$GLED,0` alone.** ⚠️ Our own n=1 cell is uncontrolled (the gun was on `$TID,1`, which is already blue) — it is the community map, not our test, that supports 1 = blue. ⚠️ **Tokens 3, 4 and per-LED addressing are NOT understood**: the two 2026-08-30 token-3 tables contradict each other because the LEDs were animating, and token 4 is **enumerated, not boolean** — the Callsign app sends `$GLED,,,,5,,,*` on death. The middle LED has never been deliberately addressed; it showed red (= index 0, the default) throughout. |
