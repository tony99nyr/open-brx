...
| `$TMP,<t1>,…,<t11>,*` | **Temporary modifier set** `[disasm]` | Each token optional (empty leaves it alone). t1/t2/t3 = HP / armour / shield **maximum bonus** (added to `$PSET` t3–t5 in the clamps; a change that leaves HP ≤ 0 kills); t4 = accuracy modifier (fn 23 writes −100 here); t8 = incoming damage % (`damage × (100 + t8) / 100`; `$INVU` writes −100); t9 = magazine % bonus (every slot's magazine += clip × t9 / 100); t11 = the default hit sound when a row has none; t5/t6/t7/t10 stored, use not traced. A native perk system. **Bench-confirmed 2026-09-18 for t4, t8 and t9 (levers sheet §21; credit the `brx-latest-playtest` session for the write-behaviour classification below), none of them resetting the magazine or reserve on their own. Per-token write behaviour:** t1/t2/t8 are **ABSOLUTE** (safe to re-send the same value; t8 = -50 took 4 off a 9-damage hit, t8 = -100 took 0, spawn protection with no `$SIR` table); t4 is **ABSOLUTE** too (`$ALCD` accuracy matches it at once, no walk-back, and a `$WEAP` push does not clear it) but see the smoke collision below; t9 is **ADDITIVE, ONE-SHOT** (top-up is RAW ROUNDS, `clip × t9 / 100`, applied once at write time; a re-send of the SAME t9 value adds again rather than replacing, so an owner that re-sends the whole `$TMP` frame every life must send t9 exactly once per life). `$SPAWN` zeroes every token, confirmed, so a per-life owner re-sends after every spawn regardless of a token's own behaviour. ⚠️ **The two revive paths leave different `$TMP` state behind (bench-confirmed 2026-09-18): `$SPAWN` clears every token, but a `$LIFE,<hp>,0,0,1,*` revive (set mode, on a dead gun) keeps every token as it was.** Anything that owns modifier state must know which revive path it just used, and re-send `$TMP` accordingly. ⚠️ **t4 is last-writer-wins against the gun's own fn 23 (smoke), and fn 23's own ~6 s timer resets t4 to 0 regardless of the last write** (bench-confirmed 2026-09-18): a host writer of t4 must not write while a smoke is active and must re-send its own value once the smoke ends. ⚠️ **`$AMMO` set mode ignores a t9-raised cap**: `$AMMO,<slot>,<n>,<reserve>,1,*` clamped back to the base clip in one bench run, so re-assert t9 after any `$AMMO` set write |
