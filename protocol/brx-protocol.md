...
| `$GLED,<led1>,<led2>,<led3>,<apply-gate>,<brightness>,,*` | **SOLVED 2026-08-30 — three INDEPENDENTLY ADDRESSABLE gun LEDs; palette completed 2026-09-02.** Tokens 1-3 are the three body LEDs front-to-back, each a **direct palette index**, **nine colours 0-8**: **0 red · 1 blue · 2 yellow · 3 green · 4 purple · 5 teal/cyan · 6 white · 7 pink · 8 orange** (9 and 10 are dark). Indices 7 and 8 were read off a gun for the first time on 2026-09-02 with the camera rig; normalised R/G/B signatures, all three LEDs agreeing: 0 = 1.00/0.16/0.26, 1 = 0.19/0.59/1.00, 2 = 0.88/1.00/0.59, 3 = 0.18/1.00/0.54, 4 = 0.59/0.49/1.00, 5 = 0.23/1.00/0.85, 6 = 0.73/0.79/1.00, 7 = 1.00/0.30/0.66, 8 = 1.00/0.38/0.30. Verified one field at a time under a synchronous protocol, then predicted and confirmed: `$GLED,3,2,1,0,10` → **green / yellow / blue**. **Token 4 is an APPLY GATE — not an effect enum, and not an off switch** (corrected 2026-09-02: black-background camera rig, 3 trials per value, pre-state verified, tested from a known GREEN by sending RED, which distinguishes outcomes a dark start cannot). **0, 6, 7, 8, 9, 10 APPLY** the colour tokens at full brightness; **5 APPLIES them at about 1/3 brightness** (measured ratios 0.29/0.41, 0.30/0.44, 0.33/0.44 on two LEDs over three trials); **1, 2, 3, 4 are NO-OPs** — the colour tokens are ignored and the gun keeps whatever it was already showing. **No token-4 value animates** — a trustworthy null, because the positive control (a spawned gun's own native pulse) produces a luminance swing of ~494 against 24-70 for every static state measured here. ⚠️ **RETRACTED 2026-09-02: t4 = 5 is NOT "the off value".** `$GLED,,,,5,,,*` — Callsign's own on-death frame, and the frame we ship for night mode — blanks the gun because **its colour tokens are empty and t4 = 5 applies them**. Applying an empty colour is what turns the LEDs off. There may be **no dedicated off value at all**. The shipped frame is still correct and unchanged; only the rationale was wrong. This retro-explains the earlier oddities: `$GLED,,,,6,,,*` and `$GLED,,,,7,,,*` blanked a lit gun because **6 and 7 also apply** and their colour tokens were empty too; `$GLED,,,,3,,,*` did **not** blank a lit gun because **3 is a no-op**, so the red it was already showing simply stayed. It also explains why four sweeps of this token disagreed with each other: **a no-op leaves the previous row's colour lit**, so a sweep that blanks between rows reports "nothing is lit" and one that does not reports "everything is lit", from identical hardware. **`<brightness>` is token 5**, and it is a three-state control: **0 = off · 1 = dim (~70%) · >= 2 = full** — it saturates by 2, and 2 through 255 are indistinguishable. Measured 3× alternating with no overlap: t5 = 1 gave 172/181/184, t5 = 2 gave 267/254/247. ⚠️ `$GLED` therefore has **two apparent brightness controls** (token 4 = 5, and token 5); whether they compose or one overrides the other is **UNTESTED**. The set colour **alternates with the team colour** rather than replacing it outright. ⚠️ The APK's `mid, effect, optionA, optionB` field names do NOT match this behaviour — that teardown recovers names in declaration order with no types, and it also wrongly claims colour is team-derived. **This makes a 3-segment health/armour gauge buildable.** |
...
| `$LED,<colour>,<useGreenLed>,<effect>,<pulses>,*` | **The headset's SMALL flash LED** (the native "camera flash" on a hit is a separate GREEN-ONLY part next to the big RGB LED). APK: LED {LedColorType, isUsedGreenLed, ledEffectType, rateOfPulses} | Bench 2026-09-04 R0BQT, Tony calling: **`$LED,9,1,1,1,*` = ONE clearly visible green flash on the small LED** (by eye the NATIVE game's green hit flash looked far brighter; **measured with the phone camera, colour-fairly, ours is at least as bright as the HOSTED game's hit flash** -- saturated core 25.2k px vs 24.5k, energy 14.2M vs 9.9M; that hosted flash is BLUE, the team colour. The native-game green flash is not yet measured. Tokens 3/4 at 0-255 and the flag at 2/255 change nothing visible.) (one flash per frame; tokens 3/4 no visible change at 0-50). With token 2 = 1 the big LED is also painted `<colour>` (0 red, 1 blue, 2 white, 3 green) -- use **9 (dark)** to fire the small LED alone (an empty token 1 fires nothing). With token 2 = 0 there is no small-LED flash and the big LED shows `<colour>` (0 = red -- first misread as a red small-LED flash, corrected on the bench). Used by A11.8 `events[ev].flash`. |
