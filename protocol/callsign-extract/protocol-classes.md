...
| 2 | outdoorMode | 0 | 1 = indoor (short range), 0 = outdoor; MC pins 0 (see `brx-protocol.md` `$GSET`). The V4_30 firmware stores this token in the same variable as `$IRLVL` and announces a non-zero value as "Indoor"; the 2018 BC app sends t2 = 1 for its indoor (night) preset and 0 for outdoor (day). cap30 (2026-09-18) agrees: Callsign sends 0 with its venue on OUTDOOR, its default. This is the reverse of what this row said before: our field result (t2 = 1 made a gun deaf at 30 ft) fits indoor being the short-range mode, not the long-range one. |
...
|---|---|---|---|---|
| 0 | 0 | 1 | **slot** | ✓ |
| 2 | 100 | 100 | **`gunRangeOutdoor`** — the field list above names it, and the 2026-09-17 garden bench PROVED it is the range lever (F231/F234): `t2`=5 landed 0 hits from 38 shots at any distance, muzzle to dome included; `t2`=100 reached about 200 ft; the transition sits roughly between 13 and 26 with a flat shelf above about 31. MC writes it from `wire.range_outdoor_pct` OUTDOORS ONLY and floors it at 13. ⚠️ This row used to read "(scale/enable const)" because both captured samples carry 100 and the diff could not discriminate it: the teardown had the NAME right and the meaning unpinned. **The scale is now known, and it is a FREQUENCY, not a power** (V4_31 disassembly, 2026-09-18, `../brx-protocol.md`): the emitter's carrier is `38000 - 125 * (100 - range)` Hz for 0 to 99, and exactly 38 kHz at 100 or more. A low value detunes the word out of the receiver's band-pass near 38 kHz, so the shelf is the pass-band and the knee near 31 (29.4 kHz) belongs to the receiver | ✅ bench-proven |
