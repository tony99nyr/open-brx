...
| `2026-08-26-chargerifle-heat-telemetry.btsnoop` | Charge Rifle + Bolt Rifle. **The trace started after the game armed, so it contains no `$WEAP`** — the weapon stats were missed and need a re-capture. | Accidentally the more valuable half: it **decodes `$ALCD` token 5 as WEAPON HEAT**, a field our own doc listed as "unknown". Heat climbs with sustained fire (56→86→97→**106**) and resets to 14 after cooldown. Non-zero in only this capture and cap16 — the two weapons with an overheat mechanic — and `0` across 400+ frames everywhere else. Live BLE telemetry for a heat bar. |
| `2026-08-26-weapons-chargerifle-boltrifle.btsnoop` | **Charge Rifle** (slot 0, `E03` — hold to charge, **fires on release**, and overheats) and **Bolt Rifle** (slot 1, `R12`). | Completes the `t28`/`t29` model: the Charge Rifle is the only charge weapon that fires on RELEASE and the only one carrying **both** sounds (`C15`/`C17`). `E03` also has `t24`=14 overheat — matching the heat telemetry in cap19 — and is the only weapon with both `t35` and `t36`. **`R12` has neither `t28` nor `t29` despite being called a Bolt Rifle.** |
| `2026-08-26-weapons-plasmasniper-forcerifle.btsnoop` | **Plasma Sniper** (`E17` — single shot, **overheats** if fired fast, shell reload) and **Force Rifle** (`R23` — 3-shot burst). | Third overheat weapon: `t24`=30 with `t35`=`D122`, so `t24`/`t35` now match on 3 of 3. Second burst weapon: `t23`=250. **Breaks the `t19` reloadType reading** — a shell reload with `t19`=0. Shows `t1`=2 co-occurring perfectly with `t12`/`t13`/`t42`. And `R23` (burst) carries the `D20`/`D19` pair while the Burst Rifle doesn't, which the charge model does not explain. |
| `2026-08-26-weapons-stinger-forcerifle.btsnoop` | **Stinger** (`E11`, full auto) and the **Force Rifle** again. | **Resolves the `t28`/`t29` question cap21 opened**: every weapon with the `D20`/`D19` pair also has `t33`=`D21` and no other weapon does, so those are one reload sound family — the pair is two extra RELOAD sounds, and the sound PREFIX (`C…` charge vs `D…` reload) is the discriminator. Also: the Stinger's suspected poison effect is **not** encoded anywhere in `$WEAP`. And `R23` is byte-identical to its cap21 frame, so these frames are deterministic. |
| `2026-08-26-weapons-energyrifle-suppressor.btsnoop` | **Energy Rifle** (`E12` — full auto, **overheats**, 300-round clip) and **Suppressor** (`Q06` — full auto, standard reload, **makes no sound**). | Takes `t24` overheat to **4 of 4** operator-described overheat weapons, with `t35` populated on exactly those four. And the Suppressor is the **only weapon of nineteen** to populate `t25` (=2, muzzleFlash) and `t26` (=50) — the first values ever seen in either — with a unique `Q` sound prefix. |
| `2026-08-26-weapons-ionsniper.btsnoop` | **Ion Sniper** (`E07`, single shot, 2-round clip) + the Suppressor again. | **Disproves a claim made one capture earlier.** cap22 concluded that every weapon with a `t28`/`t29` pair also has `t33`=`D21`; the Ion Sniper carries `D32`/`D31` with `t33`=`D15`. `D19`/`D20`/`D21` is just one reload set three weapons share — the real rule is that a five-part reload fills `t28`/`t29` from the weapon's **own** sound set. `Q06` is byte-identical to its cap23 frame (determinism control). |
