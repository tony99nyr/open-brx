...
The steps for cutting a release-signed APK. The current cut is 0.4.6; its notes are `release-notes/app-v0.4.6.md`,
written to be pasted as the GitHub release body.

## App 0.4.6 cut checklist

The first release-signed build. Tony gives the go; nothing below publishes until then.
...
2. For a dry run, set `APK_OUT_DIR` (`APK_OUT_DIR=/tmp/apk-dry npm run android:release`): the build writes
   ...
   uncommitted tree stamps `0.4.6+<sha>-dirty`, so the APK you publish must come from step 5.
3. Check the signature. Print the fingerprint only; never the keystore, its password or its properties file:
   ...
   - `aapt2 dump badging` must show `versionName='0.4.6'`, `versionCode='406'`.
...
5. Run `cd app && npm run android:release`. It builds the signed APK, copies it to `webapp/download/` as
   `brx-companion-0.4.6-android-release.apk` and writes `webapp/download/build.json` for it (`variant`
   "release", its bytes and sha256, the commit it built from, the `app-v0.4.6` asset URL). It publishes
   nothing. Repeat step 3 on this APK; the stamp must read `0.4.6+<sha>` with no `-dirty`, and the
...
7. Commit `webapp/download/build.json` on top, then push, then **publish at once**: the release `app-v0.4.6`
   with `webapp/download/brx-companion-0.4.6-android-release.apk` and `release-notes/app-v0.4.6.md` as its
   body. Until the release exists, the download page's link 404s.

## Every tester, once

The old app was signed with a test key, and Android refuses an update across different keys.
...
2. Install `app-v0.4.6` from the releases page. Allow "install unknown apps" for the browser if Android asks.
