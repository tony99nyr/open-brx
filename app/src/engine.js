...
export class Engine {
  ...
  _onIrCallout(player, magnitude, now) {
    ...
    // FFA, and LMS or extraction played with one team (Q13, the mirror of compile.py `team_damage_on`): every gun
    // shares one `$TID`, so a bare team match never means friendly (A5.2/contracts.md) and is ENEMY DOWN.
    const c = this.config;
    const solo = !!c && (c.mode === 'ffa' || (['lms', 'extraction'].includes(c.mode) && new Set((c.teams || []).map(t => Number(t.tid))).size < 2));
    const teammate = !!c && !solo && this.teamTid != null && victimTeam === this.teamTid;
