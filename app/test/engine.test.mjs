...
import { Engine, handoverPool, PLAYX, TEAM_REPAINT_MS, frameCommand, deniedCommand } from '../src/engine.js';
...
// ---------- Live accuracy is cut (Tony, 2026-09-18) ----------
// The node used to re-send the whole active-slot `$WEAP` frame plus an `$AMMO` restore every 250 ms under
// sustained fire, to move t21/t22. Long split BLE frames are a suspected cause of gun lock-ups, so the
// writer is gone. This guard proves that sustained fire writes NOTHING to the gun, even when an old
// catalogue row still carries a `recoil` block and an old config still says `recoil: true`.
test('sustained fire writes nothing to the gun -- no mid-life $WEAP, no $AMMO restore', () => {
  const h = harness();
  h.eng.onBleConnected({ name: 'GUN-A-3D4F', basename: 'GUN-A', tail: '3D4F' });
  h.eng.onMcMessage({ kind: 'assign', body: { player: h.player, team: h.team, roster: h.roster,
    catalog: { weapons: [{ weapon_id: 'assault_rifle', name: 'Assault Rifle', clip: 32, reserve: 384, reload_s: 1.4,
      recoil: { ceiling: 100, floor: 20, per_shot: 10, recover_ms: 150 } }], perks: [] } } });
  h.config.recoil = true;   // a legacy key: the node must ignore it
  h.config_().echo().start(0); h.adv(10); h.eng.tick(); h.frame('$LCD,45,70,0,0,36,216,*');
  h.writes.length = 0;
  for (let mag = 35, res = 215; mag >= 20; mag--) {   // 16 rounds, 100 ms apart, then a long quiet spell
    h.frame(`$ALCD,${mag},100,0,${res},0,*`);
    h.adv(100); h.eng.tick();
  }
  h.adv(5000); h.eng.tick();
  assert.equal(h.eng.shots, 16, 'the ordinary shot counter must still count every round');
  assert.deepEqual(h.writes.filter(f => f.startsWith('$WEAP,') || f.startsWith('$AMMO,')), [],
    'a live gun must get no $WEAP or $AMMO write from sustained fire');
});
