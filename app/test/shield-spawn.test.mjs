...
test('F344: the T-0 spawn of a Shields game starts at FULL shield (105), with no recharge and no "shields online"', () => {
  const h = harness();
  const n = h.startAt;
  h.adv(100);
...
test('F344: a timed revive starts at FULL shield too', () => {
  const h = harness();
  h.adv(5000).die();
...
test('F344: a station revive starts at FULL shield and stays there when spawn protection ends', () => {
...
test('F344 control: the Standard preset (no shield) spawns and revives exactly as before, with no pool write', () => {
...
test('F344 control: a shield broken in play still recharges the old way, and says SHIELDS ONLINE at the top', () => {
  const h = harness();
  h.adv(3000);
