...
test('A28.3: pub is dialled first; no welcome within BACKHAUL_GIVEUP_MS falls back to the LAN url', async ctx => {
  ...
  await advance(30);   // giveup fires -> falls back
...
test('A28.3: an immediate error/close on pub (before welcome) falls back to LAN right away, no backoff consumed', async ctx => {
  useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
...
test('A28.3: a pub dial that never fires ANY event (TCP accepted but the upgrade never completes, or a dead/blackholed tunnel) still falls back to LAN after BACKHAUL_GIVEUP_MS', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
...
test('A28.3: while bound on LAN with a pub in hand, checks reachability every PUB_RETRY_MS -- the probe never sends hello, and only opening it drops LAN so the normal dial ladder can claim pub for real', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
  ...
  await advance(5);                  // the LAN close -> offline -> a fresh reconnect (1 ms backoff) fires
...
test('A28.3: a probe that errors/closes before ever opening leaves the LAN socket completely untouched and keeps the normal cadence', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
  ...
  await p;
  assert.equal(t.reach, 'lan');
  await advance(20);   // PUB_RETRY_MS elapses -> a reachability probe
  assert.equal(sockets.length, 3); assert.equal(sockets[2].url, 'wss://pub/ws');
  ...
  await advance(20);   // the normal PUB_RETRY_MS cadence resumes -- another probe, same story
  assert.equal(sockets.length, 4); assert.equal(sockets[3].url, 'wss://pub/ws');
  t.close();
});

test('A28.3: a pub that changes while a probe is in flight is re-checked (against the NEW value) once that probe settles', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
    backoff: { baseMs: 1, capMs: 2, jitter: 0 }, backhaulGiveupMs: 500, pubRetryMs: 15 });
  ...
  await p;
  await advance(5);   // learning a pub for the first time probes it immediately (no PUB_RETRY_MS wait)
...
test('A28.3: _adoptPub(null) aborts an in-flight probe outright -- nothing left to check', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
    backoff: { baseMs: 1, capMs: 2, jitter: 0 }, backhaulGiveupMs: 500, pubRetryMs: 15 });
  ...
  await advance(20);   // nothing left to probe for -- no further sockets appear
  assert.equal(sockets.length, 2, 'no re-probe after an outright abort');
  t.close();
});

test('A28.2/A28.3: hello carries via + secret only when held; status carries reach; welcome.join adopts pub/secret', async ctx => {
  useClock(ctx);
  const { sockets, wsFactory } = factory();
...
test('A28.2: a re-hello (after any reconnect) carries the held secret', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' }, backoff: { baseMs: 1, capMs: 2, jitter: 0 } });
  ctx.after(() => t.close());
...
test('A28.2: MC push join {pub:null} while on backhaul drops pub and falls back to LAN via the normal loop', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' }, backoff: { baseMs: 1, capMs: 2, jitter: 0 } });
  ctx.after(() => t.close());
...
test('A28.3: a join push handing us a brand-new pub while bound on LAN checks reachability right away, no PUB_RETRY_MS wait', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  ...
  await advance(5);   // no 30s advance -- the reachability check is immediate, not on the next PUB_RETRY_MS tick
  ...
  await advance(5);      // dropLink -> offline -> reconnect backoff -> the REAL pub dial
...
test('A28.3: welcome.join handing us a brand-new pub on the very first connect (dialled over LAN) also checks reachability right away', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' },
    backoff: { baseMs: 1, capMs: 2, jitter: 0 }, pubRetryMs: 30000 });
...
test('A28.2: pub/secret are session-scoped -- a session change (new session_id) clears them; unchanged across a same-session reconnect', async ctx => {
  const advance = useClock(ctx);
  const { sockets, wsFactory } = factory();
...
test('A28.2: pub/secret are bound to the LAN url that issued them -- connecting to a DIFFERENT url drops them first', async ctx => {
  useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' }, backoff: { baseMs: 1, capMs: 2, jitter: 0 } });
  ctx.after(() => t.close());
...
test('A28.2: reconnecting to the SAME url the pub/secret were learned for keeps them (no false url-change clear)', async ctx => {
  useClock(ctx);
  const { sockets, wsFactory } = factory();
  const store = memoryStorage();
  const t = new Transport({ storage: store, wsFactory, gun: { name: 'GUN-A', tail: '3D4F' }, backoff: { baseMs: 1, capMs: 2, jitter: 0 } });
...
test('A28.2: a cosmetically different but equivalent url (case, a trailing slash) is NOT "a different MC" -- pub/secret survive', async ctx => {
  useClock(ctx);
  const { sockets, wsFactory } = factory();
  const t = new Transport({ storage: memoryStorage(), wsFactory, gun: { name: 'GUN-A', tail: '3D4F' }, backoff: { baseMs: 1, capMs: 2, jitter: 0 } });
  ctx.after(() => t.close());
