...
for (const view of VIEWS) {
  ...
  await step(`${view.name} #40 SWITCHING takeover: from → to, then ACTIVE once the swap window closes`, async () => {
  ...
  await step(`${view.name} #25/#26 DOWN: recap instead of ghost numbers`, async () => {
    const read = () => pg.evaluate(() => ({ ghost: !!document.querySelector('.down .ghost'), recap: (document.querySelector('.down .recap') || {}).textContent || '', bottoms: Array.from(document.querySelectorAll('.down .recap > .rc')).map(s => { const b = s.getBoundingClientRect(); return Math.round((b.top + b.bottom) / 2 / 4); }), tiles: document.querySelectorAll('.down .recap > .rc').length, chips: document.querySelectorAll('.down .recap .tm').length }));
...
// The death screen (2026-09-23, deathscreen.js). Tony: "what were we killed by and just a few callouts". Every stage is a
// REAL life through the engine (demo.js `fullLife`), read off the rendered screen: the killer line and final hit, four
// callouts (taken, dealt, kills, alive) and the game strip. The numbers are asserted through each block's aria-label,
// which is what a screen reader says and what the drawn number must match.
const dsGeom = pg => pg.evaluate(src => { const dsUncovered = () => eval(src);
  ...
  const f = document.getElementById('frame').getBoundingClientRect();
  const blocks = { head: box(document.querySelector('.down .dsx .l2')), life: box(document.getElementById('dslife')), count: box(document.querySelector('.down .dn')),
    pills: [...document.querySelectorAll('#frame[data-down] .chipbar .pill')].map(box).filter(Boolean),
    ...
    callouts: [...document.querySelectorAll('#dslife .co')].map(box).filter(Boolean) };
  const clipped = [...document.querySelectorAll('.mo.down *')].filter(e => { const cs = getComputedStyle(e); return !e.classList.contains('sr') && cs.display !== 'none' && (e.scrollWidth > e.clientWidth + 1 && cs.overflowX !== 'visible'); }).map(e => e.className);   // .sr is clipped on purpose: read aloud, not drawn
  // a callout's number must stay inside its own cell: a 4-digit total or 12:34 must not run into the next one
  const spill = [...document.querySelectorAll('#dslife .co')].filter(c => { const n = c.querySelector('b'); return n && n.getBoundingClientRect().right > c.getBoundingClientRect().right + 1; }).map(c => c.getAttribute('aria-label'));
  const rd = document.getElementById('rd');
  // text-size sweep: every visible text leaf on the death screen, in the 844x390 frame's own px (the HUD's floor is 11)
  const small = [...document.querySelectorAll('.mo.down *')].filter(e => { const cs = getComputedStyle(e); const own = [...e.childNodes].some(n => n.nodeType === 3 && n.textContent.trim());
    return own && !e.closest('.sr') && cs.display !== 'none' && cs.visibility !== 'hidden' && parseFloat(cs.fontSize) < 11; }).map(e => `${e.className || e.tagName}:${getComputedStyle(e).fontSize}:${e.textContent.trim().slice(0, 20)}`);
  return { small, spill, blocks, frame: { l: f.left, r: f.right, t: f.top, b: f.bottom }, clipped, rdLines: rd ? rd.offsetHeight / parseFloat(getComputedStyle(rd).lineHeight) : null,
    top: dsUncovered() };
}, dsUncoveredSrc);
// #overlay is pointer-events:none, so elementFromPoint cannot see it. "Covered" is geometric instead: no visible
// element that stacks above the death screen (#chips, or another moment in #overlay) lies on the killer or the callouts.
const dsUncoveredSrc = `(() => { const box = e => e && e.getBoundingClientRect();
  const mine = ['.mo.down .dsx .l2', '#dslife'].map(q => box(document.querySelector(q))).filter(b => b && b.width);
  if (mine.length < 2) return 'absent';   // a missing or zero-size block is a failure, never a pass
...
const dsCheck = (g, label) => {
  ...
  for (const n of ['head', 'life', 'count']) must(B[n], `${label}: the ${n} block is missing or has no size`);
  const all = [['head', B.head], ['life', B.life], ['count', B.count], ...B.pills.map((p, i) => ['pill' + i, p]), ['capwarn', B.capwarn], ['safe', B.safe], ...B.tiles.map((t, i) => ['tile' + i, t])].filter(x => x[1]);
  ...
  for (let i = 1; i < B.callouts.length; i++) must(!hit(B.callouts[i - 1], B.callouts[i]), `${label}: callout ${i - 1} overlaps callout ${i}`);
  must(g.spill.length === 0, `${label}: a callout number runs out of its cell: ${g.spill.join(', ')}`);
  must(g.clipped.length === 0, `${label}: clipped text in ${g.clipped.join(', ')}`);
  must(g.small.length === 0, `${label}: text under the 11px floor: ${g.small.join(', ')}`);
  ...
  must(new Set(B.tiles.map(x => Math.round((x.t + x.b) / 2 / 4))).size <= 1, `${label}: the game strip wrapped: ${JSON.stringify(B.tiles)}`);
};
const dsText = pg => pg.evaluate(() => { const t = id => { const e = document.getElementById(id); return e ? e.innerText.replace(/\s+/g, ' ').trim() : null; };
  const f = document.getElementById('dsfinal');
  return { final: f ? f.getAttribute('aria-label') : null, finalText: t('dsfinal'), life: t('dslife'), game: t('downrecap'), kb: ((document.querySelector('.mo.down .kb') || {}).textContent || '').replace(/\s+/g, ' ').trim(),
    co: [...document.querySelectorAll('#dslife .co')].map(e => e.getAttribute('aria-label')), tiles: [...document.querySelectorAll('#downrecap .rc')].map(e => e.getAttribute('aria-label') || e.innerText.replace(/\s+/g, ' ').trim()),
    stale: [...document.querySelectorAll('#downrecap .rc.stale .ag')].map(e => e.textContent) }; });
for (const view of VIEWS) for (const skin of ['', '&night']) {
  const sk = skin ? ' night' : '';
  await step(`${view.name}${sk} death screen, full life: the killer, the final hit, four callouts, the game strip`, async () => {
    const pg = await open(view, 'down-full', skin, 5200);   // past the 2 s death grace: the link held, so nothing reads "+"
...
for (const view of VIEWS) {
  await step(`${view.name} death screen, partial: a dropped MC link greys dealt and kills, with no "+"`, async () => {
    const pg = await open(view, 'down-partial', '', 5200);
    const t = await dsText(pg), g = await dsGeom(pg), unsure = await pg.evaluate(() => [...document.querySelectorAll('#dslife .co.unsure')].map(e => e.className.split(' ')[1])); await pg.close();
    must(t.co[1] === 'damage dealt at least 48' && t.co[2] === 'kills confirmed so far 1' && !/\+/.test(t.life), 'partial: ' + JSON.stringify([t.co, t.life]));
    must(unsure.join() === 'dl,ki', 'dealt and kills are greyed while a relay may be missing: ' + unsure);
    dsCheck(g, 'partial');
  });
  await step(`${view.name} death screen, unclear weapon: two candidates are named with a slash, never guessed`, async () => {
    const pg = await open(view, 'down-unclear', '', 4200);
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.final === 'killed with ASSAULT RIFLE or USP-S' && t.finalText === 'ASSAULT RIFLE / USP-S', 'killing weapon: ' + JSON.stringify([t.final, t.finalText]));
    dsCheck(g, 'unclear');
  });
  await step(`${view.name} death screen, zero dealt: with no report the DEALT callout is not drawn at all`, async () => {
    const pg = await open(view, 'down-zero-dealt', '', 5200);
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(!t.co.some(x => /dealt/.test(x)) && /^115 TAKEN 0 KILLS \d+:\d\d ALIVE$/.test(t.life), 'no dealt callout: ' + JSON.stringify([t.co, t.life]));
    dsCheck(g, 'zero');
  });
  await step(`${view.name} death screen, pickup: a weapon off the wider catalogue is marked PICKUP`, async () => {
    ...
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.final === 'killed with SHOTGUN, a pickup' && t.finalText === 'SHOTGUN PICKUP', 'pickup: ' + JSON.stringify([t.final, t.finalText]));
    dsCheck(g, 'pickup');
  });
  await step(`${view.name} death screen, FFA: the strip gives your place and the leader`, async () => {
    const pg = await open(view, 'down-ffa', '', 4200);
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.tiles.some(x => x === 'your place 2ND of 4, first to 25') && /2ND \/4 VIPER 13/.test(t.game), 'ffa: ' + JSON.stringify(t.tiles));
    dsCheck(g, 'ffa');
  });
  await step(`${view.name} death screen, hill: the strip names who holds the hill`, async () => {
    const pg = await open(view, 'down-hill', '', 4200);
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.tiles.includes('hill held by blue'), 'hill: ' + JSON.stringify(t.tiles));
    dsCheck(g, 'hill');
  });
  await step(`${view.name} death screen, stale board: old numbers carry their age, never pass as current`, async () => {
    ...
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.stale.length === 2 && t.stale.every(x => /^\d+ S AGO$/.test(x)), 'stale tags: ' + JSON.stringify(t.stale));
    dsCheck(g, 'stale');
  });
  ...
  await step(`${view.name} death screen, big numbers: a 4-digit total, 3-digit dealt and 12:34 alive stay in their cells`, async () => {
    const pg = await open(view, 'down-full', '', 4200);
    await pg.evaluate(() => { const e = window.brx.engine, real = e.state.bind(e);
      const L = { taken: [{ num: 19, name: 'VIPER', teamKey: 'yellow', dmg: 1250, hits: 30, weapons: [] }], takenTotal: 1250, dealt: [{ victim: 'p9', name: 'GHOSTWALKER', dmg: 999, hits: 40, weapons: [] }], dealtTotal: 999,
  ...
  // block into one word per line. The killer row stays one line and the final hit at most two, beside every hint.
  for (const [stage, ms] of [['down-find', 6200], ['down-at', 3300], ['down-hold', 3300]]) await step(`${view.name} death screen beside the station hint (${stage}): the killer block keeps its width`, async () => {
    ...
    const g = await dsGeom(pg), kb = await pg.evaluate(() => { const k = document.querySelector('.mo.down .kb'), b = k && k.querySelector('b'); return k && b ? { h: k.offsetHeight, chip: b.offsetHeight } : null; }), fl = await pg.evaluate(() => { const e = document.getElementById('dsfinal'); return e ? e.offsetHeight : null; });
    await pg.screenshot({ path: `${OUT}/${view.name}-${stage}-deathscreen.png` }); await pg.close();
    must(kb && kb.h <= kb.chip + 6, 'the killer row wrapped (the row is taller than its chip): ' + JSON.stringify(kb));   // oneLine() cannot judge a row with a padded chip in it
    must(fl != null && fl <= 64, 'the final-hit line took ' + fl + 'px, more than two lines');
    dsCheck(g, stage);
  });
  ...
  await step(`${view.name} death screen, an older MC: no roster weapons, scores or relays, and nothing invented`, async () => {
    const pg = await open(view, 'down-old-mc', '', 5200);   // past the 2 s death grace
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.final == null && !t.co.some(x => /dealt/.test(x)), 'no weapon is named and no dealt callout: ' + JSON.stringify([t.final, t.co]));
    must(/FIRST TO 25/.test(t.game), 'older MC strip: ' + t.game);
    dsCheck(g, 'old mc');
  });
  await step(`${view.name} death screen, poison: the lethal tick names POISON`, async () => {
    const pg = await open(view, 'down-poisoned', '', 6500);
    const t = await dsText(pg), g = await dsGeom(pg); await pg.close();
    must(t.kb === 'POISONED BY VIPER' && t.final === 'killed with POISON' && t.finalText === 'POISON', 'poison: ' + JSON.stringify([t.kb, t.final, t.finalText]));
    dsCheck(g, 'poison');
  });
...
const ugRead = pg => pg.evaluate(() => {
  ...
  if (!cfgOpen) {
    ...
    for (const [id, f] of [['thrSrc', 'threshold'], ['txSrc', 'tx_power']]) {
      const want = f === 'tx_power' && !U.support.txPowerControl ? "THIS PHONE CAN'T SET IT" : R.src(f) === 'mc' ? 'MC' : R.src(f) === 'station' ? (R.pending(f) ? 'SET HERE · WILL SYNC' : 'SET HERE') : ((f === 'threshold' ? !U.settings.threshold : U.settings.tx === 'high') ? 'DEFAULT' : '');
...
// REAL engine with a powerup game and fake station adverts (demo.js `puAt`/`puTake`). Each step reads what a person
// sees: the hint's words and type sizes in FRAME px (computed font-size is layout px, which the #frame scale leaves
// alone), its box against the vitals and the ammo, the held chip in the ammo column, the callout cards, the overshield
// block. Day and night, at both widths. The 2026-09-23 type floors: a label >= 11 px, an action >= 14 px.
const puRead = pg => pg.evaluate(() => {
  const vis = e => { if (!e) return false; for (let n = e; n && n.id !== 'frame'; n = n.parentElement) { const c = getComputedStyle(n); if (c.display === 'none' || c.visibility === 'hidden' || +c.opacity < .3) return false; } const r = e.getBoundingClientRect(); return r.width > 0 && r.height > 0; };
...
// then TAKEN with the winner and the countdown. Driven through the REAL wire: `station_config` with the item, then
// MC's `station_update`, then a fake player phone (utility.js `seedDemo`) that claims and turns claim_ready.
const PU_ITEM = { kind: 'weapon', weapon_id: 'rocket_launcher', charges: 2, spawn_every_s: 120, first_at_s: 120, name: 'ROCKETS', color: '#ff7a1a' };
const puStation = pg => pg.evaluate(() => { const t = id => { const e = document.getElementById(id); return e && !e.hidden && e.getBoundingClientRect().height > 0 ? e.textContent.trim() : null; };
  const ring = document.getElementById('pring'); return { name: t('team'), kind: t('kind'), state: t('pstate'), next: t('pnext'), taker: t('ptaker'), pup: !!document.getElementById('pup') && !document.getElementById('pup').hidden,
    ring: ring && !ring.hidden ? parseFloat(ring.style.getPropertyValue('--p')) : null, docH: document.documentElement.scrollHeight, vh: innerHeight,
    px: (() => { const e = document.getElementById('pstate'); return e ? parseFloat(getComputedStyle(e).fontSize) : 0; })(), advert: window.brxUtility.advertFields() }; });
for (const view of UTIL_VIEWS) {
  await step(`utility A56 powerup station @ ${view.name}: its item, AVAILABLE, the claim ring, then TAKEN BY PLAYER 7 with the countdown`, async () => {
    ...
    must(taken.state === 'TAKEN' && taken.taker === 'BY PLAYER 7' && /^NEXT 1:5\d$/.test(taken.next || ''), `taken: ${JSON.stringify(taken)}`);
    must(taken.advert.state === 0 && taken.advert.taker === 7 && taken.advert.value >= 110, `the advert names the taker: ${JSON.stringify(taken.advert)}`);
    must(taken.px >= 11 && taken.docH <= taken.vh, `fits the landscape screen, labels >= 11 px: ${JSON.stringify(taken)}`);
  });
}
